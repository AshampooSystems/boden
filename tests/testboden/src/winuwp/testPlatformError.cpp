...
void testErrorCodeMapping()
{
	...
	std::list<ExpectedMapping> expectedMappings(
	{
        ...
        // even though HRESULT are the basic error code of winuwp apps, the runtime lib
        // currently does NOT map them to generic error conditions. We want to know if that
        // is ever changed, so we add tests that verify the current behaviour.
...
void verifyToFromPlatformException(::Platform::Exception^ pPlatformException, int expectedCode)
{
    ...
    REQUIRE( String( pPlatformException->GetType()->FullName->Data() ) == String( pOutException->GetType()->FullName->Data() ) );
