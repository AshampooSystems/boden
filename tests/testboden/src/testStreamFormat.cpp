...
static void _testStreamFormatEffect(FORMAT_REF &&format)
{
    ...
    SECTION("on ad hoc stream")
    {
        ...
        // To solve this we probably have to refactor StreamFormat and wrap the
        // parameters to format in custom objects. See
        // https://systems.atlassian.net/browse/BDN-57 REQUIRE( (StringBuffer()
        // << StreamFormat().alignRight(8) << "hello") == "   hello" );
    }
...
{
    SECTION("temporary") { _testStreamFormat(StreamFormat(), true); }
