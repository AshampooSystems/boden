...
void verifyLocaleEncoderWithLocale(const String& inString, const std::locale& loc, bool knowIsUtf8, bool containsSurrogatePairCharacters)
{	
	std::string expected;

	// if we know that the codec is UTF-8 then we use our own codec implementation
	// to generate the expected result.
	// BUT we only do that if the string does not contain characters that are encoded
	// as surrogate pairs in the wchar representation. The reason is that the standard
	// library codec often does not handle surrogate pairs in the input data correctly
	// and will generate two separate UTF-8 sequences for each pair element.
	// That is something that we cannot prevent, since the locale codec is simply
	// buggy. So we must adapt our own expected string accordingly. We do that
	// by using wideToLocaleEncoding to generate it (which also uses the codec).
	...
	else
	{
		...
		// is not encodable.
		// So, for our test to work we need to replace such double ? in expected with
		// single ?. BUT we cannot simply replace any ?? with ?, since the unicode
		// character may also occur twice in the input.
		// So instead we use our knowledge of the input string and replace
...
void verifyLocaleEncoder(const String& inString, bool containsSurrogatePairCharacters)
{
	...
	SECTION("classic utf8 locale")
		verifyLocaleEncoderWithLocale(inString, deriveUtf8Locale( std::locale::classic() ), true, containsSurrogatePairCharacters );
...
{	
	...
	for(int t=0; t<dataCount; t++)
	{
		...
		{
			verifyLocaleEncoder( L"hello" + pCurrData->inString
                                    + L"wo" + pCurrData->inString+pCurrData->inString
                                    + L"rld",
