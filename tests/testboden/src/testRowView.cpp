...
static void
testChildAlignment(P<bdn::test::ViewTestPreparer<RowView>> pPreparer,
                   ...
                   View::VerticalAlignment vertAlign)
{
    ...
    if (pButton->verticalAlignment() == vertAlign) {
        ...
        // of the requested alignment is registered as a change
        pButton->setVerticalAlignment(vertAlign == View::VerticalAlignment::top
                                          ? View::VerticalAlignment::bottom
                                          : View::VerticalAlignment::top);
    }

    CONTINUE_SECTION_WHEN_IDLE(pPreparer, pRowView, pButton, horzAlign,
                               vertAlign)
    {
        int layoutCountBefore =
            cast<bdn::test::MockViewCore>(pRowView->getViewCore())
                ->getLayoutCount();
        ...
        SECTION("vertical")
        {
            ...
            CONTINUE_SECTION_WHEN_IDLE(pPreparer, pRowView, pButton, vertAlign,
                                       layoutCountBefore)
            {
                // but layout should have happened
                REQUIRE(cast<bdn::test::MockViewCore>(pRowView->getViewCore())
                ...
                Rect containerBounds =
                    Rect(pRowView->position(), pRowView->size());
                ...
                if (vertAlign == View::VerticalAlignment::top) {
                    ...
                    double expectedY =
                        containerBounds.height - margin.bottom - bounds.height;
                    REQUIRE_ALMOST_EQUAL(bounds.y, expectedY, 0.0000001);
                } else if (vertAlign == View::VerticalAlignment::expand) {
                    REQUIRE_ALMOST_EQUAL(bounds.y, margin.top, 0.0000001);
                    REQUIRE_ALMOST_EQUAL(bounds.height,
                                         containerBounds.height - margin.top -
                                             margin.bottom,
                                         0.0000001);
                }
            };
        ...
        SECTION("horizontal")
        {
            ...
            CONTINUE_SECTION_WHEN_IDLE(pPreparer, pRowView, pButton,
...
TEST_CASE("RowView")
{
    ...
    SECTION("RowView-specific")
    {
        P<bdn::test::ViewTestPreparer<RowView>> pPreparer =
            newObj<bdn::test::ViewTestPreparer<RowView>>();
        P<bdn::test::ViewWithTestExtensions<RowView>> pRowView =
            pPreparer->createView();
        P<bdn::test::MockContainerViewCore> pCore =
            cast<bdn::test::MockContainerViewCore>(pRowView->getViewCore());
        ...
        SECTION("child alignment")
        {
            for (int horzAlign = (int)View::HorizontalAlignment::left;
                ...
                for (int vertAlign = (int)View::VerticalAlignment::top;
                    ...
                    {
                        SECTION("no margin")
                        testChildAlignment(pPreparer, pRowView, pButton,
        ...
        SECTION("calcContainerPreferredSize")
        {
            SECTION("availableWidth bigger than needed")
            {
                ...
                Size size = pRowView->calcContainerPreferredSize(
        ...
        SECTION("child views detached before destruction begins")
        {
            ...
            pRowView->setDestructFunc(
                [pData, pButton](
                    bdn::test::ViewWithTestExtensions<RowView> *pColView) {
...
SECTION("multiple child views properly arranged")
{
    ...
    CONTINUE_SECTION_WHEN_IDLE(pPreparer, pRowView, pButton, pButton2, pCore, m,
                               m2)
    {
        Rect bounds = Rect(pButton->position(), pButton->size());
