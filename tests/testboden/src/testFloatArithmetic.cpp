...
static void testFloatArithmethic()
{
    ...
    SECTION("has NaN")
    {
        REQUIRE( std::numeric_limits<Type>::has_quiet_NaN );
    }
    ...
    // Operations involving infinite and nan as operands can produce incorrect results
    // if aggressive floating point optimizations
    // are enabled (-ffast-math or -ffinite-math-only)
     
    // However, these flags are used quite a bit, so we, as a framework,
    // want to work properly when they are enabled. So we do not assume that nan and infinity
    // operations are standards compliant.

    // We do still perform these tests when we do not detect the fast-math option to be on.
