...
{
	std::basic_stringbuf<char32_t, typename STREAM_TYPE::traits_type>* pBuffer = dynamic_cast< std::basic_stringbuf<char32_t, typename STREAM_TYPE::traits_type>* >( stream.rdbuf() );
...
std::basic_ostream<CHAR,TRAITS>& operator<<( std::basic_ostream<CHAR,TRAITS>& stream, const TestTextOutStreamSupportedDummy_& x)
{
	return stream << "dummy";
}

template<typename STREAM_TYPE>
static void _testTextOutStream()
{
	...
	SECTION("string")
	{
		...
		SECTION("std::string")	// should be the same as basic_string<char> but we test it anyway
			_verifyString< std::string >(stream);

		SECTION("std::basic_string<wchar_t>")
			_verifyString< std::basic_string<wchar_t> >(stream);

		SECTION("std::wstring")	// should be the same as basic_string<wchar_t> but we test it anyway
	...
	SECTION("void*")
	{
		...
		// the format in which pointers are printed depends on the implementation.
		// However, all known implementations print the address in hex, either with or
		// without a leading 0x. Some pad with zeros.
...
TEST_CASE("TextOutStream")
{
	...
	// We do all tests with each type name, just to verify that both work exactly the same.
	// Note that since they are in different namespaces there is potential for overloading
	// rules to work differently (although they should not).
