...
public:
    ...
    {
        // we do not care about this for these tests. But we have to do a little bit
        // so that the standard view tests succeed.
...
TEST_CASE("ContainerView")
{
	...
	{
		P<bdn::test::ViewTestPreparer<DummyContainerViewForTests> >         pPreparer = newObj< bdn::test::ViewTestPreparer<DummyContainerViewForTests> >();
		P< bdn::test::ViewWithTestExtensions<DummyContainerViewForTests> >  pContainerView = pPreparer->createView();
		P<bdn::test::MockContainerViewCore>                                 pCore = cast<bdn::test::MockContainerViewCore>( pContainerView->getViewCore() );
        ...
        {   
            ...
            SECTION("addChildView")
            {
                ...
                {
                    int layoutCountBefore = cast<bdn::test::MockViewCore>(pContainerView->getViewCore())->getLayoutCount();
                    ...
                    CONTINUE_SECTION_WHEN_IDLE(pPreparer, pContainerView, pButton, pCore, layoutCountBefore)
                    {
                        ...
                        REQUIRE( cast<bdn::test::MockViewCore>(pContainerView->getViewCore())->getLayoutCount()==layoutCountBefore+1 );
        ...
        SECTION("_childViewStolen")
        {
            ...
            SECTION("one")
            {
                ...
                SECTION("stole first")
                {
                    ...
                    // stolen view should not be accessed, so parent should still be set there
                    REQUIRE( pButton->getParentView() == cast<View>(pContainerView) );
        ...
        SECTION("findPreviousChildView")
        {
            SECTION("empty")
            {
                SECTION("not a child")
                {
                    REQUIRE( pContainerView->findPreviousChildView(pNonChild) == nullptr );
                }
            }
            ...
            SECTION("one")
            {
                ...
                SECTION("first")
                {
                    REQUIRE( pContainerView->findPreviousChildView(pButton) == nullptr );
                }
