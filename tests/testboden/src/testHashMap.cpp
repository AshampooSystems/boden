...
namespace bdn
{
    namespace test
    {
        ...
        template <> struct CollectionElementOrderUndefined_<bdn::HashMap<int, double>>
        {
            enum
            {
                value = 1
        ...
        struct CollectionElementOrderUndefined_<
            bdn::HashMap<TestCollectionElement_OrderedComparable_, TestCollectionElement_UnorderedComparable_>>
        {
            enum
            {
                value = 1
...
static void testHashMap(std::initializer_list<std::pair<const KeyType, ValType>> initElList,
                                  ...
                                  const std::pair<const KeyType, ValType> &elNotInList)
{
    SECTION("empty")
    {
        ...
        {
            coll.findCustomAndRemove([elNotInList](const typename HashMap<KeyType, ValType>::Iterator &it) {
    ...
    SECTION("not empty")
    {
        SECTION("found")
        {
            ...
            for (auto &el : elList) {
                SECTION(std::to_string(elIndex))
                {
                    ...
                    SECTION("findCustomAndRemove")
                    {
                        coll.findCustomAndRemove([el](const typename HashMap<KeyType, ValType>::Iterator &it) {
        ...
        SECTION("not found")
        {
            ...
            SECTION("findCustomAndRemove")
            {
                coll.findCustomAndRemove([](const typename HashMap<KeyType, ValType>::Iterator &it) { return false; });
                _verifyGenericCollectionReadOnly(coll, expectedElements);
            }
        }
        ...
        SECTION("all match")
        {
            ...
            SECTION("findCustomAndRemove")
            {
                coll.findCustomAndRemove([](const typename HashMap<KeyType, ValType>::Iterator &it) { return true; });
...
TEST_CASE("HashMap")
{
    ...
    {
        std::unordered_map<int, double> tempMapForOrder{{17, 1.7}, {42, 4.2}, {3, 0.3}};

        std::list<std::pair<const int, double>> expectedInitElOrder(tempMapForOrder.begin(), tempMapForOrder.end());

        testHashMap<int, double>({{17, 1.7}, {42, 4.2}, {3, 0.3}}, expectedInitElOrder,
