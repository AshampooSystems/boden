...
namespace bdn
{
    namespace test
    {
        ...
        struct CollectionElementOrderUndefined_<
            bdn::HashMap<TestCollectionElement_OrderedComparable_,
...
template <typename COLL> static void _testHashMapToString(COLL &coll)
{
    String expected;
...
static void
testHashMap(std::initializer_list<std::pair<const KeyType, ValType>> initElList,
            ...
            ConstructArgs... constructArgs)
{
    SECTION("test traits")
    {
        REQUIRE(!(bdn::test::CollectionSupportsBiDirIteration_<
                  HashMap<KeyType, ValType>>::value));
    }
    ...
    SECTION("construct")
    {
        ...
        SECTION("iterators")
        {
            SECTION("empty")
            {
                HashMap<KeyType, ValType> coll(newElList.begin(),
                                               newElList.begin());
        ...
        SECTION("copy")
        {
            ...
            SECTION("std::unordered_map")
            {
                ...
                HashMap<KeyType, ValType> coll(
                    (const std::unordered_map<KeyType, ValType> &)src);
    ...
    SECTION("non-empty")
    {
        ...
        {
            typename HashMap<KeyType, ValType>::Element elToAdd =
                *newElList.begin();
            std::list<typename HashMap<KeyType, ValType>::Element>
        ...
        SECTION("add ops with existing key")
        {
            SECTION("single el")
            {
                ...
                SECTION("add(key, value) movable")
                {
                    ...
                    SECTION("both")
                    {
                        coll.add(std::move(elToAdd.first),
                                 std::move(elToAdd.second));
                    ...
                    SECTION("key")
                    {
                        ...
                        // we must manually move the value away so that
                        // isMovedRemnant can be used
                    ...
                    SECTION("value")
                    {
                        ...
                        // we must manually move the key away so that
                        // isMovedRemnant can be used
    ...
    SECTION("findAll(key)")
    {
        SECTION("non const")
        {
            _testCollectionFindXWithCaller<HashMap<KeyType, ValType>>(
                elList, elNotInList,
                [](HashMap<KeyType, ValType> &coll,
                   const typename HashMap<KeyType, ValType>::Element
                       &elToFind) { return coll.findAll(elToFind.first); });
        }
        ...
        SECTION("const")
        {
            _testCollectionFindXWithCaller<HashMap<KeyType, ValType>>(
                ...
                [](HashMap<KeyType, ValType> &coll,
                   const typename HashMap<KeyType, ValType>::Element
                       &elToFind) {
                    return ((const HashMap<KeyType, ValType> &)coll)
            ...
            for (auto &el : elList) {
                SECTION(std::to_string(elIndex))
                {
                    ...
                    SECTION("findCustomAndRemove")
                    {
                        coll.findCustomAndRemove(
                            [el](
                                const typename HashMap<KeyType,
                                                       ValType>::Iterator &it) {
...
TEST_CASE("HashMap")
{
    SECTION("key simple type")
    {
        ...
        std::list<std::pair<const int, double>> expectedInitElOrder(
            tempMapForOrder.begin(), tempMapForOrder.end());

        testHashMap<int, double>(
            {{17, 1.7}, {42, 4.2}, {3, 0.3}}, expectedInitElOrder,
    ...
    SECTION("addSequence with compatible but different type")
    {
        ...
        // the ordering is implementation defined, so we must use a temporary
        // hashmap to get it.
        std::list<std::pair<const int, std::string>> expectedElementList;
        {
            HashMap<int, std::string> tempMap(
                {{1, std::string("hello")}, {2, std::string("world")}});

            expectedElementList.insert(expectedElementList.begin(),
    ...
    {
        testHashMap<TestCollectionElement_OrderedComparable_,
                    TestCollectionElement_UnorderedComparable_>(
            {
