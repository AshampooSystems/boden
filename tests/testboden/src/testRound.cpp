...
static void testRound(RoundType roundType, double in, double scaleFactor,
                      double expectedOut)
{
    SECTION("roundType: " + std::to_string((int)roundType) +
            ", value: " + std::to_string(in) +
...
TEST_CASE("stableScaledRound")
{
    SECTION("factor=0")
    {
        ...
        REQUIRE_THROWS_AS(stableScaledRound(RoundType::down, 1, 0),
                          InvalidArgumentError);
        REQUIRE_THROWS_AS(stableScaledRound(RoundType::up, 1, 0),
                          InvalidArgumentError);
        REQUIRE_THROWS_AS(stableScaledRound(RoundType::nearest, 1, 0),
                          InvalidArgumentError);
    }
