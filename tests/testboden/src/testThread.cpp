...
{
  public:
    TestJoinRunnable(bool throwException) { _throwException = throwException; }
...
void verifyExec(bool throwException)
{
    ...
    {
        {
            std::future<int> result = Thread::exec(
                [throwException](int val) {
                    Thread::sleepSeconds(2);
                    if (throwException) {
                        throw InvalidArgumentError("hello");
...
class ThreadLocalTestData : public Base
{
  public:
    ...
    ~ThreadLocalTestData() { destructedCount++; }
...
TEST_CASE("ThreadLocalStorage")
{
    ...
    SECTION("directCall") { getThreadLocalSomeType().dummy(); }
    ...
    {
        // here we simply test that there is no low limit to the number of
        // thread local objects one can have
        p1().dummy();
        p2().dummy();
        p3().dummy();
        p4().dummy();
