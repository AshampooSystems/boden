...
static void testTypeSupportsShiftLeftWith()
{
    ...
    // the following verifies that the function can be used in compile time statements.
    static_assert( typeSupportsShiftLeftWith<std::ostream, int>(), "typeSupportsShiftLeftWith must be usable as a compile expression" );
}
...
static void testTypeHasCustomArrowOperator()
{
    ...
    // the following verifies that the function can be used in compile time statements.
    static_assert( typeHasCustomArrowOperator< P<Base> >(), "typeHasCustomArrowOperator must be usable as a compile expression" );
}
