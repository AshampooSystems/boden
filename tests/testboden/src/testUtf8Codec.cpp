...
public:
	...
	DummyUtf8TestIterator_& operator++()
	{
		return *this;
	}
...
TEST_CASE( "Utf8Codec", "[string]" )
{
	...
	SECTION("iterator category")
	{
		SECTION("decoding")
		{
			SECTION("inner input")
			{
				REQUIRE( typeid( typename Utf8Codec::DecodingIterator< DummyUtf8TestIterator_<uint8_t, std::input_iterator_tag> >::iterator_category )
						== typeid( std::input_iterator_tag ) );
			}

			SECTION("inner forward")
			{
				REQUIRE( typeid( typename Utf8Codec::DecodingIterator< DummyUtf8TestIterator_<char, std::forward_iterator_tag> >::iterator_category )
						== typeid( std::forward_iterator_tag ) );
			}

			SECTION("inner bidir")
			{
				REQUIRE( typeid( typename Utf8Codec::DecodingIterator< DummyUtf8TestIterator_<char, std::bidirectional_iterator_tag> >::iterator_category )
			...
			SECTION("inner random")
			{
				...
				REQUIRE( typeid( typename Utf8Codec::DecodingIterator< DummyUtf8TestIterator_<char, std::random_access_iterator_tag> >::iterator_category )
						== typeid( std::bidirectional_iterator_tag ) );
			}
		...
		SECTION("encoding")
		{
			SECTION("inner input")
			{
				REQUIRE( typeid( typename Utf8Codec::EncodingIterator< DummyUtf8TestIterator_<char32_t , std::input_iterator_tag> >::iterator_category )
						== typeid( std::input_iterator_tag ) );
			}

			SECTION("inner forward")
			{
				REQUIRE( typeid( typename Utf8Codec::EncodingIterator< DummyUtf8TestIterator_<char32_t, std::forward_iterator_tag> >::iterator_category )
						== typeid( std::forward_iterator_tag ) );
			}

			SECTION("inner bidir")
			{
				REQUIRE( typeid( typename Utf8Codec::EncodingIterator< DummyUtf8TestIterator_<char32_t, std::bidirectional_iterator_tag> >::iterator_category )
						== typeid( std::bidirectional_iterator_tag ) );
			}
			...
			SECTION("inner random")
			{
				// the result should still be bidir, since we do not support 
				// random access for utf-16
				REQUIRE( typeid( typename Utf8Codec::EncodingIterator< DummyUtf8TestIterator_<char32_t, std::random_access_iterator_tag> >::iterator_category )
						== typeid( std::bidirectional_iterator_tag ) );
			}
