...
class TestGtkScrollViewCore
    : public bdn::test::TestGtkViewCoreMixin<bdn::test::TestScrollViewCore>
{
  protected:
    ...
    void verifyScrollBarLayoutSpaces(bool overlayAllowed)
    {
        ...
        bdn::gtk::ScrollViewCore::getScrollBarLayoutSpaces(
            _pGtkScrolledWindow, vertBarSpaceWidth, horzBarSpaceHeight);

        if (overlayAllowed) {
            REQUIRE(vertBarSpaceWidth >= 0);
            REQUIRE(horzBarSpaceHeight >= 0);

            // overlay scrolling should be active for BOTH scrollbars or none of
            // them.
            REQUIRE((vertBarSpaceWidth == 0) == (horzBarSpaceHeight == 0));
        } else {
            // overlay is not allowed, so the scrollbars MUST take up actual
            // layout space
            REQUIRE(vertBarSpaceWidth > 0);
            REQUIRE(horzBarSpaceHeight > 0);
        }

        // if the scrollbars take up space then the scrollbar spacing must be
        // included
        if (vertBarSpaceWidth > 0) {
            GtkWidget *pHBar = gtk_scrolled_window_get_hscrollbar(
                GTK_SCROLLED_WINDOW(_pGtkScrolledWindow));
            GtkWidget *pVBar = gtk_scrolled_window_get_vscrollbar(
                GTK_SCROLLED_WINDOW(_pGtkScrolledWindow));
            ...
            gtk_widget_get_preferred_height(pHBar, &minHorzBarHeight,
                                            &naturalHorzBarHeight);

            gint minVertBarWidth = 0;
            gint naturalVertBarWidth = 0;
            gtk_widget_get_preferred_width(pVBar, &minVertBarWidth,
                                           &naturalVertBarWidth);

            int scrollBarSpacing =
                bdn::gtk::ScrollViewCore::getScrollBarSpacing(
                    _pGtkScrolledWindow);

            REQUIRE(vertBarSpaceWidth ==
                    naturalVertBarWidth + scrollBarSpacing);
    ...
    void runPostInitTests() override
    {
        ...
        SECTION("scrollbar spacing")
        {
            GtkScrolledWindowClass *pScrollClass =
                GTK_SCROLLED_WINDOW_GET_CLASS(_pGtkScrolledWindow);
            ...
            try {
                SECTION("class scrollbar_spacing = 0")
                {
                    ...
                    // It is unclear if that is a GTK bug, but at the time of
                    // this writing that is how GTK behaves, so that is what our
                    // code should return.

                    int spacing = bdn::gtk::ScrollViewCore::getScrollBarSpacing(
                        _pGtkScrolledWindow);
                    REQUIRE(spacing == 0);
                }
        ...
        SECTION("scrollbar layout spaces")
        {
            ...
            {
                gtk_scrolled_window_set_overlay_scrolling(_pGtkScrolledWindow,
                                                          TRUE);

                BDN_CONTINUE_SECTION_WHEN_IDLE(pThis)
                {
                    // overlayScrolling = TRUE means that the scrolled window is
                    // ALLOWED to use overlay scrollbars. The documentation
                    // states that it might do so when no mouse is connected.
            ...
            SECTION("overlayScrolling=FALSE")
            {
                ...
                BDN_CONTINUE_SECTION_WHEN_IDLE(pThis)
                {
                    // overlayScrolling = FALSE means that overlay scrolling is
                    // forbidden. So the scrollbars MUST take up space.
    ...
    void verifyScrollsHorizontally(bool expectedScrolls) override
    {
        ...
        // Instead we check if the size of the scrolled area exceeds the
        // viewport size.
    ...
    Size getScrolledAreaSize()
    {
        ...
        gtk_layout_get_size(GTK_LAYOUT(_pGtkScrolledArea), &scrolledWidth,
                            &scrolledHeight);
