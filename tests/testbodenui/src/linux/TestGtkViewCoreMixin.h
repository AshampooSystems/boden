...
namespace bdn
{
    namespace test
    {
        ...
        /** A mixin class that adds implementations of GTK view specific
           functionality on top of the base class specified in the template
           parameter BaseClass.*/
        template <class BaseClass> class TestGtkViewCoreMixin : public BaseClass
        {
          ...
          protected:
            void initCore() override
            {
                ...
                _pGtkCore =
                    cast<bdn::gtk::ViewCore>(BaseClass::_pView->getViewCore());
            ...
            Size getWidgetSize() const
            {
                ...
                if (GTK_IS_WINDOW(_pGtkWidget))
                    gtk_window_get_size(GTK_WINDOW(_pGtkWidget), &width,
                                        &height);
                else
            ...
            {
                if (GTK_IS_WINDOW(_pGtkWidget))
                    REQUIRE(getWindowPosition() ==
                            BaseClass::_pView->position());
            }
