...
namespace bdn
{
    namespace test
    {
        ...
        /** A mixin class that adds implementations of winuwp view specific
        ...
        class TestWinuwpViewCoreMixin : public BaseClass
        {
          ...
          protected:
            void initCore() override
            {
                ...
                P<bdn::winuwp::IUwpViewCore> pOwner =
                    cast<bdn::winuwp::IUwpViewCore>(_pView->getViewCore());
            ...
            void verifyCoreVisibility() override
            {
                ...
                REQUIRE(_pWinFrameworkElement->Visibility ==
                        (expectedVisible
                             ? Windows::UI::Xaml::Visibility::Visible
                             : Windows::UI::Xaml::Visibility::Collapsed));
            }

            virtual UiMargin getExpectedDefaultPadding() { return UiMargin(); }
            ...
            void verifyIsExpectedWinPadding(::Windows::UI::Xaml::Thickness pad)
            {
                ...
                Margin expectedPixelPadding =
                    _pView->uiMarginToDipMargin(expectedPadding);

                REQUIRE_ALMOST_EQUAL(pad.Top, expectedPixelPadding.top, 0.0001);
                REQUIRE_ALMOST_EQUAL(pad.Right, expectedPixelPadding.right,
                                     0.0001);
                REQUIRE_ALMOST_EQUAL(pad.Bottom, expectedPixelPadding.bottom,
                                     0.0001);
                REQUIRE_ALMOST_EQUAL(pad.Left, expectedPixelPadding.left,
                                     0.0001);
            }
            ...
            void verifyInitialDummyCoreSize() override
            {
                ...
                // Instead they must be updated by a layout cycle after
                // construction. So at this point in time the bounds should be
                // zero
            ...
            {
                // the position of the framework element is not reflected by any
                // readable frameworkelement properties. The Arrange
                // implementation of the container simply sets it to the value
                // from the View::position() property, but this cannot be
                // verified later.
            ...
            {
                // ActualWidth and ActualHeight are apparently not updated until
                // the element is visible. So we cannot use them to verify the
                // size. So, all in all, we cannot do any size verification at
                // all.
                /*
