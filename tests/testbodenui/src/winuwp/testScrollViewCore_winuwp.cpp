...
class TestWinuwpScrollViewCore
    : public bdn::test::TestWinuwpViewCoreMixin<bdn::test::TestScrollViewCore>
{
  protected:
    void initCore() override
    {
        ...
        _pWinScrollViewer = (::Windows::UI::Xaml::Controls::ScrollViewer ^)
            _pWinFrameworkElement;
        _pWinContentWrapper =
            (::Windows::UI::Xaml::Controls::Panel ^) _pWinScrollViewer->Content;
    }

    double getVertBarWidth() { return 0; }

    double getHorzBarHeight() { return 0; }
    ...
    {
        // the view will scroll when the scrolled area view is bigger than the
        // scroll view.
        double scrolledAreaWidth = _pWinContentWrapper->ActualWidth;
        double viewWidth = _pWinScrollViewer->ActualWidth;

        bool enabled = (_pWinScrollViewer->HorizontalScrollMode !=
    ...
    void verifyScrollsVertically(bool expectedScrolls) override
    {
        ...
        bool enabled = (_pWinScrollViewer->VerticalScrollMode !=
                        ::Windows::UI::Xaml::Controls::ScrollMode::Disabled);
                                 ...
                                 double maxDeviation = 0) override
    {
        ...
        if (pContentView != nullptr) {
            ...
            P<bdn::winuwp::IUwpViewCore> pContentViewCore =
                cast<bdn::winuwp::IUwpViewCore>(pContentView->getViewCore());

            ::Windows::UI::Xaml::FrameworkElement ^ pCoreEl =
                pContentViewCore->getFrameworkElement();

            // we cannot verify the position of the framework element because
            // UWP does not expose it. So we only verify the size
    ...
    void verifyScrolledAreaSize(const Size &expectedSize) override
    {
        ...
        // If scrolling is disabled then the wrapper will have the size dictated
        // by the content view. That might exceed the bounds of the scroll view
        // if the content cannot be made to fit. In that case the "scrolled
        // area" is the size of the viewport, not the size of the content
        // wrapper.

        if (_pWinScrollViewer->HorizontalScrollMode ==
                ::Windows::UI::Xaml::Controls::ScrollMode::Disabled &&
            width > _pWinScrollViewer->ViewportWidth)
            width = _pWinScrollViewer->ViewportWidth;

        if (_pWinScrollViewer->VerticalScrollMode ==
                ::Windows::UI::Xaml::Controls::ScrollMode::Disabled &&
    ...
    {
        // the padding is not represented in the Xaml controls. We implement it
        // manually. So there is nothing to test here.
    }
