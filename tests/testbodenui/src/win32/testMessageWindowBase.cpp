...
{
  public:
    TestMessageWindow() : MessageWindowBase("testMessageWindow") {}
  ...
  protected:
    virtual void handleMessage(MessageContext &context, HWND windowHandle,
...
void verifyMessageWindow(bool setResult, bool callDefault)
{
    ...
    LRESULT result =
        SendMessage(window.getHwnd(), WM_SETTEXT, 0, (LPARAM)newText);
    ...
    buf[0] = 0;
    buf[9] = 0;
    SendMessage(window.getHwnd(), WM_GETTEXT, 9, (LPARAM)buf);
    ...
    if (callDefault) {
        ...
        // default implementation should have been called and the text should
        // have been changed
        REQUIRE(text != "xyz");
    }
}
