...
void continueTestPostCall(P<TestCallable> pCallable,
                          P<GlobalMessageWindow> pWindow);
                          ...
                          P<GlobalMessageWindow> pWindow)
{
    if (pCallable->callCount == 0) {
        CONTINUE_SECTION_WHEN_IDLE_WITH([pCallable, pWindow]() {
            continueTestPostCall(pCallable, pWindow);
        });
    }
}
...
void testCallOnceWhenIdle(P<GlobalMessageWindow> pWindow)
{
    ...
    {
        callFromMainThread([pCallable, pCallable2, pWindow]() {
...
TEST_CASE("win32.GlobalMessageWindow")
{
    ...
    SECTION("postCall")
    {
        ...
        SECTION("otherThread")
        {
            CONTINUE_SECTION_IN_THREAD(pWindow) { testPostCall(pWindow); };
        }
    }
