...
namespace bdn
{
    ...
    /** Manages zur preferred size caching of views.
        ...
        The manager stores preferred sizes that a view has returned for a given
       amount of available space (see View::calcPreferredSize() ).

        It also implements some optimizations. It can sometimes infer the
       preferred size for an amount of available space, even when no size has
       been stored for that specific amount of space.
    ...
    */
    class PreferredViewSizeManager : public Base
    {
      public:
        ...
        /** Stores a preferred size value for a given amount of available
         * space.*/
        ...
        /** Gets the preferred size for the specified amount of available space.
            ...
            If a value is found in the cache then preferredSize is set to that
           value and true is returned.

            Otherwise preferredSize is set to Size::none() and false is
           returned.
            */
        bool get(const Size &availableSpace, Size &preferredSize) const
        {
            // If we have a preferred size for infinite available space and
            // if the specified available space is bigger than the infinite
      ...
      private:
        struct Key
        {
            ...
            bool operator<(const Key &o) const
            {
                ...
                // Note that we cannot use Size::operator< because that will
                // only return true if both width and height are smaller than
                // other size. So the "not smaller and not bigger" set includes
                // a lot of sizes for which one component is smaller and the
                // other bigger.

                // also note that any of these components might be infinity. But
                // infinity compares equal to itself so we do not have to check
