...
namespace bdn
{
    ...
    /** Base class for key->value mapping containers that are based on a
    ...
    class StdMapCollection : public StdCollection<BASE_COLLECTION_TYPE>
    {
      public:
        ...
        /** Adds the specified element to the map. The element is a key value
           pair, i.e. a std::pair<const KEYTYPE, VALUETYPE> object.

            Note that there is also a version of add() that takes the key and
           value as separate parameters.

            If the map already has an entry for the key then the associated
           value is overwritten with the new value.
            */
        void add(const std::pair<const Key, Value> &keyValuePair)
        {
            // note that this implementation might be slightly faster than
            // (*this)[key] = value, since for new elements no initial
        ...
        void add(std::pair<const Key, Value> &&keyValuePair)
        {
            ...
            //    This has the drawback that we actually cannot use move
            //    semantics in the initial insert because we might need the
            //    value again for the manual assign. So with this we always copy
            //    key and value, BUT we prevemt a temporary default-constructed
            //    element to be created.
            // 2) we can use [key] = value. This allows us to use move
            // semantics,
            //    but for new elements a temporary default constructed element
            //    is created
            // 3) we use c++17 insert_or_assign. This is out at the moment
            //    because we only require C++11

            // The move semantics have more general potential to improve
        ...
        {
            // we have to use the [] iterator here to get exactly the semantics
            // we want. In C++17 we could use insert_or_assign instead. But in
            // C++11 we only have the option to do this. And all other insert or
            // emplace operations do not overwrite if the element is already in
            // the collection.
            (*this)[key] = value;
        }
        ...
        /** Adds the elements from the specified [beginIt ... endIt)
            ...
            Each element must be a key value pair, i.e. a std::pair<const
           KEYTYPE, VALUETYPE> object.

            If the map already has an entry for one or more of the new element
           keys then the associated value is overwritten with the new value.

            The beginIt and endIt iterators must not refer to the target map.
        ...
        void addSequence(InputIt beginIt, InputIt endIt)
        {
            ...
            // elements one by one. This should not make much of a difference,
        ...
        /** Adds the elements from the specified initializer list to the
           ...
           keys then the associated value is overwritten with the new value.
            ...
            This version of addSequence can be used to add multiple elements
           with the {...} notation. For example:

            \begin
            Map<int, String> myMap;

            // we now add three elements to the map. Each is a pair of an
           integer and a string. myMap.addSequence( {    {1, "one"} {17,
           "seventeen"} {42, "fortytwo"} }  );
        ...
        /** Constructs a new element and adds it to the set, if it not yet in
           ...
           constructor of the newly constructed element.
            ...
            Note that elements are key-value pairs (std::pair<const Key, Value>
           objects). So the arguments are passed to a std::pair constructor.
           Usually this means that one passes exactly two arguments: one for the
           key and one for the value.

            If the map already has an entry for the key then its value is
           overwritten.
           ...
           any copying or moving.
            ...
            Note that addNew is only more efficient than add() if the key is not
           yet in the map. If the entry is overwritten then addNew() may
           actually perform worse than add().
        ...
        /** In general, this collection operation prepares the collection for a
           ...
           with other collection types only - for map collections this function
           has no effect and does nothing.
        */
        ...
        /** Returns true if the map contains the specified key/value pair.
            ...
            This function does not just check that the key is in the map - it
           also checks the associated value. The parameter is a std::pair<const
           KEYTYPE, VALTYPE> object.

            Note that there is also an overload of contains() that checks only
           if the key is in the map.

            Returns true if the key/value pair is in the map and false
           otherwise.
        */
        ...
        */
        ...
        {
          public:
            KeyMatcher_(const Key &key) : _key(key) {}
        ...
        {
          public:
            ElementMatcher_(const Element &element) : _element(element) {}
            ...
            void operator()(COLL &map, ITERATOR &it)
            {
                ...
                if (it == map.begin()) {
                    ...
                    if (it != map.end()) {
                        // found a matching key. But is the associated value
                        // also equal?
        ...
        template <typename MATCH_FUNC_TYPE> class FuncMatcher_
        {
          public:
            FuncMatcher_(MATCH_FUNC_TYPE matchFunc) : _matchFunc(matchFunc) {}
        ...
        // note that in C++11 there is a theoretical problem here (though not
        // one in practice). The advanceAfterRemoval method of the
        // SequenceFilter Iterators assumes that the order of the elements does
        // not change when an element is removed. In C++11 this is not
        // guaranteed for std::unordered_map. In C++14 it IS guaranteed.
        // Luckily, it seems that all known implementations in C++11 do NOT
        // reorder the elements when one is removed. So in practice this is not
        // a problem. And since C++14 guarantees that the ordering remains the
        // same, this will also not become a problem in the future. Also see
        ...
        /** Searches for all elements for which the specified match function
           returns true.
            ...
            The match function can be any function that takes a Map iterator as
        ...
        /** Searches for the specified key/value pair in the map.
            ...
            This function does not just search for the key. If the key is in the
           map then it also checks if the value associated with the key matches
           the one from the specified element \c toFind.

            Note that there is also an overload of find() that only takes a Key
           object as its parameter. That version only for the key.

            find is quite fast: its complexity is logarithmic to the size of the
           map.
        ...
        */
        ...
        /** If the map contains the specified key/value pair, remove it.
            ...
            This only removes an element from the map if key AND its associated
           value match the specified one. Note that there is also an overload of
           findAndRemove() that only searches for the key.
        ...
        /** If the map contains the specified key, remove the corresponding
           entry.
