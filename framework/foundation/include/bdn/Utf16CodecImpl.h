...
namespace bdn
{
    ...
    template <class INPUT_CATEGORY> struct Utf16CodecImplIteratorCategory_
    {
    ...
    /** Implements the Utf-16 string codec.
        ...
        \tparam ElementType specifies the type of the encoded elements. It
            must be a 16 bit integer type. Usually either char16_t or wchar_t
       (on systems where wchar_t is 16 bit). The default is char16_t.
    */
    template <class ElementType = char16_t> class Utf16CodecImpl
    {
      public:
        ...
        */
            ...
            */
            ...
            DecodingIterator &operator--()
            {
                ...
                if (_chr >= 0xd800 && _chr <= 0xdfff) {
                    if (_chr < 0xdc00) {
                        // a leading surrogate at the end of a sequence =>
                        // invalid
                        _chr = 0xfffd;
                    } else {
                        // a trailing surrogate. Need one more value.
                        if (_sourceIt == _beginSourceIt) {
                            // invalid sequence at the start. Cannot go further
                            // back.
                            _chr = 0xfffd;
                        } else {
                            _sourceIt--;
                            ...
                            if (highVal < 0xd800 || highVal > 0xdbff) {
                                // invalid leading surrogate. Which means that
                                // the first one is also invalid.
                                _chr = 0xfffd;
                                ++_sourceIt;
                            } else {
                                highVal -= 0xd800;
                ...
                if (_chr >= 0xd800 && _chr <= 0xdfff) {
                    if (_chr >= 0xdc00) {
                        ...
                        if (_nextIt == _endSourceIt) {
                            // no more data after the initial surrogate. Invalid
                            // value.
                            _chr = 0xfffd;
                        } else {
                            ...
                            if (lowVal >= 0xdc00 && lowVal <= 0xdfff) {
                                ...
                                // not a valid trailing surrogate. So the
                                // leading surrogate should be treated as an
                                // invalid value.
                                _chr = 0xfffd;
                            }
                        }
        ...
        template <class SourceIterator> class EncodingIterator
        {
          public:
            ...
            {
                if (_offset <= 0) {
                    // _offset==-1 means that the current character has not yet
                    // been encoded. That also means that we are at the
            ...
            /** Returns true if a character ends with the current UTF-16 code
               value. This is true for all UTF-16 code values, except the
               starting value of a surrogate pair.*/
          ...
          protected:
            void encode() const
            {
                ...
                if (chr > 0x10ffff || (chr >= 0xd800 && chr <= 0xdfff)) {
                    // invalid, unencodable or in the surrogate range. Use
