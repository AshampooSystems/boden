...
namespace bdn
{
    ...
    /** Base interface for objects that manages notifications for arbitrary
       ...
       called shortly afterwards.

        IAsyncNotifier implementations call the subscribed functions in an
       asynchronous way. I.e. the calls are posted to the application's main
       even queue and are executed during normal event processing from the main
       thread. This is especially useful for events that come from background
       threads: with asynchronous execution the subscriber does not have to deal
       with multithreading and knows that he is only called from the main
       thread.

        See ISyncNotifier for an alternative that calls subscribed functions in
       an synchronous way (immediately).

        It is possible to pass arguments to postNotification(), which it will in
       turn pass on to each of the functions it calls. The number and types of
       these arguments are defined by the template parameters of the INotifier
       class.
       ...
       notification happens.
        ...
        All IAsyncNotifier implementations must be thread safe. In particular,
       notifications can be triggered from any thread. However, the subscribed
       functions will ONLY be called from the main thread. So subscribers
       ...
       and the design decisions can be found here: \ref notifier_internals.md
        ...
        // The IAsyncNotifier instance manages the notifications of certain
       ...
       example someObj)
        // calls the "postNotification" method of the notifier and passes the
       correct parameters that describe the event.
       ...
       event queue,
        ...
        // note that the notification calls happen delayed, from the main
       thread's event
       ...
       &MyClass::myCallbackMethod);
        ...
        // What happens now when postNotification is called? Some might expect a
       crash here,
       ...
       method
        // from its list. So calling postNotification has no bad effects here
       and will simply
    ...
    template <class... ArgTypes> class IAsyncNotifier : virtual public INotifierBase<ArgTypes...>
    {
      public:
        /** Schedules a call to all subscribed functions with the specified
           arguments. The call does not happen immediately. It is posted to the
           main thread and happens asynchronously.

            The arguments passed to postNotification are copied, even if they
           are passed by reference. This is important: if you pass a reference
           to an object then a copy is made and the subscribed functions will
           get a reference to the copied object.

            If you want to pass a specific object to the subscribed functions
           then you need to pass a pointer to it. The objects referred to by
           pointers are not copied. However, it is the responsibility of the
           caller to postNotification to ensure that the object is still valid
           at the time when the notification calls are actually made from the
           main thread. A good way to do that is to pass a bdn::P object instead
           of a plain pointer.

            Note that the notification calls will use the subscriber set at the
           time when the calls are actually made, not the subscriber set at the
           time when postNotification() was called.
        */
