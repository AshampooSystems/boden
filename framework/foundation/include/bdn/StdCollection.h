...
namespace bdn
{
    ...
    class StdCollection : public BaseCollectionType, public Base
    {
      public:
        ...
        /** The type of the allocator that is used to manage the collection's
         * memory allocations.*/
        typedef typename BaseCollectionType::allocator_type Allocator;

        /** The integer type that is used to represent indexes and collection
           sizes. This is usually an alias to size_t.*/
        ...
        /** Returns an iterator to the "first" element of the collection.
            ...
            An iterator is similar to a pointer. It refers to an element or a
           position in the collection. The iterator can be advanced to the next
           element with the ++ operator. For many collection types the --
           operator can be used to go back to the previous element. The element
           the iterator points to can be accessed with the * operator. If the
           collection elements are objects the then iterator's -> operator can
           be used to directly access method and fields of the element it points
           to.
            ...
            // go back to the first element (not supported by all collections /
           iterator types).
            --it;

            \endcode

            When the iterator is moved to other elements then the ordering of
           the elements depends on the actual underlying collection type.

            Iterators can be compared with each other with the == and !=
           operators. They are "equal" when they point to the same element /
           position in the collection.

            If the collection is empty then calling begin() is still ok. It will
           return the same iterator as end().
        */
        Iterator begin() noexcept { return BaseCollectionType::begin(); }
        ...
        /** Returns an iterator that points to the point just after (!) the last
           element in the collection.
            ...
            The end iterator does point to a valid element. It is intended as a
           marker during iterations to know when to stop.

            For example, to iterate over all elements of the collection one
        ...
        */
        Iterator end() noexcept { return BaseCollectionType::end(); }

        /** Const version of end() - returns an iterator for read-only access to
         * the elements.
         */
        ConstIterator end() const noexcept { return BaseCollectionType::end(); }
        ...
        Size getSize() const noexcept { return BaseCollectionType::size(); }
        ...
        */
        ...
        /** Removes all elements in the iterator range [beginIt ... endIt).
            ...
            endIt indicates to the position just after the last element to
           remove.

            Returns an iterator to element that follows the removed range. If
           the removed range was at the end of the collection then the end()
        ...
        */
        ...
        /** Searches for the first element for which the specified match
        ...
        */
        ...
        /** Searches for the first element for which the specified match
           function returns true, starting from the position indicated by \c
           startPos.
