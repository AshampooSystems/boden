        ...
        */
        ...
        struct TailData
        {
            ...
            /** Points to an external uint64_t array that contains the remaining
                64 bit values in the tail data. The array must have
               tailSizeBytes/8 (rounded down) entries. So depending on the value
               of tailSizeBytes, this array can have between 0 and 3 entries.
                */
            const uint64_t *p64BitValues;

            /** If the remaining tail data after the 64 bit values (see
               p64BitValues) is >= 4 bytes, then this points to an single
               uint32_t value, representing the first 4 bytes of the remaining
               tail data, after the 64 bit values. The pointer is not used if
               the remaining tail data is less than 4 bytes.
                */
            const uint32_t *p32BitValue;

            /** Points to an external uint8_t array that contains the
                remaining bytes of the tail data, after the 64 bit values from
               p64BitValues and the 32 bit value from p32BitValue (if that is
        ...
        /** Calculates the hash, using data provided by the specified
           dataProvider object. This function can be used if not all the data is
           available at the same time, or if the data is not available in the
           form of a single buffer.

            xxHash internally regards the input data as a stream of blocks of
           four uint64_t values (32 bytes). After these blocks there can be an
           optional tail data block, consisting of 0-3 uint64_t values, 0 or 1
           32 bit value and 0-3 individual bytes. So the tail data can have a
           total size between 0 and 31 bytes.

            The data provider can be any object that provides the same functions
           as the following example class:
            ...
            public:
                ...
                // point to an array of 4 uint64_t values. The memory of this
           array must be managed
                // by the data provider and must remain valid until the next
           data provider call.
                // Returns nullptr at the end, when no full 32 byte block is
           available anymore. const uint64_t* next4x8ByteBlock()
                {
                    ...
                }

                // Returns the remaining tail data after all available 32 byte
           blocks have been read
                // and less than 32 bytes of data remain. The hasher will call
           this exactly once, and
            ...
            The \c seed parameter of calcHashWithDataProvider is an optional
           parameter that is usually left at its default value 0 for normal
           hashing. The seed can be used in special case to predictably alter
           the result. If you hash the same data with a different seed then you
        ...
        class SimpleDataProvider
        {
          public:
            ...
            const uint64_t *next4x8ByteBlock()
            {
                ...
                // machine.
                // This might not be necessary if a portable hash is not
                // required. But it is a very quick operation and this should be
                // negligible compared to the other operations we do. So we
                // simply do this always on big endian systems.
            ...
            TailData getTailData()
            {
                ...
                // order of _8ByteValues[3], that means that it is still in
                // little endian order. So the "next" 32 bits will be at the
                // beginning of the 8 byte area.
