    ...
    class StdPositionalCollection : public StdCollection<BaseCollectionType>
    {
      public:
        ...
        /** Returns a reference to the first element of the collection. Throws
           OutOfRangeError if the collection is empty.*/
        Element &getFirst()
        {
            if (this->isEmpty())
                throw OutOfRangeError("StdPositionalCollection::getFirst "
                                      "called on empty collection.");

            return BaseCollectionType::front();
        }

        /** Returns a const reference to the first element of the collection.
           Throws OutOfRangeError if the collection is empty.*/
        ...
        /** Returns a reference to the last element of the collection. Throws
           OutOfRangeError if the collection is empty.*/
        Element &getLast()
        {
            if (this->isEmpty())
                throw OutOfRangeError("StdPositionalCollection::getLast called "
                                      "on empty collection.");

            return BaseCollectionType::back();
        }

        /** Returns a const reference to the last element of the collection.
           Throws OutOfRangeError if the collection is empty.*/
         ...
         */
        void add(const Element &value) { BaseCollectionType::push_back(value); }
        ...
        /** Adds the elements from the specified [beginIt ... endIt)
            ...
            The beginIt and endIt iterators must not refer to the target
           collection. They can be from an arbitrary other collection -- also
           from a collection of a different type. The only condition is that the
        ...
        /** Adds the elements from the specified source \ref sequence.md
           ...
           a compatible element type.
            ...
            // note that the element types do not even have to be equal, only
           compatible.
            // Here the source list uses int, while the destination array uses
           long. List< int > sourceList( { 1, 9, 3} );
        ...
        /** Constructs a new element and adds it to the collection (at the end).
            The arguments passed to addNew are passed on to the constructor of
        ...
        /** Inserts a copy of the specified element \c el at position \c pos.
            ...
            If \c pos points to an element of the collection then the new
           element is inserted before that element.

            If \c pos points to the collection end then the new element is added
           at the end.
        ...
        */
        ...
        /** Inserts \c count copies copy of the specified element \c el at the
           specified position \pos.
            ...
            If \c pos points to an element of the collection then the new
           elements are inserted before that element.

            If \c pos points to the collection end then the new elements are
           added at the end.
        ...
        */
        ...
        /** Inserts copies of the elements from the specified [beginIt ...
           endIt) iterator range at position \c pos. endIt points to the
           location just *after* the last element to add.

            The beginIt and endIt iterators must NOT refer to the target
           collection that is being changed. They can be from an arbitrary other
            collection -- also from a collection of a different type. The only
           condition is that the element type of the source collection must be
           compatible to the element type of the target collection.
        ...
        /** Inserts the elements from the specified initializer list at position
           ...
           added at the end.
            ...
            This can be used to insert multiple elements with the {...}
           notation. For example:

            \begin
            Array<int> ar;

            ar.insertSequenceAt( ar.begin(), {1, 4, 7} );    // inserts three
        ...
        /** Constructs a new element and inserts it to the collection at
           position \c pos. Any arguments passed to insertNew after \c pos are
           passed on to the constructor of the newly constructed element.

            This function can be used instead of inset() when one wants to
           insert a newly created value more efficiently. With insert() the new
           value would initially be constructed as a temporary variable and then
           a copy or move operation would be made to bring it into the
           collection. insertNew constructs the new element directly inside the
           collection, avoiding any copying or moving.
        ...
        */
        ...
        /** Inserts copies of the elements from the specified [beginIt ...
           endIt) iterator range at the beginning of the collection. endIt
           points to the location just *after* the last element to add.
        ...
        /** Inserts the elements from the specified initializer list at the
           ...
           notation. For example:
            ...
            ar.insertSequenceAtBegin( {1, 4, 7} );    // inserts three elements
        ...
        /** Constructs a new element and inserts it at the beginning of the
           collection.

            Same as insertNewAt( begin(), args...).
        */
        template <class... Args> Iterator insertNewAtBegin(Args &&... args)
        {
            return insertNewAt(this->begin(), std::forward<Args>(args)...);
        }

        /** Removes the last element of the collection. Throws OutOfRangeError
           if the collection is empty.*/
        void removeLast()
        {
            if (this->isEmpty())
                throw OutOfRangeError("StdPositionalCollection::removeLast "
                                      "called on empty array.");

            this->pop_back();
        }

        /** Removes the first element from the collection. Throws
           OutOfRangeError if the collection is empty.*/
        void removeFirst()
        {
            if (this->isEmpty())
                throw OutOfRangeError("StdPositionalCollection::removeFirst "
                                      "called on empty array.");
        ...
        /** Changes the size of the collection to \c count number of elements.
            ...
            If the collection is currently bigger then the elements at the end
           are removed.

            If the collection is currently smaller then new default-constructed
           elements are added at the end. If you the new elements to be
           initialized differently then you should use the setSize(Size, const
           Element&) overload of this method instead.
        */
        void setSize(Size count) { BaseCollectionType::resize(count); }
        ...
        */
        ...
        /** Searches for all elements for which the specified match function
           returns true.
            ...
            The match function can be any function that takes a collection
           iterator as its parameter and returns true if the element at the
           corresponding position should be in the find results.
        ...
        */
        ...
        /** Searches backwards from the specified start position for
            the last element that compares equal to the specified \c el
           parameter,
        ...
        */
        ...
        /** Searches for the last element for which the specified match function
           returns true.
