...
namespace bdn
{
    ...
    /** Implements an array of element the type indicated by the first template
        ...
        The Array class is derived from std::vector and is 100 % compatible with
       it. It can be used as a drop-in replacement.

        Array provides renamed aliases for some std::vector methods and types to
       ...
       library.
        ...
        Note that Array is also derived from bdn::Base, so it can be used with
       smart pointers (see bdn::P).

        Additionally, some utility and convenience functions have been added to
       Array that std::vector does not have to make commonly used functionality
      ...
      public:
        ...
        */
        ...
        Array(const Array &other)
            : StdPositionalCollection<std::vector<ELTYPE, ALLOCATOR>>(
                  static_cast<const std::vector<ELTYPE, ALLOCATOR> &>(other))
        {}
        ...
        */
        ...
        /** Replaces the current contents of the array with copies of the
           elements from the specified other Array.
        ...
        /** Moves the data from the specified other Array object to this array,
           replacing any current contents in the process. The other Array object
        ...
        /** Returns a reference to the element at the specified zero-based
           index.

            The reference can be used to modify the element in-place.
        */
        Element &atIndex(Size index) { return this->at(index); }

        /** Const version of atIndex() -- returns a const reference to the
         * element at the specified zero-based index.
         */
        const Element &atIndex(Size index) const { return this->at(index); }

        /** [] operator that returns a reference to the element at the specified
           zero-based index.
            ...
            ar[1] = 23;     // the 42 element will be overwritten with the
           number 23

            \endcode
        */
        Element &operator[](Size index) { return this->at(index); }

        /** Const version of operator[] -- returns a const reference to the
         * element at the specified zero-based index.
         */
        ...
        /** Prepares the array for a bigger insert operation. This is purely for
           optimization purposes and can be used to prevent intermediate
           re-allocations when a lot of elements are added.

            The function will ensure that the internal buffer can hold at least
           the specified total number of elements. This number of elements
           includes any current elements that the array may have.

            Note that the number of elements of the array is not modified by
           this function - it only optimizes internal memory management.
        */
        void prepareForSize(Size size) { this->reserve(size); }
        ...
        /** Returns an iterator to the element at the specified zero based
           index.
            ...
            If the index equals the size of the array then the end() iterator is
           returned.
        */
        Iterator indexToIterator(Size index) { return this->begin() + index; }
        ...
        /** Sorts the elements in ascending order (small first), using the
           ...
           myComparisonFunc. myArray.sort( myComparisonFunc );
            }
        ...
        /** Sorts the elements in a custom order. comesBefore must be a function
           ...
           also two predefined comparison functions that can be used: pass
