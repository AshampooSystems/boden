...
namespace bdn
{
    ...
    class XxHash32
    {
      public:
        ...
        */
        static uint32_t calcHash(const void *data, size_t bytes, uint32_t seed = 0)
        {
        ...
        struct TailData
        {
            ...
            /** Must point to an external uint32_t array that contains the
               remaining 32 bit values in the tail data. The array must have
               tailSizeBytes/4 (rounded down) entries. So depending on the value
            ...
            /** Points to an external uint8_t array that contains the
                remaining bytes of the tail data, after the 32 bit values from
               p32BitValues. The array must have (\c tailSizeBytes % 4) entries
               - so it is between 0 and 3 bytes.
                */
            const uint8_t *bytes;
        };
        ...
        /** Calculates the hash, using data provided by the specified
           ...
           four uint32_t values (16 bytes). After these blocks there can be an
           optional tail data block, consisting of 0-3 uint32_t values and 0-3
           individual bytes. So the tail data can have a total size between 0
           and 15 bytes.
           ...
           as the following example class:
            ...
            public:
                ...
                // point to an array of 4 uint32_t values. The memory of this
           array must be managed
                // by the data provider and must remain valid until the next
           data provider call.
                // Returns nullptr at the end, when no full 16 byte block is
           available anymore. const uint32_t* next4x4ByteBlock()
                {
                    ...
                }

                // Returns the remaining tail data after all available 16 byte
           blocks have been read
                // and less than 16 bytes of data remain. The hasher will call
        ...
        */
        ...
        static uint32_t calcHashWithDataProvider(DataProvider &dataProvider, uint32_t seed = 0)
        {
            ...
            const uint32_t *tail32BitValuesEnd = tailData.p32BitValues + (tailData.tailSizeBytes >> 2);
            while (tailData.p32BitValues != tail32BitValuesEnd) {
        ...
        class SimpleDataProvider
        {
          public:
            ...
            const uint32_t *next4x4ByteBlock()
            {
                ...
                std::memcpy(_4ByteValues, _nextData, 16);
            ...
            {
                memcpy(_4ByteValues, _nextData, _bytesLeft);
                ...
                return TailData{_bytesLeft, _4ByteValues, _nextData + _bytesLeft - (_bytesLeft & 3)};
            }
