...
namespace bdn
{
    ...
    /** Implements a double linked list with elements of the type indicated by
       the first template parameter. For example List<String> is an list of
       Strings.

        The List class is derived from std::list and is 100 % compatible with
       it. It can be used as a drop-in replacement.

        List provides renamed aliases for some std::list methods and types to
       ...
       library.
        ...
        Note that List is also derived from bdn::Base, so it can be used with
       smart pointers (see bdn::P).

        Additionally, some utility and convenience functions have been added to
       List that std::list does not have to make commonly used functionality
       more convenient (like sorting, etc).

        Like the C++ standard library collections, one can specify an
       "allocator" type as an optional second template argument. This is used
      ...
      public:
        ...
        */
        ...
        List(const List &other)
            : StdPositionalCollection<std::list<ELTYPE, ALLOCATOR>>(
                  static_cast<const std::list<ELTYPE, ALLOCATOR> &>(other))
        {}
        ...
        /** Initializes the list with the specified initializer list. This is
           called by the compiler when {...} initialization is used (see example
           below).
        ...
        */
        ...
        /** Replaces the current contents of the list with copies of the
           elements from the specified other List.
        ...
        /** Replaces the current contents of the list with copies of the
           elements from the specified std::list object.
        ...
        /** Replaces the current contents of the list with copies of the
           elements from the specified initializer list. This is called by the
           compiler if a  "= {...} " statement is used. For example:
        ...
        /** Moves the data from the specified other List object to this list,
           replacing any current contents in the process. The other List object
        ...
        /** Sorts the elements in ascending order (small first), using the
           ...
           If you need descending order or a custom order, there is also a
           variant of sort that takes a comparison function as a parameter.
           ...
           void doSort()
           {
               ...
               // sort in a custom order, defined by the custom
           myComparisonFunc. myList.sort( myComparisonFunc );
           }
           ...
           sort is *not* guaranteed to be implemented with a stable sorting
           algorithm. Note that "stability" is a special term when discussing
           sorting algorithms. It refers to how elements that are "equal" to
           each other (neither one of them is smaller than the other) are
           handled. Stable sorting algorithms guarantee that equal elements keep
           their original relative order amongst each other. Non-stable sorting
           algorithms do not guarantee that.
        ...
        /** Sorts the elements in a custom order. comesBefore must be a function
           ...
           true if the first one should come before the second one. There are
           two predefined comparison functions that can be used: pass
           bdn::ascending<ElementTypeName> to get ascending order or
        ...
        */
        ...
        /** In general, this collection operation prepares the collection for a
           ...
           with other collection types only - for bdn::List this function has no
        ...
        */
        ...
        /** Merges two sorted lists.
            Both lists *must* be sorted in ascending order (as defined by the
           list element's < operator) before this is called. If this is not the
           case then undefined behaviour, including crashes, might occur.

            All elements from the other list are transferred to this list and
           inserted in the correct places so that the result is a properly
           sorted list again.

            The elements from the \c other list are moved, not copied.
            The \c other list will be empty after the function returns.

            This function does *not* invalidate any iterators. Iterators from
           the \c other list remain valid and refer to the merged list
           afterwards.
            */
        ...
        /** Merges two sorted lists.
            Both lists *must* be sorted in ascending order (as defined by the
           list element's < operator) before this is called. If this is not the
           case then undefined behaviour, including crashes, might occur. All
           elements from the other list are transferred to this list and inserts
           them in the correct places so that the result is a properly sorted
           list again.
        ...
        /** Merges two sorted lists.
            Both lists *must* be sorted according to the order indicated by the
           \c comesBefore parameter.  If this is not the case then undefined
           behaviour, including crashes, might occur.

            comesBefore must be a function that takes references to two elements
           as parameters and returns true if the first one should come before
           the second one. See sort() for more information.
        ...
        /** Transfers all elements from \c otherList to this list and inserts
           them at the position indicated by \c insertPosition.
        ...
        /** Removes consecutive duplicates from the list.
           ...
           all duplicates are next to each other).
            ...
            This version of the function considers to elements to be duplicates
           if their
        ...
        /** Removes consecutive duplicates from the list.
           ...
           all duplicates are next to each other).
            ...
            The \c isDuplicate parameter must be a function that takes two
        ...
        /** Removes all elements for which the specified function matchFunc
           returns true.
            ...
            matchFunc must be a function that takes a reference to a List
