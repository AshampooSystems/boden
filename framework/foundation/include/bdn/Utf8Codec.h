...
namespace bdn
{
    ...
    template <class INPUT_CATEGORY> struct Utf8CodecImplIteratorCategory_
    {
        using Type = std::input_iterator_tag;
    };

    template <> struct Utf8CodecImplIteratorCategory_<std::forward_iterator_tag>
    {
    ...
    class Utf8Codec
    {
      public:
         ...
         * for a character.*/
        constexpr static int getMaxEncodedElementsPerCharacter() { return 6; }

        /** Decodes a single unicode character from the specified UTf-8 data.

            If the amount of data is empty ( it == endIt ) then the function
           returns 0xfffd and does not modify the position of the \c it
           parameter.

            If the character was successfully decoded then the decoded character
           is returned and the \c it parameter is modified to point to the byte
           following the decoded Utf-8 byte sequence.

            If the data is not a valid encoded utf-8 character sequence then the
           function returns the special unicode value 0xfffd and moves the \c it
           parameter a single byte ahead.

            \param it reference to a byte iterator (returning uint8_t elements)
           pointing to the start of the encoded UTF-8 character. This iterator
           will be modified by the function - see above for an explanation of
           its position after the function completes.

            \param endIt an iterator pointing to the byte after the last one of
           the encoded data. This is NOT necessarily the end of the single
           character - it is the end of the total encoded data. This is
           specified for safety to ensure that the decode function will not
           overshoot the valid data in case of incorrectly encoded data.
        ...
        */
        template <typename IteratorType> static char32_t decodeChar(IteratorType &it, const IteratorType &endIt)
        {
            ...
            if ((firstByte & 0x80) == 0)
                ...
                // safe this iterator position in case the encoded sequence is
                // invalid.
                ...
                while (true) {
                    ...
                    uint8_t val = *it;
                    if ((val & 0xc0) != 0x80) {
                        // invalid sequence. We should abort right away, because
                        // the current value could be part of a valid sequence
        ...
        /** A character iterator that decodes UTF-8 data (char elements) from an
            ...
            The decoder handles invalid / corrupted sequences gracefully. When a
           bad sequence is encountered then each bad byte(!) is replaced by the
           Unicode "replacement character" (which has the value 0xfffd).
        */
        template <class SourceIterator> class DecodingIterator
        {
          public:
            ...
            // generated on the fly that are not actually stored by the
            // underlying container. While we could return a reference to a
            // member of the iterator, that would only remain valid while the
            // iterator is alive. And parts of the standard library (for example
            // std::reverse_iterator) will create temporary local iterators and
            // return their value references, which would cause a crash. By
            // defining reference as a value, we ensure that the standard
            // library functions return valid objects. Note that defining
            // reference as a value is allowed for input iterators, but not for
            // forward or bidirectional iterators, according to the standard.
            // However, input iterators are single pass only, so if we were to
            // define our category as input iterator then a lot of algorithms
            // would be less efficient. For example, in string construction the
            // memory would be reallocated several times because the string
            // constructor thinks that it can only go over the data once. So, to
            // avoid that, we define our category as forward or bidir (depending
            // on the source iterator category) and break standard compliance by
            // defining reference as a value. In practice this is usually not a
            // problem.
            using reference = char32_t;

            /** @param sourceIt the source iterator that provides the UTF-8
               data.
               ...
               overshooting the data boundaries if the UTF-8 data is corrupted.
            ...
            DecodingIterator() { _chr = (char32_t)-1; }
            ...
            DecodingIterator &operator--()
            {
                ...
                if (invalid) {
                    ...
                    // over were consumed. When we decode the "next character"
                    // forwards from here, the next bytes until the end of the
                    // invalid sequence will be decoded as a single replacement
                    // character each. Since we want decoding forwards and
                    // backwards to yield exactly the same result we only go
                    // back a single byte.
                    _nextIt = startIt;
                    _sourceIt = startIt;
                    --_sourceIt;
            ...
            const SourceIterator &getInner() const { return _sourceIt; }
          ...
          public:
            ...
            EncodingIterator &operator--()
            {
                if (_offset == -1 || _encoded[_offset - 1] == 0xff) {
                    ...
                    // of the iterator, i.e. the current position is the first
                    // byte of the first character.. _encoded[_offset-1]==0xff
                    // means that the current byte is the first byte of the
                    // encoded sequence.

                    _sourceIt--;
                    encode();

                    // move to the last byte of the sequence. Note that that is
                    // always index 6 (the first byte offset changes depending
                    // on the sequence size - the end index stays the same).
                    _offset = 6;
                } else
                    _offset--;
            ...
            /** Returns true if the iterator points to the last byte of a
             * encoded UTF-8 character byte sequence.
             */
            ...
            bool operator==(const EncodingIterator &o) const
            {
                ...
                if (_offset == -1) {
                    // We know that o._offset is not -1 (otherwise we would have
                    // already returned true). See if o is at the first byte.
