...
namespace detail
{
    namespace
    {
...
#if defined(DT_BLK)
        template <class DirEntT, class = decltype(DirEntT::d_type)> static file_type get_file_type(DirEntT *ent, int)
        {
            switch (ent->d_type) {
            case DT_BLK:
        ...
        template <class DirEntT> static file_type get_file_type(DirEntT *ent, long) { return file_type::none; }

        static pair<string_view, file_type> posix_readdir(DIR *dir_stream, error_code &ec)
        {
...
class __dir_stream
{
  public:
    ...
    __dir_stream(__dir_stream &&__ds) noexcept
        : __stream_(__ds.__stream_), __root_(move(__ds.__root_)), __entry_(move(__ds.__entry_))
    {
    ...
    bool advance(error_code &ec)
    {
        while (::FindNextFile(__stream_, &__data_)) {
            ...
            __entry_.__assign_iter_entry(__root_ / __data_.cFileName,
...
class __dir_stream
{
  public:
    ...
    __dir_stream(__dir_stream &&other) noexcept
        : __stream_(other.__stream_), __root_(move(other.__root_)), __entry_(move(other.__entry_))
    {
    ...
    __dir_stream(const path &root, directory_options opts, error_code &ec) : __stream_(nullptr), __root_(root)
    {
        if ((__stream_ = ::opendir(root.c_str())) == nullptr) {
            ...
            const bool allow_eacess = bool(opts & directory_options::skip_permission_denied);
    ...
    bool advance(error_code &ec)
    {
        while (true) {
            ...
            if (str == "." || str == "..") {
                ...
                __entry_.__assign_iter_entry(__root_ / str,
