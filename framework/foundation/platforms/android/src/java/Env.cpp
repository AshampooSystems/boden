...
namespace bdn
{
    namespace java
    {
            ...
            if (_pEnvIfKnown == nullptr) {
                if (_pGlobalVm == nullptr) {
                    throw std::runtime_error(
                        "bdn::java::Env used from unattached thread, but "
                ...
                int getResult =
                    _pGlobalVm->GetEnv((void **)&pEnv, JNI_VERSION_1_6);
                if (getResult == JNI_EDETACHED) {
                    // thread is not yet attached to the VM. Attach it.
                    int attachResult =
                        _pGlobalVm->AttachCurrentThread(&pEnv, nullptr);
                    if (attachResult != JNI_OK) {
                        throw std::runtime_error(
                            "Fatal error: unable to attach JNI to current "
                    ...
                    throw std::runtime_error(
                        "Fatal error: unable to get JNI env for current thread "
        ...
        void Env::throwAndClearExceptionFromLastJavaCall()
        {
            ...
            pEnv->ExceptionClear();
            if (exc != nullptr)
                JavaException::rethrowThrowable(JThrowable(
                    Reference::convertAndDestroyOwnedLocal((jobject)exc)));
        }
        ...
        void Env::setJavaSideException(const std::exception_ptr &exceptionPtr)
        {
            ...
            try {
                ...
                // exception.
                pEnv->Throw(
                    (jthrowable)e.getJThrowable_().getRef_().getJObject());
            }
            catch (...) {
