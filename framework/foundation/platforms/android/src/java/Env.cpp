...
namespace bdn::java
{
    ...
    void Env::throwAndClearExceptionFromLastJavaCall()
    {
        ...
        env->ExceptionClear();
        if (exc != nullptr)
            JavaException::rethrowThrowable(wrapper::Throwable(Reference::convertAndDestroyOwnedLocal((jobject)exc)));
    }
    ...
    void Env::setJavaSideException(const std::exception_ptr &exceptionPtr)
    {
        ...
        try {
            ...
            // throw that.
            bdn::java::wrapper::NativeException nativeException(exceptionPtr);
