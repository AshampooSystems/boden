...
/** Encodes string data to the multibyte encoding used in the given locale.
	...
	it provides begin() and end() methods that return iterators. The iterators
	return the encoded char elements.

	The input data must be full 32 bit unicode characters. It is also provided 
	via iterators. The template parameter of LocaleEncoder must be the type of
	the iterators that provide the source characters to encode.

	If a character cannot be represented in the locale's multibyte encoding
	then the encoder will try to encode the unicode replacement character instead
	(U'fffd'). If that character also cannot be encoded then the '?' character is
	encoded instead.
...
{
public:
	/** \param sourceBeginIt an iterator that points to the first character to encode
		\param sourceEndIt an iterator that points to the position just after the last character to encode
		\param loc the locale whose multibyte encoding should be used to
			encode the data.*/
	LocaleEncoder(
		SOURCE_ITERATOR_TYPE&& sourceBeginIt,
		...
		// note that we store a copy of the locale to ensure that it is not
		// deleted while we decode.
		, _locale( loc )
	{
    ...
    // This typedef is set when the standard library codec implementations are so
    // broken that they are unusable and the platform only supports UTF-8 locales.
    
    using Iterator = Utf8Codec::EncodingIterator<SOURCE_ITERATOR_TYPE>;
    
    Iterator begin() const
    {
        return Iterator( _sourceBeginIt );
    }
    
    Iterator end() const
    {
        return Iterator( _sourceEndIt );
    }
	...
	{
	public:
		// If we follow the standard precisely then we would need to declare this as
		// an input_iterator instead of a forward_iterator. The reason is that
		// we return values, not references, which only an input iterator may do.
		// However, input iterators are also single pass only, so their use is very
		// restrictive. String algorithms frequently want to do multiple passes over the data
		// (for example, one to determine the length and one to do the actual work).
		// We do support multipass, so we declare ourselves as a forward iterators, so that
		// this capability will actually be used. In practice it almost never causes a problem
		// if a forward iterator returns values instead of references.
    ...
    public:
		...
		Iterator& operator++()
		{
			if(_pEncodeState==nullptr)
			{
				// this happens when we are at the start position and the first
				// character has not been accessed.
				// fill the output buffer first.
				fillOutBuffer();
			}
			...
			if(_pEncodeState!=nullptr)
			{
				...
				if( _pEncodeState->outPos >= _pEncodeState->outAvailable	)
				{
					...
					// if there is no more data to be decoded then this was an invalid call.
					// (advancing past the end of the data).
				}
			}
		...
		bool operator==(const Iterator& o) const
		{
			...
			{
				// we must ensure that iterators that are on their last character
				// do not match iterators that have reached the end.
				// For both of these types, outPos may be 0 and wideNext points
				// to the end of the sequence.
				// The key is to compare the number of unreturned bytes in the
				// output buffer.
				int left = (_pEncodeState==nullptr) ? 0 : (_pEncodeState->outAvailable - _pEncodeState->outPos);
				int otherLeft = (o._pEncodeState==nullptr) ? 0 : (o._pEncodeState->outAvailable - o._pEncodeState->outPos);
	...
	private:
            ...
            OutType*& pOutNext)
		{
            ...
            // some buggy codec implementations will return stop when they encounter a '\0'
            // character and return ok.
            // But they will not consume the zero character and will not write it to output.
            ...
            {
                // some codec implementations (incorrectly) return "partial" when they
                // encounter an error.
                // If zero input and output was consumed then we can be certain that this is an "error partial"
                // case.
                // But if there were successfully converted characters before that
                // then we cannot easily distinguish the error case from the normal partial result (end of buffer reached).
                // But that does not matter: we can treat these cases like a normal partial result initially and
                // then, in the next iteration, we will get a partial result again, this time with zero input
                // and output consumed. So we will diagnose the error then.
                if(pInNext==pInBegin
                    && pOutNext==pOutBegin
                    && pInBegin!=pInEnd
                    && pOutBegin!=pOutEnd)
                {
                    // this also sometimes happens when the character is a zero character '\0'
		...
		/** Fills the output buffer. Returns false when that is not possible because
			the end of the input data has been reached.*/
			...
			if( _wideNext!=_wideEnd )
			{
				...
				do
				{
					...
					if(convResult==std::codecvt_base::error )
                    {
						...
						// the first character, even if it is not the problem.
			
						// Luckily, this is not a problem in this case, since we only convert a single character at the time.
						// So we know that the first and only character is the problem.
					...
					// note: outAvailable may still be 0 here if there was an encoding
					// error and none of the replacement characters could be encoded either.
					// In that case we try again with the next character.
