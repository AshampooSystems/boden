...
/** A recursive Mutex. Only one thread at a time can hold a lock on the mutex object.
	...
	Each lock() call must be paired with exactly one unlock() call. It is recommended to use #Mutex::Lock
...
public:
	...
	/** Represents a lock on a Mutex object.
		...
		When the Lock object is constructed it locks the mutex. When the Lock
	...
	/** Unlocks a mutex when the object is constructed and re-locks it when the object is destructed.
		...
		void myFunction()
		{
			...
			// mutex is now automatically locked again here, since the Unlock object was destroyed.
