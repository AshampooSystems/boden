...
/** Character traits type for 32 bit unicode character streams.
	...
	This type currently behaves identical to std::char_traits<char32_t>, but it is a different type.

	The existence of this type has fairly technical reasons that most users of the Boden framework
	do not need to be concerned about. See the documentation of bdn::TextOutStream.
...
/** Writes a character sequence to an output stream, just like a normal string would be written.
	Alignment and padding are properly handled according to the stream formatting options,
	in the same way as they would be if a normal string object was written.

	The character sequence is specified as an iterator interval: the beginIt iterator points
	to the first character of the sequence and endIt points to the position just after the last
	character in the sequence. The iterators must return characters of the same type that the stream
	uses (CHAR_TYPE).
*/
template<typename CHAR_TYPE, class CHAR_TRAITS, typename IT_TYPE>
std::basic_ostream<CHAR_TYPE, CHAR_TRAITS>& streamPutCharSequence(
	std::basic_ostream<CHAR_TYPE, CHAR_TRAITS>& stream, 
	const IT_TYPE& beginIt,
	const IT_TYPE& endIt )
{
...
// we have to provide a specialization for basic_ios::widen and narrow because the default
// implementation uses std::ctype<char32_t> in some implementations - and that
// does not always exist
template<>
inline char32_t basic_ios<char32_t, bdn::UnicodeCharTraits>::widen(char c) const
{
    return (char32_t)use_facet< ctype<wchar_t> >( getloc() ).widen(c);
}
    
template<>
inline char basic_ios<char32_t, bdn::UnicodeCharTraits>::narrow(char32_t c, char defChar) const
{
...
public:
	...
	explicit basic_ostream( std::basic_streambuf<char_type, traits_type>* pBuffer )
		: BasicIos(pBuffer)
	{
		// by default we use a variant of the classic locale, with the multibyte encoding
		// replaced with UTF-8.
		imbue( defaultLocale() );
...
private:
	...
	basic_ostream& _doSentriedNumPut(VALUE value, ios_base& fmt, char32_t fillChar )
	{	
		sentry s(*this);
		if(s)
		{
			std::locale loc = getloc();
			auto it = std::use_facet< std::num_put<wchar_t> >( loc )
					.put(
...
public:
	...
	basic_ostream& operator<<( std::basic_streambuf<char_type, traits_type>* pBuffer)
	{
		...
		if(s)
		{
			...
			else
			{
				try
				{
					// if put fails then we MUST NOT extract the input character.
					// So we read first without advancing and then advance afterwards.

					int_type chr = pBuffer->sgetc();
					if( traits_type::eq_int_type(chr, traits_type::eof() ) )
					{
						// if no characters are inserted (if the input buffer is empty)
						// then we must set failbit
	...
	basic_ostream& operator<<( std::basic_ios<char_type, traits_type>& (*func)(std::basic_ios<char_type,traits_type>&) )
	{
		func(*this);
		return *this;
	}

	basic_ostream& operator<<( std::basic_ostream<char_type,traits_type>& (*func)(std::basic_ostream<char_type,traits_type>&) )
	{
		func(*this);
		return *this;
	}

	// the following are nonstandard operator overloads that the normal std::basic_ostream does not provide.
	// We provide this because in contrast with the normal basic_ostream we want to properly
	// decode char strings according to the locale encoding. So we do not want the global
	// generic template implementation of this to be used.

	basic_ostream& operator<<( const char* s )
	{
		if( usingUtf8Encoding() )
		{
			// use our own UTF-8 decoding routines rather than relying on those
			// from the locale (the standard library decoding can be quite buggy with some
			// standard libraries - especially when incorrectly encoded data is encountered).
			// So, to get consistent behaviour across all platforms and
			// compilers we decode ourselves
...
private:
	...
	public:
		WideAdapterStreamBuffer( basic_ostream* pStream)
			: _pStream(pStream)
		{
			setp( _wideChars, &_wideChars[capacity] );
		}

		void setStream(basic_ostream* pStream)
		{
			_pStream = pStream;
		}
	...
	protected:
		std::basic_streambuf<wchar_t>::int_type overflow(std::basic_streambuf<wchar_t>::int_type chr) override
		{
			...
			if( WideTraits::eq_int_type( chr, WideTraits::eof() ) )
			{
				...
				if(pBuffer!=nullptr)
				{
					// we can ignore the return value here, since we will ALWAYS return eof
...
/** An output stream for unicode text (32 bit unicode characters - char32_t).

	This is an alias for std::basic_ostream<char32_t, UnicodeCharTraits>,
	which is a specialized version of basic_ostream.
	
	TextOutStream is used in the same way as other standard library I/O streams (like std::cout, std::stringstream, etc.),
	since it is a customized version of the standard library class std::basic_ostream.
	To write values and objects you simply push them into the stream with the << operator. See examples below.

	The helper class StreamFormat can be used to control the formatting of the next written value.

	Locale independence
	-------------------

	By default, the formatting of TextOutStream does NOT depend on the global locale. Unless it is explicitly changed,
	the stream uses a locale with english formatting rules and UTF-8 encoding for char strings.
	The default locale is actually a variant of the "classic" locale (see std::locale::classic() ), in which the normal ASCII
	encoding has been replaced with UTF-8.

	Bool formatting
	---------------

	When a bool value is written to the stream then it is printed as a word by default.
	It prints "true" or "false" (or the locale's translation of these, if a custom locale was set
	with imbue()). This is in contrast to the default for other std::basic_ostream implementations,
	which print bools as numbers (0 or 1) by default.

	Note that you can change this setting and switch to printing 0 and 1 instead by doing this:
	\code
	...
	TextOutStream supports strings of all standard character types: char, wchar_t, char16_t and char32_t.
	In addition to the Boden String class (see bdn::String), both zero terminated C-style strings (e.g. const char*)
	and std::basic_string objects (like std::string, std::wstring) are supported.

	All strings that use the "char" type are interpreted according to the encoding
	of the stream's locale (which is UTF-8 by default).


	Adding support for custom object types
	-----------------------------

	To enable objects of your custom types to be used with TextOutStream, you need to provide a 
	...
	// Note that this overload not only adds support for pushing ShoppingItem objects into
	// TextOutStream, but also for pushing them into any other standard I/O stream (like std::cout).

	template< typename CHAR, class TRAITS >
	std::basic_ostream<CHAR,TRAITS>& operator<<( std::basic_ostream<CHAR,TRAITS>& stream, const ShoppingItem& item)
	{
		// generate a string representation of the object. This is an example
	...
	If the << operator is not overloaded for a given object type then "myStream << myObject" will cause a compiler error.
	However, sometimes is is useful to be able to "stringify" any object. For such cases the global function
	bdn::writeAnyToStream() exists. It can be used to write ANY object of any type to a stream like TextOutStream.
	If there is not << operator for the object type then this function writes a default string and does not cause
	a compiler error.
	...
	See \ref text_streams.md for information on the technical implementation of these streams.
	Most users of the Boden framework do not need to worry about this, though.
...
// rvalue references.
// This overload ensures that the << operator also works with ad hoc created TextOutStream objects.
// Note that this is only necessary with some compilers / standard libraries
// because their conversion operator for this is sometimes buggy or not generic enough.
// For example, for MSVC the standard library conversion operator
// is only there for std::ostream, not for basic_ostream object of other char or traits types.
...
template<>
struct StreamWriterImpl_<false>
{
	template<typename STREAM_TYPE, typename VALUE_TYPE>
	static void write(STREAM_TYPE&& stream, VALUE_TYPE&& value)
	{
...
/** Writes a string representation of any value or object (whatever its type) to the specified
	standard I/O stream.
	The stream can be any stream derived from or compatible with std::basic_ostream. This includes
	the Boden classes TextOutStream and StringBuffer.

	If a << operator overload exists for the specified value type and stream
	(i.e. if "stream << value" is supported) then the << operator will be used to write the value.

	If there is no << operator overload for the given value then a generic text consisting of
	the value type and its address is written instead.
