...
/** Implements a filter on top of a provided \ref sequence.md "sequence" of elements, using a provided filtering function.

	It is usually more convenient to use the filterSequence() utility function instead
	of manually creating the SequenceFilter object. When using filterSequence the compiler can automatically
	deduce the necessary template parameters from the function arguments.
		
    The SequenceFilter object works on top of the provided base \ref sequence.md "sequence", which can be of any type.
    The SequenceFilter then uses the provided filtering function to filter out (skip) some of the elements
    of the underlying base sequence. The result is a new sequence - i.e. SequenceFilter is itself a \ref sequence.md "sequence"
    object.

    SequenceFilter is a template with the following template parameters:
    
    - BaseSequenceType: the type of the base sequence. For example, if an integer Array is filtered then this would be bdn::Array<int> .
...
public:
    using BaseSequence = BaseSequenceType;
		...
		// BUT unfortunately in VS2015 there is a compiler bug. While the type alias works in
		// general, it does not produce a type that can be used to define a conversion operator
		// (VS produces this error message: 'operator decltype' is not a recognized operator or type).
		// But if we let the compiler jump through a few hoops then this works. We define
		// the BaseIterator as the std::result_of of a function that returns the iterator.
		// This works on VS2015.
