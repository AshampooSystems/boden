...
/** Interface for objects that manages notifications for arbitrary "events".
	...
	ISyncNotifier implementations call the subscribed functions in a synchronous way. I.e. the
	calls happen immediately when the event happens, from the same thread that triggered the event.

	See IAsyncNotifier for an alternative that calls subscribed functions in an asynchronous way
	...
	// The ISyncNotifier instance manages the notifications of certain events. These are usually
    ...
    // calls the "notify" method of the notifier and passes the correct parameters that describe the event.
	...
	// Since the notifier is synchronous, the subscribed functions have all been called at 
	// this point.
    ...
    // What happens now when notify is called? Some might expect a crash here,
