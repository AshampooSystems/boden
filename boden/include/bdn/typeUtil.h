...
/** Internal helper for typeSupportsShiftLeftWith().
    ...
    of the left and right operand of the statement (x and y in this example).
 
    If the << statement is valid then TypeSupportsShiftLeftWith_< XType, YType >::value
...
private:
    ...
    // That returns an rvalue reference.  We want this template to work with streams and there
    // is a generic conversion operator for rvalue streams that takes any parameter and simply calls the lvalue
    ...
    // decide that std::declval<TYPE>() << XYZ is valid for all XYZ - which is not
    // what we want.
    // So we use *((TYPE*)nullptr) instead, which is an lvalue reference.
    
    template<typename TEST_ARG_TYPE>
    static uint8_t _test( int dummy, decltype( &( (*(LEFT_TYPE*)nullptr) << std::declval<TEST_ARG_TYPE>() ) ) pDummy=nullptr );
...
/** Returns true if the statement "x << y" is valid for two objects x and y of
    the types TYPE and ARG_TYPE (specified via template parameters).
 
    typeSupportsShiftLeftWith is a constexpr function, so it can also be used in
    ...
    // the next statement also returns true, since there is a builtin shift operator
    // for integer types (doing a bitwise shift).
    typeSupportsShiftLeftWith< int, int >();
 
    // the next statement returns false, since the std::vector class does not
...
struct TypeHasCustomArrowOperator_
{
private:
    // note that we do not use std::declval here, since that returns an
    // rvalue reference. Since we are only allowed to call const functions on rvalue
    // references with some compilers, we would not be able to detect non-const operator-> overloads
...
/** Returns true if the type indicated by the template argument
    ...
    typeHasCustomArrowOperator is a constexpr function, so it can also be used in
    compile time statements.
 */
template<class TYPE>
constexpr bool typeHasCustomArrowOperator()
{
