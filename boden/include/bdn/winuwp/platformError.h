...
/** Creates a SystemError / std::system_error object for the system error with the
	...
	*/
SystemError platformExceptionToSystemError(::Platform::Exception^ e, const ErrorFields& infoFields = ErrorFields() );


/** Creates a Platform::Exception object from the specified standard exception.
    The function tries to return an exception object with a more specific sub-type of Platform::Exception,
    depending on the error condition (similar to Platform::Exception::CreateException).*/
...
/** \def BDN_WINUWP_TO_STDEXC_BEGIN
    ...
    that should throw std::exception object instead of Platform::Exception objects.
    When a Platform::Exception is thrown inside the code block then it is converted
    to a corresponding std::exception.
...
/** \def BDN_WINUWP_TO_STDEXC_END
    ...
    */
#define BDN_WINUWP_TO_STDEXC_END }catch(::Platform::Exception^ e){ throw bdn::winuwp::platformExceptionToSystemError(e); }


/** \def BDN_WINUWP_TO_PLATFORMEXC_BEGIN

    Used together with #BDN_WINUWP_TO_PLATFORMEXC_END to encapsulate a block of code
    that should throw Platform::Exception objects instead of std::exception objects.
    When a std::exception is thrown inside the code block then it is converted
    to a corresponding Platform::Exception.
    ...
    ... any std::exception thrown inside here will be converted to a Platform::Exception.
...
#define BDN_WINUWP_TO_PLATFORMEXC_END }catch(std::exception& e){ throw bdn::winuwp::exceptionToPlatformException(e); }    
