...
/** Represents the field of a Java-side object or class and enables access to it.
 *
 ...
 *
 *  The template parameter Kind specifies what type of field this is. It can be either
 *  ObjectFieldKind for an instance field or StaticFieldKind for a static field.
 *
 ...
 *
 * */
template<class NativeType, class Kind>
class Field
{
public:
    /** \param contextRef reference to the java entity that owns the field.
     *      What kind of entity this is depends on the Kind template parameter.
     *      For StaticFieldKind it is a reference to the class object. For ObjectFieldKind
    ...
    Field(const Field& o)
    : _contextRef( o._contextRef )
    , _id( o._id )
    {
    }

    Field(Field&& o)
    : _contextRef( std::move(o._contextRef) )
    , _id( std::move(o._id) )
    {
    }
    ...
    {
        FieldAccessor<Kind, NativeType>::setValue( (typename Kind::ContextJType)_contextRef.getJObject(), _id.getId(), newValue );
    }

    NativeType get() const
    {
        return FieldAccessor<Kind, NativeType>::getValue( (typename Kind::ContextJType)_contextRef.getJObject(), _id.getId() );
    }
