 ...
 *
 *  The java objects are wrappers for a C++ exception object. They are used when an exception
 *  needs to be passed to the java side.
 *
 * */
...
public:
    ...
    *      */
    ...
    /** Returns the ExceptionReference object that refers to the original C++ exception.*/
    P<ExceptionReference> getExceptionReference_()
    {
        static MethodId methodId;

        return cast<ExceptionReference>( invoke_<JNativeStrongPointer>(getStaticClass_(), methodId, "getNativeExceptionData").getPointer_() );
    }
