 ...
 *
 *  Creating a new object of this type is mainly useful for these use cases:
 *
...
public:
    ...
    *      */
    explicit JByteBuffer(const Reference& ref)
     : JObject(ref)
    {
    }


    /** Creates a new byte buffer on the java side, which references the specified
     *  memory block.
     *
     *  In some cases the intention of creating the Java buffer is simply to store a memory address
     *  on the Java side. If only the memory address is relevant to your use case and the memory
     *  is not accessed on the Java side then then you can set the capacity to a dummy value >0 (e.g. 1).
     *
     *  */
