...
/** Provides an efficient way to construct strings from individual
	parts. StringBuffer is derived from \ref TextOutStream (which is a std::basic_ostream implementation).
	So StringBuffer can be used in the same way as C++ standard library output streams (like
	std::cout).
	...
	- Formatting is locale independent by default, using the formatting rules
	  from the classic locale (english formatting).
	- Strings based on any character type (char, wchar_t, char16_t, char32_t) can be
	  written to the buffer. The string data is properly decoded.
	- Multibyte strings (std::string, const char*) are interpreted as UTF-8 by default.
	  You can change that by setting a different locale with a different multibyte encoding
	  (see imbue() ).
	- When parts of string arguments cannot be decoded because of corrupted data then the corresponding
	  parts of the string are silently replaced with the unicode replacement character (U+fffd).
	  So StringBuffer can safely be used with any kind of input data.


	Usage
	-----
	  
	Like and standard C++ stream, you can write values and objects using the << operator.
	It is recommended to use \ref StreamFormat objects to configure the formatting of the individual values
	(although the normal C++ stream formatting works as well).
	...
	buffer << "The number is " << 42 << ". The size is " << Size(123, 456) << "." << std::endl;
	buffer << "Zero padding looks like this: " << StreamFormat.zeroPad(4) << 17;
...
private:
	...
	public:
		...
		void swap(StreamBuffer& other)
		{
            ...
            // the end iterator of a list may get invalidated when it is swapped.
            // So we need to manually transfer those.
		...
		size_t totalChars() const
		{
			...
			{
				// the data in the last chunk is not included in _totalCharsInFinishedChunks
		...
		void advanceWritePosition(size_t chars)
		{
			pbump( (int)chars );
		}
	...
	public:
		...
		public:
			...
			using value_type = char32_t;	// not CHAR32_TYPE because we never want this to be const
			...
			// accept other iterator types so that const and non-const iterators
		...
		Iterator begin()
		{
			...
			// will return the same iterator as end(), which is what we want.
	...
	protected:
		...
		int_type underflow() override
		{
			...
			if(pChunk->used() <= chunkAlreadyRead)
			{
				...
				if(_readChunkIt==_chunkList.end())
				{
					// no more chunks. Set the iterator back to the last chunk,
					// since more data may be added to that later.
					--_readChunkIt;
					return traits_type::eof();
				}
				...
				// note that we never have empty chunks. So we know that there is data in
				// the new chunk.
				pChunk = &*_readChunkIt;
				chunkAlreadyRead = 0;
			}
	...
	private:
		...
		bool _moveToPosition(
			List<Chunk>::Iterator& chunkIt,
			size_t& posInChunk,
			std::streamoff& chunkStartPos,
			off_type offset,
	...
	protected:
		...
		pos_type seekoff( off_type off, std::ios_base::seekdir anchor, std::ios_base::openmode which = ios_base::in | ios_base::out ) override
		{
			syncChunkUsed();
			...
			{
				// if both positions are modified at the same time then cur cannot be used,
				// since the return value is not defined.
				// We fail here, just like the normal std::basic_stringbuf.
		...
		void syncChunkUsed() const
		{
			if(_writeChunkIt != _chunkList.end() )
			{
				...
				if(posInChunk > chunk.used())
				{
					...
					{
						// update the end of the read buffer, since the amount
						// of valid data in this chunk has changed.
...
public:
		...
		: TextOutStream( std::move(rhs) )
	{
		...
		// the move constructor does not touch the buffer pointer. So we must
		// manually remove the source object's buffer, since it may have been invalidated
		// by the move
	...
	/** Returns the contents of the StringBuffer as a normal String object.
		...
		references that are often the result of << operations on a StringBuffer
		can be implicitly converted to Strings as well.

		So instead of calling toString explicitly, you can also simply assign the StringBuffer
		object or the result of a << chain directly to a String object.
		*/
...
/** Returns a string representation of the specified value or object.
	...
	that have an overloaded << operator for \ref StringBuffer, \ref TextOutStream or \ref std::basic_ostream.
	See \ref TextOutStream for more inforation on how to add support for your own custom types.

	When you call toString with an object or a value of an unsupported type then you will get a
	compiler error. There is also an alternative function that
	will never give you a compiler error and that can be called with any type of value or object.
	See anyToString() for more information.
*/
template<typename VALUE_TYPE>
inline String toString(VALUE_TYPE&& value)
{
	StringBuffer buf;
...
/** Returns a string representation of the specified value or object (all types are supported).

	anyToString() is similar to the global function toString(), except that
	it works with values and objects of any type.

	Like toString(), anyToString internally uses \ref StringBuffer to generate the
	string representation of the value or object. If the specified value has a
	<< operator for \ref StringBuffer, \ref TextOutStream or \ref std::basic_ostream then
	anyToString will use that to generate the string representation. See \ref TextOutStream
	for more inforation on how to add a string generator for your own type.

	When there is no usable << operator for the specified value type then calling anyToString
	does *not* cause a compiler error (in contrast to toString). Instead, anyToString will generate
	a string representation for such values automatically. The generated string representation contains
	the type name of the value, as returned by typeid(value).name(), as well as the value's memory address.

	*/
template<typename VALUE_TYPE>
inline String anyToString(VALUE_TYPE&& value)
{	
	StringBuffer buf;
