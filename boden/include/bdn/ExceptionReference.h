...
public:
    ...
    /** Returns an ExceptionReference object that refers to the currently
        ...
        If the creating the reference object involves allocating memory internally
        and there is not enough memory for that then the returned reference
        will refer to a std::bad_alloc exception.
        */
    ...
    /** Similar to fromActiveException, except that the reference object
        is allocated with newObj and a pointer to the reference object is returned.
     */
    ...
    operator bool() const
    {
        return static_cast<bool>( _exceptionPtr );
    }
...
private:
    ...
    {
        // note that we do not use safe static here, because safe static involves
        // allocating with new. And we need the global bad alloc reference when
        // there is not enough memory, so new should be avoided.
