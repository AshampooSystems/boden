...
public:
    ...
    {
        gtk_button_set_label( GTK_BUTTON( getGtkWidget() ), label.asUtf8Ptr() );
    }    

    void setState(const TriState& state) override
    {
        gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( getGtkWidget() ), state == TriState::on );
        gtk_toggle_button_set_inconsistent( GTK_TOGGLE_BUTTON( getGtkWidget() ), state == TriState::mixed );
    }
    ...
    void generateClick()
    {
        ...
        if(pView!=nullptr)
        {
            gtk_toggle_button_set_inconsistent(GTK_TOGGLE_BUTTON(getGtkWidget()), false);
    ...
    TriState _gtkToggleButtonStateToCheckboxState()
    {
        ...
        if ( gtk_toggle_button_get_inconsistent(GTK_TOGGLE_BUTTON(getGtkWidget())) )
        {
            newCheckboxState = TriState::mixed;
        }
        else if ( gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(getGtkWidget())) )
        {
