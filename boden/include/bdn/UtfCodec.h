...
/** A generic helper template that selects the corresponding utf codec
    ...
    - UtfCodec<wchar_t> is WideCodec (which in turn is either Utf16Codec or Utf32Codec, depending
      on the size of wchar_t on the system).
...
template<>
class UtfCodec<char> : public Utf8Codec
{
};

template<>
class UtfCodec<wchar_t> : public WideCodec
{
};

template<>
class UtfCodec<char16_t> : public Utf16Codec
{
};

template<>
class UtfCodec<char32_t> : public Utf32Codec
{
};
