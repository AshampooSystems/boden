...
#define BDN_WIN32SWITCH_HANDLE_OFFSET_ON    BDN_WIN32SWITCH_WIDTH - BDN_WIN32SWITCH_HANDLE_MARGIN - BDN_WIN32SWITCH_HANDLE_WIDTH
...
public:
	SwitchCore(T* pOuter)
		: ViewCore(pOuter, 
				   SwitchCore::SwitchCoreClass::get().getName(),
				   pOuter->label(),
				   WS_CHILD,
				   0)
	{
	...
	{
	    SendMessage(getHwnd(), BM_SETCHECK, on ? BST_CHECKED : BST_UNCHECKED, 0);
	    InvalidateRect(getHwnd(), NULL, TRUE);
	}

	// Convenience function to retrieve the current "checked" state of the win32 window
	bool _on() const
	{
	...
	void generateClick()
	{
	    ...
	    if(pView!=nullptr)
	    {
	        ...
	        // Click is triggered also when the user uses the keyboard to toggle the checkbox,
	        // so we set the outer's on state here to inform the framework about a state change.
	...
	{
		Size switchMinSize(BDN_WIN32SWITCH_WIDTH, BDN_WIN32SWITCH_HEIGHT);
		double switchMargin = 10.; // hardcoded value that looks good

	    String label;

	    // we completely ignore the available space as there is no way to reduce the size for the switch.
	    ...
	    // SwitchCore height is at least the height of the switch control. If the label's height
	    // is greater than the switch control's height, then the core's height is the label's height.
	    // In the latter case the switch control will be centered vertically.
	    ...
	    if(pad.isNull())
	    {
	        // Use a default padding that looks good – win32 does not have a built-in default
...
protected:
	void handleMessage(MessageContext& context, HWND windowHandle, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_GETTEXT:
			{
				...
				context.setResult(wcsnlen_s((WCHAR*)lParam, wParam), false);
				break;
			}
		...
		case BM_GETCHECK:
			context.setResult(pOuter != nullptr && pOuter->on() ? BST_CHECKED : BST_UNCHECKED, false);
			break;
		...
		case WM_LBUTTONUP:
			{
				...
				if (PtInRect(&switchRect, pos)) {
					SendMessage(GetParent(windowHandle), WM_COMMAND, 0, (LPARAM)windowHandle);
				}
	...
	RECT computeSwitchRect(RECT* clientRect = nullptr)
	{
		...
		return {
			clientRect->right - int(BDN_WIN32SWITCH_WIDTH * scaleFactor),
			clientRect->top + clientHeight / 2 - BDN_WIN32SWITCH_HEIGHT / 2,
			clientRect->right,
			clientRect->top + clientHeight / 2 + BDN_WIN32SWITCH_HEIGHT / 2
		};
	}
	...
	void paint()
	{
	    ...
	    HBRUSH handleBackgroundBrush = CreateSolidBrush(handleBackgroundColor);
	    HBRUSH backgroundBrush = CreateSolidBrush(pOuter != nullptr && pOuter->on() ? backgroundColorOn : backgroundColorOff);
	    HPEN handleOutlinePen = CreatePen(PS_SOLID, 1, handleOutlineColor);
		...
		Rectangle(hdc,
		      	  switchRect.left + int(handleOffset() * scaleFactor),
		          switchRect.top + int(BDN_WIN32SWITCH_HANDLE_MARGIN * scaleFactor),
		          switchRect.left + int((handleOffset() + BDN_WIN32SWITCH_HANDLE_WIDTH) * scaleFactor),
	  	          switchRect.top + int((BDN_WIN32SWITCH_HANDLE_MARGIN + BDN_WIN32SWITCH_HANDLE_HEIGHT) * scaleFactor));
		...
		if (pOuter != nullptr) {
		    P<Font> pFont = UiProvider::get().getFontForView(pOuter.getPtr(), scaleFactor);
			hFont = pFont->getHandle();
			if (hOldFont = (HFONT)SelectObject(hdc, hFont))
			{
			    SetTextColor(hdc, RGB(0,0,0));
			    SetBkMode(hdc, TRANSPARENT);
			    DrawText(hdc, pOuter->label().asWidePtr(), -1, &rect, DT_SINGLELINE | DT_LEFT | DT_VCENTER);
	...
	{
		P<const T> pOuter = cast<const T>( getOuterViewIfStillAttached() );		
		return pOuter != nullptr && pOuter->on() ? BDN_WIN32SWITCH_HANDLE_OFFSET_ON : BDN_WIN32SWITCH_HANDLE_OFFSET_OFF;
	}	
