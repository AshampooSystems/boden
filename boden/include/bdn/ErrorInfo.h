...
/** Contains information about an error / an exception.
	...
	ErrorInfo is a helper class that is used to encode structured additional information
	("fields") in error messages and extract it again later.

	Encode the fields as a string (possibly to add them to an error message):
...
public:
	...
	/** Constructs an ErrorInfo object from an exception object. This is currently the same as
		ErrorInfo( error.what() ), but in the future additional information might be extracted
	...
	/** Returns the value of a field. If the field is not defined then an empty string
		is returned.*/
	...
	/** Returns the additional information fields as a name-value string map in the form
		of an ErrorFields object.*/
	const ErrorFields&	getFields() const
	{
		return _fields;
	}

	
	/** Returns the error message without the encoded additional information.*/
