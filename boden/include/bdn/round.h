...
{
    /** Round to the nearest number. Halfway values are rounded away from zero.*/
    nearest,

    /** Round up, to the next bigger number. Negative values are rounded toward zero.*/
    up,
...
/** User internally - not part of the public API.
    ...
    cases where a number needs to be represented with a value that is slightly bigger or
...
/** Performs a stable, scaled rounding operation to the next highest boundary. Negative
    numbers are rounded toward zero.

    In contrast to normal integer rounding, scaled rounding can be used to round to a
    number grid with a non-integer step size. The scaleFactor parameter indicates the
    scale factor from the rounding grid to integer numbers. So a scale factor of 2 would mean
    that there are 2 rounding boundaries per integer, i.e. the rounding grid is in 0.5 increments
    (0, 0.5, 1, 1.5, etc.). The function will round to the next higher rounding boundary.
    
    The term "stable" means that when the result (!) of the rounding operation is passed to the same
    rounding function again then you get the exact same result. This might seem to be obvious, but when
    a scale factor is involved then the limitations of floating point numbers can mean that the exact
    mathematical result of the rounding operation cannot be represented exactly. In these cases
    the floating point system will return an imperceptibly smaller or larger value.
    If a slightly larger value is returned then passing it to a native up-rounding operation would ordinarily bump
    it to the NEXT boundary. This does not happen with a "stable" implementation like this one. The slight inexactness
    is detected and the rounding result is the same boundary.
*/
...
/** Performs a stable, scaled rounding operation, down to the next smaller scaled rounding boundary. Negative
    numbers are rounded away from zero.

    This function works analogously to stableScaledRoundUp(), except that it rounds down, not up. See
    stableScaledRoundUp() for more information.*/
...
/** Performs a stable, scaled rounding operation. The rounding type is indicated by the roundType parameter - it
    controls whether the function behaves like stableScaledRoundUp(), stableScaledRoundDown() or stableScaledRoundNearest.
