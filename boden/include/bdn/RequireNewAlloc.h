...
/** Wraps a class and adds special code that enforces that instances of the class
	...
	The ActualType template parameter is used to generate a better error message
	if the object is not allocated correctly. It should be the type of the
	class that derives from RequireNewAlloc.
...
public:
	RequireNewAlloc()
	{
		...
		if(!allocatedWithNew)
		{
			// note that we cannot use typeid(*this).name() to get the type of the object.
			// Since construction of the class deriving from us has not finished yet, it is likely
			// that typeid(*this) will simply return the type of RequireNewAlloc<BaseType>.
