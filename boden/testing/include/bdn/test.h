 ...
 *  Original version from Catch Copyright (c) 2012 Two Blue Cubes Ltd. All
 rights reserved.
...
// BDN_CONFIG_CPP11_GENERATED_METHODS : The delete and default keywords for
// compiler generated methods BDN_CONFIG_CPP11_IS_ENUM : std::is_enum is
// supported? BDN_CONFIG_CPP11_TUPLE : std::tuple is supported
// BDN_CONFIG_CPP11_LONG_LONG : is long long supported?
// BDN_CONFIG_CPP11_OVERRIDE : is override supported?
// BDN_CONFIG_CPP11_UNIQUE_PTR : is unique_ptr supported (otherwise use
// auto_ptr)
...
namespace bdn
{
    ...
    // This is just here to avoid compiler warnings with macro constants and
    // boolean literals
...
// nullptr_t support based on pull request #154 from Konstantin Baumann
        ...
        // So the compare overloads can be operator agnostic we convey the
        // operator as a template enum, which is used to specialise an Evaluator
        ...
        bool compare(std::nullptr_t lhs, T *rhs)
        {
            T *p(nullptr); // this temporary variable is needed so that the code
                           ...
                           // (ARC). nullptr cannot be implicity converted to a
                           // reference counted pointer, so passing it to
                           // evaluate(T*,T*) directly would cause an error.
                           // With this temp object we explicitly create a null
                           // pointer variable so this will work.
...
#define INTERNAL_BDN_ALMOST_EQUAL(value, expectedValue, maxDeviation,          \
    ...
    do {                                                                       \
        bdn::ResultBuilder __catchResult(                                      \
        ...
        try {                                                                  \
            ...
            bool _result =                                                     \
                (_val >= _expectedVal - _dev && _val <= _expectedVal + _dev);  \
...
namespace bdn
{
    ...
    template <typename T> struct IGenerator
    {
        ...
        // It will cause a compilation error with G++. Instead we add an
        // implementation that only throws an exception.
      ...
      public:
        ...
        operator T() const
        {
            ...
            return T(); // Suppress spurious "not all control paths return a
                        // value" warning in Visual Studio - if you know how to
...
/** Verifies that the specified expression throws a ProgrammingError.
    ...
    This is very similar to REQUIRE_THROWS_AS( expr, bdn::ProgrammingError). But
   in addition to that functionality, this macro automatically uses
   ExpectProgrammingError to prevent debug breaks, asserts and logging to happen
...
/** \def #define ASYNC_SECTION( sectionName, captures... )
    ...
    section code. The difference is that this code will be executed
   asynchronously, as if CONTINUE_SECTION_WHEN_IDLE was used at the end of a
   normal section.

    There is one difference in usage compared to SECTION, though: there must be
   a semicolon at the end of the async section's code block (see example below).

    The code block after ASYNC_SECTION will actually end up being executed as a
   lambda function. The capture statement of the lambda expression are the
   remaining parameters of the ASYNC_SECTION macro, after the section name.
   Often one will simply specify = here, to automatically capture the local
   variables that you use inside the continuation.

    This macro is primarily useful if you want pending user interface events to
   be executed between the initialization code for your test case and the actual
   test code. For example, you might set up a user interface and might want a
   layout cycle to happen between the initialization and the tests.
    ...
    ASYNC_SECTION("some section", = )   // capture statement is "=", i.e.
...
/** \def CONTINUE_SECTION_WHEN_IDLE_WITH( continuationFunc )
    ...
    Continues the current test section after all pending events and work have
   finished. Then the section resumes and the specified continuation function /
   lambda is called.

    This is very similar to CONTINUE_SECTION_WHEN_IDLE, except that you can pass
   the code to be executed as a function object parameter, instead of specifying
   it in a code block after the CONTINUE_SECTION_WHEN_IDLE statement.

    This can sometimes be useful if you want the continuation to be a real
   function, or a function with bounds parameters (using std::bind) or things
   like that.

    Apart from this difference, the macro works exactly like
...
/** \def CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS_WITH(seconds, continuationFunc
    ...
    Continues the current test section after waiting for the specified number of
   seconds, according to the normal clock. Note that this macro will always wait
   the requested amount of time, no matter what happens on the computer. Also
   see \ref CONTINUE_SECTION_AFTER_RUN_SECONDS_WITH, which will automatically
   wait longer when the CPU is under high load or when the test application gets
   suspended by the operating system.

    Seconds is a floating point number, so you can also specify non-integer
   amounts of seconds (e.g. 0.1 to wait for 100 milliseconds).

    After the delay the section is continued by calling the specified
   continuation function / lambda.

    This is very similar to CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS, except that
   you can pass the code to be executed as a function object parameter, instead
   of specifying it in a code block after the
   CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS statement.
...
/** \def CONTINUE_SECTION_AFTER_RUN_SECONDS_WITH(seconds, continuationFunc )
    ...
    Continues the current test section after waiting for the specified number of
   seconds. If the computer is under high load, or if the test application is
   suspended then the wait time is automatically increased. Note that this
   increase is just a rough heuristic - it is not guaranteed that the process
   will get the exact specified amount of processing time before the wait
   finishes.

    Also see \ref CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS_WITH, which waits
   exactly the specified time, without increasing it due to high load.
   ...
   continuation function / lambda.
    ...
    This is very similar to CONTINUE_SECTION_AFTER_RUN_SECONDS, except that you
   can pass the code to be executed as a function object parameter, instead of
   specifying it in a code block after the CONTINUE_SECTION_AFTER_RUN_SECONDS
   statement.
...
/** \def CONTINUE_SECTION_IN_THREAD_WITH( continuationFunc )
    ...
    Similar to CONTINUE_SECTION_WHEN_IDLE_WITH, except that the continuation
   function is executed immediately from a newly created secondary thread. This
   is useful if you want to test your code from another thread (for example, to
   verify that it also works from arbitrary threads, not just the main thread).
...
/** \def CONTINUE_SECTION_WHEN_IDLE( captures... )
    ...
    Continues the current test section after all pending UI events and scheduled
   work has finished and the event queue is empty.

    Caveat: On some platforms it may not be possible to detect a true "idle"
   state for the app. In that case the implementation is a "best effort" one
   that tries to make it likely that all pending work has finished before the
   test continues - see asyncCallFromMainThreadWhenIdle() for more information.

    A code block with the code for the continuation must follow (see example
   below). There must be a semicolon after the code block.

    CONTINUE_SECTION_WHEN_IDLE is mainly useful if you need pending events to be
   processed before the test continues. It is often used in tests that use user
   interface elements.

    The continuation code block that follows is always called from the main
   thread. There is also a similar macro called CONTINUE_SECTION_IN_THREAD which
   runs the continuation function in a new thread instead of the main thread.

    CONTINUE_SECTION_WHEN_IDLE can also be used directly in TEST_CASE blocks,
   not just in SECTION blocks.

    Your test section (or test case if you do not use sections) should end after
   the CONTINUE_SECTION_WHEN_IDLE statement and its code block. If there is
   additional test code afterwards then it will be executed BEFORE the
   continuation code is run. Since that is unintuitive, it should be avoided.

    Continuations can also be chained. I.e. continuation code block can also
   have a CONTINUE_SECTION_WHEN_IDLE statement at the end (inside the
   continuation code block) to add another asynchronous continuation.

    The normal test macros (like REQUIRE() ) can all be used as normal in the
   continuation. There is no difference to a synchronous test in this regard.

    The code block after CONTINUE_SECTION_WHEN_IDLE will actually end up being
   executed as a lambda function. The capture statement of the lambda expression
   are the (optional) parameters of the CONTINUE_SECTION_WHEN_IDLE macro. Often
   one will simply specify = here to capture the local variables by value.
...
/** \def CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS(seconds, captures... )
   ...
   seconds, according to the normal clock. Note that this macro will always wait
   the requested amount of time, no matter what happens on the computer. Also
   see \ref CONTINUE_SECTION_AFTER_RUN_SECONDS, which will automatically wait
   longer when the CPU is under high load or when the test application gets
   suspended by the operating system.

    Seconds is a floating point number, so you can also specify non-integer
   amounts of seconds (e.g. 0.1 to wait for 100 milliseconds).

    A code block with the code for the asynchronous continuation must follow
   (see example below). There must be a semicolon after the code block.

    CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS is mainly useful if you need to wait
   a while during the test and you want pending events to be processed while you
   wait. It is often used in tests that use user interface elements.

    The continuation code block that follows is always called from the main
   thread.

    CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS can also be used directly in
   TEST_CASE blocks, not just in SECTION blocks.

    Your test section (or test case if you do not use sections) should end after
   the CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS statement and its code block. If
   there is additional test code afterwards then it will be executed BEFORE the
   continuation code is run. Since that is unintuitive, it should be avoided.

    Continuations can also be chained. I.e. continuation code block can also
   have a CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS statement at the end (inside
   the continuation code block) to add another asynchronous continuation.

    The normal test macros (like REQUIRE() ) can all be used as normal in the
   continuation. There is no difference to a synchronous test in this regard.

    The code block after CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS will actually
   end up being executed as a lambda function. The capture statement of the
   lambda expression are the (optional) parameters of the
    CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS macro. Often one will simply specify
   = here to capture the local variables by value.
...
/** \def CONTINUE_SECTION_AFTER_RUN_SECONDS(seconds, captures... )
   ...
   finishes.
    ...
    Also see \ref CONTINUE_SECTION_AFTER_ABSOLUTE_SECONDS, which waits exactly
   the specified time, without increasing it due to high load.
   ...
   (see example below). There must be a semicolon after the code block.
    ...
    CONTINUE_SECTION_AFTER_RUN_SECONDS is mainly useful if you need to wait a
   while during the test and you want pending events to be processed while you
   wait. It is often used in tests that use user interface elements.

    The continuation code block that follows is always called from the main
   thread.

    CONTINUE_SECTION_AFTER_RUN_SECONDS can also be used directly in TEST_CASE
   blocks, not just in SECTION blocks.

    Your test section (or test case if you do not use sections) should end after
   the CONTINUE_SECTION_AFTER_RUN_SECONDS statement and its code block. If there
   is additional test code afterwards then it will be executed BEFORE the
   continuation code is run. Since that is unintuitive, it should be avoided.

    Continuations can also be chained. I.e. continuation code block can also
   have a CONTINUE_SECTION_AFTER_RUN_SECONDS statement at the end (inside the
   continuation code block) to add another asynchronous continuation.

    The normal test macros (like REQUIRE() ) can all be used as normal in the
   continuation. There is no difference to a synchronous test in this regard.

    The code block after CONTINUE_SECTION_AFTER_RUN_SECONDS will actually end up
   being executed as a lambda function. The capture statement of the lambda
   expression are the (optional) parameters of the
    CONTINUE_SECTION_AFTER_RUN_SECONDS macro. Often one will simply specify =
   here to capture the local variables by value.
...
/** \def CONTINUE_SECTION_IN_THREAD( captures... )
    ...
    Similar to CONTINUE_SECTION_WHEN_IDLE, except that the continuation function
   is executed immediately from a newly created secondary thread. This is useful
   if you want to test your code from another thread (for example, to verify
   that it also works from arbitrary threads, not just the main thread).
...
namespace bdn
{
    /** Executes a test session (runs all tests). This can be called
        ...
        Instead of calling this manually you can also use the macro
    ...
    namespace test
    {
        ...
        /** You can use this inside test applications (those that use one of
           ...
           unhandled problems and redirect their handler to a custom function.
            ...
            The handler is redirected to the custom function when the object is
           constructed and the redirection is removed when the object is
           destructed.
...

            {
                ...
                // test code that can cause an unhandled problem. Any problems
           that happen
                // here will be redirected to the function (and not cause the
           app to exit).
                ...
            }

            // the redirection has been removed here, so unhandled problems get
           the default
