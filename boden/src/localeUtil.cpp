...
bool isUtf8Locale(const std::locale& loc)
{
	...
	{
		// the locale does not have a "multibyte <-> wide" transcoder. That should never happen.
		// But since the locale does not have an encoding, it is not UTF-8. So we return false.
		return false;
	}
