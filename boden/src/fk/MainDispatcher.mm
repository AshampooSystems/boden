...
MainDispatcher::~MainDispatcher()
{
    ...
    {
        // unregister our observer with a call from the main thread because this destructor might
        // be called from a different thread.
        // Note that the observer only holds a reference to the IdleQueue object, which
        // remains valid as long as the observer exists. So the scheduled call does not access
        // the destructed MainDispatcher object.
...
void MainDispatcher::dispose()
{
    ...
    while(!_normalQueue.empty())
    {
        BDN_LOG_AND_IGNORE_EXCEPTION(
                                     ...
                                     , "Error clearing MainDispatcher normal queue item during dispose. Ignoring.");
    }
    
    // for the timed queue the pending calls have iterators to the timed item.
    // So we cannot clear the queue. Instead we have to invalidate the items.
    ...
    for( std::function<void()>& item: _timedNormalQueue )
    {
        BDN_LOG_AND_IGNORE_EXCEPTION(
                                     ...
                                     , "Error clearing MainDispatcher timed normal queue item during dispose. Ignoring.");
    }
    
    
    
    // for timers we also cannot remove the items. So do the same thing that
    // we did for the normal timed items
                                      ...
                                      Priority                 priority )
{
    if(priority==Priority::normal)
    {
        ...
        // we do not schedule the func call directly. Instead we add func
        // to our own queue and schedule a call to our callNextQueued.
        // That enables us to dipose all queued items when dipose() is called.
    ...
    else if(priority==Priority::idle)
    {
        ...
        // Both of these can be solved by enqueuing the setup code with normal
        // priority
        ...
        throw InvalidArgumentError("MainDispatcher::enqueueInSeconds called with invalid priority: "+std::to_string((int)priority) );
}
...
void MainDispatcher::ensureIdleObserverInstalled()
{
    if(!_idleObserverInstalled)
    {
        ...
        _idleObserver = CFRunLoopObserverCreateWithHandler(kCFAllocatorDefault, kCFRunLoopAllActivities, true, 0 /* order */, handler);
...
void MainDispatcher::IdleQueue::activateNext()
{
    ...
    // Afterwards, if nothing else is pending, we will be called again to
...
void MainDispatcher::IdleQueue::dispose()
{
    while(!_funcList.empty())
    {
        BDN_LOG_AND_IGNORE_EXCEPTION(
                                     ...
                                     , "Error clearing MainDispatcher::IdleQueue item during dispose. Ignoring.");
    }
}
