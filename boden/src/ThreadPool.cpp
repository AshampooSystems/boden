    ...
    , _maxThreadCount(maxThreadCount)
{
    ...
    if(_maxThreadCount<_minThreadCount)
        throw InvalidArgumentError("ThreadPool constructor parameter maxThreadCount must be >=minThreadCount");
...
ThreadPool::~ThreadPool()
{
    ...
    // should we stop busy runners or detach them (i.e. let them finish their current job)?
    // We have to consider two cases:
    // 1) the thread pool is deleted when the program exits.
    // 2) the thread pool is deleted at some other time
    // In the case of 1 then it does not really matter. There is a strong likelihood that the app exits
    // before the runner finishes anyway, whether we signal stop or not.
    // In the case of 2 the choice is basically between having the job finish normally and
    // having it be aborted. Since the pool is deleted then it is likely that the caller wants
    // the job to be aborted. If the job is not intended to be aborted then the job could simply
    // keep a reference to the pool and keep it alive until it is finished.
    // So the correct action here is to abort.
...
void ThreadPool::PoolRunner::run()
{
    while(true)
    {
                ...
                
        {
            ...
            if(_pJob==nullptr)
            {
                // this should never happen (note that Signals have no sporadic wake ups).
                programmingError("ThreadPool PoolRunner was woken up, but it has no job and was also not asked to stop.");
            }
        }
        ...
        {
            // the pool wants us to end to reduce the total number of threads.
            break;
        }
    }
}
