...
void MainDispatcher::processItem(std::list< std::function<void()> >& queue)
{    
    try
    {
        std::function<void()> func;
...
void MainDispatcher::enqueue( std::function<void()> func, Priority priority)
{
    ...
    if(priority==Priority::normal)
    {
        ...
        // note that add_idle_full is correct, even if the priority is not idle.
        // That is just a case of a slightly bad function name on the GTK side.
        gdk_threads_add_idle_full(  G_PRIORITY_DEFAULT,
                                    &MainDispatcher::processNormalItemCallback,
                                    static_cast<MainDispatcher*>(this),
                                    NULL );
    }    
...
void MainDispatcher::enqueueInSeconds(double seconds, std::function<void()> func, Priority priority )
{
    ...
    {    
        ...
        gdk_threads_add_timeout_full(   gtkPrio,
                                        millis,
                                        processTimedItemCallback,
                                        pItem,
                                        NULL );
    }
}
