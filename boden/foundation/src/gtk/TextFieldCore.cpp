...
namespace bdn
{
    namespace gtk
    {
            ...
            : ViewCore(pOuter, gtk_entry_new())
        {
            ...
            g_signal_connect(GTK_WIDGET(widget), "activate",
                             G_CALLBACK(activateCallback), this);
        ...
        {
            const gchar *gtkText =
                gtk_entry_get_text(GTK_ENTRY(getGtkWidget()));
            if (String(gtkText) != text) {
                // Disconnect and re-connect handlers so as to avoid triggering
                // them by programmatic changes to the text property
        ...
        void TextFieldCore::changedCallback(GtkEditable *editable,
                                            gpointer user_data)
        {
            TextFieldCore *pCore = (TextFieldCore *)user_data;

            P<TextField> pOuter =
                cast<TextField>(pCore->getOuterViewIfStillAttached());
            if (pOuter != nullptr) {
        ...
        void TextFieldCore::activateCallback(GtkEditable *editable,
                                             gpointer user_data)
        {
            TextFieldCore *pCore = (TextFieldCore *)user_data;
        ...
        void TextFieldCore::connectChangedHandler()
        {
            GtkWidget *widget = getGtkWidget();
            _changedHandlerId =
                g_signal_connect(GTK_WIDGET(widget), "changed",
                                 G_CALLBACK(changedCallback), this);
        }
