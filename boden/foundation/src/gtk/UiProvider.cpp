    ...
    namespace gtk
    {
        ...
        UiProvider::UiProvider()
        {
            ...
            if (pFontName != nullptr) {
                ...
                // reverse iterate through the font and find the last number in
                // the name,
            ...
            if (fontSize <= 0) {
                // unable to extract a font size. Log an error and use 11 as a
                // default.
                logError("Unable to determine default GTK font size. Using "
                         "default.");

                fontSize = 11;
            }

            // the font size is in points, which is "72 DPI". The virtual device
            // coordinates
            // (=DIPs) are assumed to be 96 dpi. So we have to multiple
            // accordingly.
            _semDips = fontSize / 72.0 * 96.0;
        }

        String UiProvider::getName() const { return "gtk"; }
