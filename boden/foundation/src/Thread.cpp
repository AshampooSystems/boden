...
namespace bdn
{
...
#ifdef BDN_PLATFORM_ANDROID
        // ensure that the thread is connected to the VM. This is important for
        // debugging so that the thread will always show up in the IDE.
    ...
    void Thread::sleepMillis(int64_t millis)
    {
        if (millis <= 0)
            ...
            // the emscripten standard library sometimes sleeps for a few millis
            // too little. So we add a few so that we always sleep at least the
            // requested amount.
            millis += 5;
#endif
