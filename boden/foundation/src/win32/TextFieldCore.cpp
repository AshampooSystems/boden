#define NOMINMAX // prevent definition of min()/max() macros in windows.h to
                 // avoid clashes with std::numeric_limits
#include <bdn/init.h>
...
namespace bdn
{
    namespace win32
    {
        ...
        TextFieldCore::TextFieldCore(TextField *pOuter)
            : ViewCore(pOuter, "EDIT", pOuter->text(),
                       WS_CHILD | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 0)
        {
            setText(pOuter->text());
        }
        ...
        void TextFieldCore::setText(const String &text)
        {
            if (text != getWindowText(getHwnd())) {
                P<bdn::TextField> outer =
                    cast<bdn::TextField>(getOuterViewIfStillAttached());
                if (outer) {
        ...
        Size TextFieldCore::calcPreferredSize(const Size &availableSpace) const
        {
            ...
            if (pad.isNull()) {
                // we should use the "default" padding. On win32 there is no
                // real system-defined default, so we choose one that looks good
                // for most text fields:
                uiPadding = UiMargin(UiLength::sem(0.2), UiLength::sem(0.));
            } else
                uiPadding = pad;
        ...
        void TextFieldCore::handleMessage(MessageContext &context,
                                          ...
                                          WPARAM wParam, LPARAM lParam)
        {
            ...
            return ViewCore::handleMessage(context, windowHandle, message,
                                           wParam, lParam);
        }

        void TextFieldCore::handleParentMessage(MessageContext &context,
                                                HWND windowHandle, UINT message,
                                                WPARAM wParam, LPARAM lParam)
        {
            switch (message) {
            ...
            return ViewCore::handleParentMessage(context, windowHandle, message,
                                                 wParam, lParam);
        }
