...
namespace bdn
{
    namespace win32
    {
        ...
        Array<String> parseWin32CommandLine(const String &commandLine)
        {
            int argCount = 0;
            wchar_t **argPtrs =
                ::CommandLineToArgvW(commandLine.asWidePtr(), &argCount);
            if (argPtrs == NULL) {
                BDN_WIN32_throwLastError(
                    ErrorFields()
                        .add("func", "CommandLineToArgvW")
        ...
        {
            // we ignore the commandline arguments we get from the main
            // function. There are problems with the unicode-encoding with those
            // (e.g. if the code page cannot represent the actual parameters).
            // So we always get the command line from GetCommandLine, which
            // handles Unicode properly
            Array<String> args =
                bdn::win32::parseWin32CommandLine(::GetCommandLineW());
