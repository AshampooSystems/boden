...
namespace bdn
{
    namespace win32
    {
        ...
        BDN_SAFE_STATIC_IMPL(ContainerViewCore::ContainerViewCoreClass,
                             ContainerViewCore::ContainerViewCoreClass::get);

        ContainerViewCore::ContainerViewCore(ContainerView *pOuter)
            : ViewCore(pOuter, ContainerViewCoreClass::get().getName(), "",
                       WS_CHILD, 0)
        {}

        Size
        ContainerViewCore::calcPreferredSize(const Size &availableSpace) const
        {
