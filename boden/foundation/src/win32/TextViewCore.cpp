...
namespace bdn
{
    namespace win32
    {
        ...
        TextViewCore::TextViewCore(TextView *pOuter)
            : ViewCore(pOuter, "STATIC", pOuter->text(), SS_NOPREFIX | WS_CHILD,
                       0)
        {}
        ...
        {
            // note that we ignore availableHeight because there is no way for
            // us to reduce the height of the text. We can only limit the width
            // (by wrapping lines)

            P<TextView> pTextView =
                cast<TextView>(getOuterViewIfStillAttached());
            String text;
            Size maxSize;
            Size minSize;
            Size hintSize;
...

            {
                ...
                // if we have a maximum and it is less than the hint then we
                // wrap according to that.
            ...
            if (pTextView != nullptr) {
                ...
                // clip against the max size again. It may have been exceeded if
                // the content did not fit, and we never want that.
