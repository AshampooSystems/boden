...
namespace bdn
{
    ...
    P<IUiProvider> getDefaultUiProvider()
    {
        if (getAppRunner()->isCommandLineApp()) {
            // on win32 we use wchar_t so that we get full unicode support on
            // the commandline.
            static P<StdioUiProvider<wchar_t>> pProvider(
                newObj<StdioUiProvider<wchar_t>>(&std::wcin, &std::wcout,
                                                 &std::wcerr));
...
namespace bdn
{
    namespace win32
    {
        ...
        {
            // the default UI font on Windows changed multiple times over the
            // years. There are several deprecated legacy ways to get it (the
            // font name "MS Shell Dlg", or GetStockObject(DEFAULT_GUI_FONT) or
            // the DS_SHELLFONT attribute. But all of these return old oudated
            // fonts on modern Windows versions. This thread contains a good
            ...
            if (!::SystemParametersInfo(SPI_GETNONCLIENTMETRICS, metrics.cbSize,
                                        &metrics, FALSE)) {
            ...
            // the default UI font will have a size that is already scaled
            // according to the UI scalefactor of the default monitor.
            POINT ptZero = {0, 0};
            double primaryUiScaleFactor = getUiScaleFactorForMonitor(
                MonitorFromPoint(ptZero, MONITOR_DEFAULTTOPRIMARY));

            // store the UI font size at scale factor 1 as a double, so that we
            // do not get rounding errors when we scale back up.
            _defaultUiFontSizeAtScaleFactor1 =
                _defaultUiFontInfo.lfHeight / primaryUiScaleFactor;

            _defaultUiFontInfo.lfHeight =
                std::lround(_defaultUiFontSizeAtScaleFactor1);
        ...
        P<Font> UiProvider::createFont(const FontSpec &spec)
        {
            ...
            info.lfHeight = std::lround(_defaultUiFontSizeAtScaleFactor1 *
                                        spec.uiScaleFactor);
        ...
        {
            {
                ...
                if (_pTextUi == nullptr) {
                    ...
                    _pTextUi = newObj<TextUiCombiner>(
                        newObj<ViewTextUi>(),
                        newObj<StdioTextUi<wchar_t>>(&std::wcin, &std::wcout,
                                                     &std::wcerr));
                }
            }
