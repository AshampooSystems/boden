...
namespace bdn
{
    namespace win32
    {
        ...
        GlobalMessageWindow::GlobalMessageWindow()
            : RequireNewAlloc<MessageWindowBase, GlobalMessageWindow>(
                  "bdn::GlobalMessageWindow")
        {}

        GlobalMessageWindow::~GlobalMessageWindow() { destroy(); }

        void GlobalMessageWindow::postCall(ISimpleCallable *pCallable)
        {
            pCallable->addRef();
            ::PostMessage(getHwnd(), MessageCall, 0,
                          reinterpret_cast<LPARAM>(pCallable));
        }
        ...
        void GlobalMessageWindow::callOnceWhenIdle(ISimpleCallable *pCallable)
        {
            ...
            // need to break it with a dummy message and cause a new idle phase
            // to be begun.
        ...
        void GlobalMessageWindow::notifyIdleBegun()
        {
            ...
            while (!toCallList.empty()) {
                ...
                try {
                    ...
                    logError(e,
                             "Exception thrown by ISimpleCallable::call during "
                             "GlobalMessageWindow::notifyIdleBegun. Ignoring.");
                }
        ...
        void GlobalMessageWindow::handleMessage(MessageContext &context,
                                                HWND windowHandle, UINT message,
                                                WPARAM wParam, LPARAM lParam)
        {
            if (message == MessageCall) {
                ISimpleCallable *pCallable =
                    reinterpret_cast<ISimpleCallable *>(lParam);
                ...
                try {
                    ...
                    logError(
                        e,
                        "Exception thrown by ISimpleCallable::call during "
                        "GlobalMessageWindow MessageCall handling. Ignoring.");
                }
