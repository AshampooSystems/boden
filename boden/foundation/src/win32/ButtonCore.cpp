...
namespace bdn
{
    namespace win32
    {
        ...
        ButtonCore::ButtonCore(Button *pOuter)
            : ViewCore(pOuter, "BUTTON", pOuter->label(),
                       BS_PUSHBUTTON | WS_CHILD, 0)
        {}
        ...
        Size ButtonCore::calcPreferredSize(const Size &availableSpace) const
        {
            ...
            // we completely ignore the available space. We do not have any way
            // to reduce the needed size for the button.

            P<const Button> pButton =
                cast<const Button>(getOuterViewIfStillAttached());
            if (pButton != nullptr)
                label = pButton->label();
            ...
            if (pad.isNull()) {
                ...
                // for most buttons:
                uiPadding = UiMargin(UiLength::sem(0.12), UiLength::sem(0.5));
            } else
                uiPadding = pad;

            prefSize += uiMarginToDipMargin(uiPadding);

            // size for the 3D border around the button. Note that prefSize is
            // in DIPs, so it is ok to add the values unscaled.
        ...
        void ButtonCore::handleParentMessage(MessageContext &context,
