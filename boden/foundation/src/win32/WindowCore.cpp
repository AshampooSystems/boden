...
namespace bdn
{
    namespace win32
    {
        ...
        BDN_SAFE_STATIC_IMPL(WindowCore::WindowCoreClass,
                             WindowCore::WindowCoreClass::get);

        WindowCore::WindowCore(Window *pWindow)
            : ViewCore(pWindow, WindowCore::WindowCoreClass::get().getName(),
                       pWindow->title(), WS_OVERLAPPEDWINDOW | WS_POPUPWINDOW,
                       WS_EX_APPWINDOW)
        {
            initUiScaleFactor();
        }

        WindowCore::~WindowCore() { Win32Window::destroy(); }
        ...
        void WindowCore::initUiScaleFactor()
        {
            ...
            if (FAILED(result)) {
                throw hresultToSystemError(
                    result,
                    ErrorFields()
                        .add("func", "GetDpiForMonitor")
        ...
        {
            // note that the DPI we get here is not a real physical DPI. Rather
            // it is a more complicated value that takes into account the device
            // type and even user settings. It is basically the UI scale factor
            // that Windows wants us to use, multiplied by 96.
        ...
        void WindowCore::dpiChanged(int newDpi, const RECT *pSuggestedNewRect)
        {
            ...
            if (pSuggestedNewRect != NULL) {
                Rect newRect =
                    win32RectToRect(*pSuggestedNewRect, getUiScaleFactor());
        ...
        {
            // whenever our size changes it means that we have to update our
            // layout
            P<View> pView = getOuterViewIfStillAttached();
            if (pView != nullptr)
                pView->needLayout(
        ...
        {
            // In Win32 there is AdjustWindowRect to get the window size from
            // the content area size, given a set of style flags.

            // We use that to extract the size of the non-client border. Note
            // that this will give an incorrect result if Windows ever decides
            // to adapt the border size to the client area size (i.e. if bigger
            // windows get a bigger border, for example). If something like that
            // happens then we will have to adapt calcPreferredSize and
            // adjustAndSetBounds accordingly.
            ...
            if (!::AdjustWindowRectEx(&windowRect, style,
                                      (menuHandle != NULL) ? TRUE : FALSE,
                                      exStyle)) {
                BDN_WIN32_throwLastError(
                    ErrorFields()
                        .add("func", "AdjustWindowRectEx")
                        .add("action", "WindowCore::getNonClientMargin"));
            }

            // the resulting rect is for an unspecified display, i.e. windows
            // will assume that the display's scale factor is 1. I.e. it is in
            // DIPs.

            return Margin(contentRect.top - windowRect.top,
        ...
        void WindowCore::layout()
        {
            ...
            if (pWindow != nullptr) {
                ...
                defaultWindowLayoutImpl(
                    pWindow, win32RectToRect(clientRect, getUiScaleFactor()));
            }
        }
        ...
        void WindowCore::autoSize()
        {
            P<Window> pOuterView = cast<Window>(getOuterViewIfStillAttached());
            if (pOuterView != nullptr)
                defaultWindowAutoSizeImpl(pOuterView,
        ...
        void WindowCore::handleMessage(MessageContext &context,
