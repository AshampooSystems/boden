...
namespace bdn
{
    namespace win32
    {
        ...
        BDN_SAFE_STATIC_IMPL(ScrollViewCore::ScrollViewCoreClass,
                             ScrollViewCore::ScrollViewCoreClass::get);
        BDN_SAFE_STATIC_IMPL(ScrollViewCore::ContentContainerClass,
                             ScrollViewCore::ContentContainerClass::get);

        ScrollViewCore::ScrollViewCore(ScrollView *pOuter)
            : ViewCore(pOuter,
                       ScrollViewCore::ScrollViewCoreClass::get().getName(),
                       "ScrollView", WS_CHILD, 0)
        {
            // we need to know the size of our nonclient border for various
            // things. Calculate that

            HWND hwnd = getHwnd();
            if (hwnd != NULL) {
                RECT rect{0, 0, 0, 0};
                ::AdjustWindowRect(
                    &rect, (DWORD)::GetWindowLongPtr(getHwnd(), GWL_STYLE),
                    FALSE);
        ...
        ScrollViewCore::ContentContainer::ContentContainer(HWND parentHwnd)
            : Win32Window(
                  ScrollViewCore::ContentContainerClass::get().getName(),
                  "ScrollViewContentContainer", WS_CHILD | WS_VISIBLE, 0,
                  parentHwnd)
        {}
        ...
        {
            // we don't care. The setting is only used in layout and there we
            // get it from the outer view directly.
        }
        ...
        double
        ScrollViewCore::getHorizontalScrollBarHeight(double uiScaleFactor)
        {
            // these values are unscaled (i.e. for scale factor 1). That means
            // that they are in DIPs.
        ...
        void ScrollViewCore::layout()
        {
            ...
            if (pOuterView == nullptr || hwnd == NULL) {
                // the outer view has already been destroyed or our window has
                // already been destroyed. No layout necessary. Do nothing.
            } else {
                double uiScaleFactor = getUiScaleFactor();

                double horzBarHeight =
                    getHorizontalScrollBarHeight(uiScaleFactor);
                double vertBarWidth = getVerticalScrollBarWidth(uiScaleFactor);

                // the client rect automatically excludes the space needed for
                // the scroll bars. Subtracting it manually does not work
                // properly in all cases since there seems to be short time
                // period in which the client rect is not yet updated after a
                // scrollbar is hidden or shown. So instead we use the window
                // size, not the client size:

                RECT winRect;
                ::GetWindowRect(hwnd, &winRect);
                Size viewPortSizeWithoutScrollbars =
                    win32RectToRect(winRect, uiScaleFactor).getSize();

                // subtract the size of the border.
                viewPortSizeWithoutScrollbars.width -=
                    _nonClientMargins.left + _nonClientMargins.right;
                viewPortSizeWithoutScrollbars.height -=
                    _nonClientMargins.top + _nonClientMargins.bottom;
                ...
                _viewPortSizePixels =
                    sizeToWin32Size(viewPortSize, uiScaleFactor);
                ...
                ::SetWindowPos(_pContentContainer->getHwnd(), NULL,
                               -_scrollPositionPixels.x,
                               -_scrollPositionPixels.y, _clientSizePixels.cx,
        ...
        void ScrollViewCore::updateWin32ScrollInfo()
        {
            ...
            if (hwnd == NULL) {
                ...
                ::SetScrollInfo(hwnd, SB_HORZ, &scrollInfo,
                                TRUE); // TRUE means redraw scrollbar
                ...
                ::SetScrollInfo(hwnd, SB_VERT, &scrollInfo,
                                TRUE); // TRUE means redraw scrollbar
            }
        }

        void ScrollViewCore::handleMessage(MessageContext &context,
                                           ...
                                           WPARAM wParam, LPARAM lParam)
        {
            if (message == WM_VSCROLL || message == WM_HSCROLL) {
                ...
                // If action is SB_THUMBTRACK or SB_THUMBPOSITION then the
                // tracking pos will already be the desired new scroll position.
                // If action is anything else then we must calculate the
                // position ourselves.

                int action = LOWORD(wParam);

                bool basePosIsTrackPos =
                    (action == SB_THUMBPOSITION || action == SB_THUMBTRACK);
                ...
                if (action == SB_LINEDOWN) {
                ...
                else if (action == SB_THUMBPOSITION ||
                    ...
                    // Just fall through and set the current tracking position
                    // as the scroll position - this should not change anything.
                } else {
                    // unknown action. Let this through to the default
                    // implementation so that Windows can detect that we do not
                    // support this type of scrolling (and can possibly give the
                    // user feedback of some kind)
                    ViewCore::handleMessage(context, windowHandle, message,
                                            wParam, lParam);
                    return;
                }
                ...
                // and make it the actual position. If we do not do this then
                // the scrollbar will jump back to the old position when the
                // users stops scrolling.

                info.fMask = SIF_POS;
                ::SetScrollInfo(windowHandle, barType, &info,
                                TRUE); // TRUE means redraw scrollbar
                ...
                ViewCore::handleMessage(context, windowHandle, message, wParam,
                                        lParam);
        }
        ...
        {
            ::SetWindowPos(_pContentContainer->getHwnd(), NULL,
                           -_scrollPositionPixels.x, -_scrollPositionPixels.y,
                           0, 0, SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_NOSIZE);
        }

        POINT ScrollViewCore::getScrollPositionInPixels(
            ScrollViewCore::ScrollPosType scrollPosType)
        {
            POINT scrollPos{0, 0};
            HWND hwnd = getHwnd();
            ...
            if (hwnd != NULL) {
                ...
                memset(&info, 0, sizeof(info));
                info.cbSize = sizeof(info);
                info.fMask = (scrollPosType == ScrollPosType::track)
                                 ? SIF_TRACKPOS
                                 : SIF_POS;

                if (::GetScrollInfo(hwnd, SB_HORZ, &info))
                    scrollPos.x = (scrollPosType == ScrollPosType::track)
                                      ? info.nTrackPos
                                      : info.nPos;

                if (::GetScrollInfo(hwnd, SB_VERT, &info))
                    scrollPos.y = (scrollPosType == ScrollPosType::track)
                                      ? info.nTrackPos
                                      : info.nPos;
            }
        ...
        void ScrollViewCore::scrollClientRectToVisible(const Rect &rect)
        {
            ...
            if (hwnd != NULL) {
                ...
                currVisibleRect.right =
                    _scrollPositionPixels.x + _viewPortSizePixels.cx;
                currVisibleRect.bottom =
                    _scrollPositionPixels.y + _viewPortSizePixels.cy;

                POINT maxScrollPos{
                    _clientSizePixels.cx - _viewPortSizePixels.cx,
                ...
                if (targetRect.right - targetRect.left >
                    _viewPortSizePixels.cx) {
                    if (currVisibleRect.left >= targetRect.left &&
                        currVisibleRect.right <= targetRect.right) {
                        // if the current visible rect is already inside the
                        // target rect then we do not want to scroll
                        targetRect.left = currVisibleRect.left;
                        targetRect.right = currVisibleRect.right;
                    } else {
                        // we want to scroll towards the of the target rect that
                        // is closest
                        int leftDistance =
                            std::abs(targetRect.left - currVisibleRect.left);
                        int rightDistance =
                            std::abs(targetRect.right - currVisibleRect.right);

                        if (rightDistance < leftDistance)
                            targetRect.left =
                                targetRect.right - _viewPortSizePixels.cx;
                        else
                            targetRect.right =
                                targetRect.left + _viewPortSizePixels.cx;
                    }
                }

                if (targetRect.bottom - targetRect.top >
                    _viewPortSizePixels.cy) {
                    if (currVisibleRect.top >= targetRect.top &&
                        currVisibleRect.bottom <= targetRect.bottom) {
                        targetRect.top = currVisibleRect.top;
                        targetRect.bottom = currVisibleRect.bottom;
                    } else {
                        int topDistance =
                            std::abs(targetRect.top - currVisibleRect.top);
                        int bottomDistance = std::abs(targetRect.bottom -
                                                      currVisibleRect.bottom);

                        if (bottomDistance < topDistance)
                            targetRect.top =
                                targetRect.bottom - _viewPortSizePixels.cy;
                        else
                            targetRect.bottom =
                                targetRect.top + _viewPortSizePixels.cy;
                    }
                }
        ...
        void ScrollViewCore::updateVisibleClientRect()
        {
            ...
            if (pOuterView != nullptr) {
                ...
                Point scrollPos =
                    win32PointToPoint(_scrollPositionPixels, scaleFactor);
                Size viewPortSize =
                    win32SizeToSize(_viewPortSizePixels, scaleFactor);
