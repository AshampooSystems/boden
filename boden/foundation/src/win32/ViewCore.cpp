...
namespace bdn
{
    namespace win32
    {
        ...
        ViewCore::ViewCore(View *pOuterView, const String &className,
                           const String &name, DWORD style, DWORD exStyle,
                           int x, int y, int width, int height)
            : Win32Window(className, name,
                          style | (pOuterView->visible() ? WS_VISIBLE : 0),
                          exStyle,
                          ViewCore::getViewParentHwndForChildren(
                              pOuterView->getParentView()),
                          x, y, width, height)
        {
            ...
            if (pParentView == nullptr) {
                // we are a top level window without a parent. In that case the
                // constructor of the derived class will initialize the ui scale
                // factor.
            } else {
        ...
        void ViewCore::setUiScaleFactor(double factor)
        {
            if (factor != _uiScaleFactor) {
                ...
                for (const P<View> &pChildView : childViews) {
                    P<ViewCore> pCore =
                        cast<ViewCore>(pChildView->getViewCore());
                    if (pCore != nullptr)
        ...
        void ViewCore::updateFont()
        {
            ...
            if (pOuter != nullptr) {
                _pFont =
                    UiProvider::get().getFontForView(pOuter, _uiScaleFactor);

                _emSizeDips = _pFont->getSizePixels() / _uiScaleFactor;

                ::SendMessage(getHwnd(), WM_SETFONT,
                              (WPARAM)_pFont->getHandle(), FALSE);
            } else
                _emSizeDips = 0;
        }
        ...
        {
            // do nothing. We handle it on the fly when our preferred size is
            // calculated.
        }
        ...
        {
            // nothing to invalidate for ourselves (since we do not cache
        ...
        {
            // do nothing in the base implementation. Most views do not have
            // child views.
        }
        ...
        Rect ViewCore::adjustAndSetBounds(const Rect &requestedBounds)
        {
            ...
            ::SetWindowPos(getHwnd(), NULL, winRect.left, winRect.top,
        ...
        Rect ViewCore::adjustBounds(const Rect &requestedBounds,
        ...
        void
        ViewCore::setHorizontalAlignment(const View::HorizontalAlignment &align)
        {
            // do nothing. The View handles this.
        }

        void
        ViewCore::setVerticalAlignment(const View::VerticalAlignment &align)
        {
            // do nothing. The View handles this.
        }
        ...
        {
            // do nothing in the base implementation. Most views do not use the
            // size hint.
        }

        void ViewCore::setPreferredSizeMinimum(const Size &limit)
        {
            // do nothing here. We use the property from the view directly when
            // we need it.
        }
        ...
        void ViewCore::updateOrderAmongSiblings()
        {
            ...
            if (pOuter != nullptr) {
                ...
                if (ourHwnd != NULL) {
                    ...
                    if (pParentView != nullptr) {
                        View *pPrevSibling =
                            pParentView->findPreviousChildView(pOuter);

                        if (pPrevSibling == nullptr) {
                            // we are the first child
                            ::SetWindowPos(ourHwnd, HWND_TOP, 0, 0, 0, 0,
                                           SWP_NOMOVE | SWP_NOSIZE);
                        } else {
                            HWND prevSiblingHwnd = getViewHwnd(pPrevSibling);

                            if (prevSiblingHwnd != NULL)
                                ::SetWindowPos(ourHwnd, prevSiblingHwnd, 0, 0,
        ...
        bool ViewCore::tryChangeParentView(View *pNewParentView)
        {
            ...
            if (ourHwnd == NULL) {
                ...
                if (getViewParentHwndForChildren(pNewParentView) ==
                    currentParentHwnd) {
                    // the underlying win32 window is the same.
                    // Note that this case is quite common, since many View
                    // containers are actually light objects that do not have
                    // their own backend Win32 window. So often the Win32 window
                    // of some higher ancestor is shared by many windows.

                    // This also happens when a child view's ordering position
                    // is changed inside the parent. So we have to check if we
        ...
        void ViewCore::handleParentMessage(MessageContext &context,
        ...
        void ViewCore::handleMessage(MessageContext &context, HWND windowHandle,
                                     UINT message, WPARAM wParam, LPARAM lParam)
        {
            ViewCore *pChildCore =
                findChildCoreForMessage(message, wParam, lParam);
            if (pChildCore != nullptr)
                pChildCore->handleParentMessage(context, windowHandle, message,
                                                wParam, lParam);

            if (message == WM_SIZE) {
                // we invalidate the window contents whenever the size changes.
                // Otherwise we have found that some controls (e.g. static text)
                // are only partially updated (seen on Windows 10).
                ::InvalidateRect(windowHandle, NULL, NULL);
            }

            Win32Window::handleMessage(context, windowHandle, message, wParam,
                                       lParam);
        }

        P<ViewCore> ViewCore::findChildCoreForMessage(UINT message,
                                                      WPARAM wParam,
                                                      LPARAM lParam)
        {
            HWND childHwnd = NULL;
