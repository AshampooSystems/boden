...
- (void)setCheckboxCore:(bdn::ios::CheckboxCore*)pCore
{
    _pCore = pCore;
}
...
{
    BdnIosCheckboxComposite* switchComposite = [[BdnIosCheckboxComposite alloc] initWithFrame:CGRectMake(0,0,0,0)];
...
void CheckboxCore::layout()
{
    ...
    CGRect checkboxFrame = CGRectMake(0, compositeBounds.size.height / 2. - checkboxBounds.size.height / 2., checkboxBounds.size.width, checkboxBounds.size.height);
    ((BdnIosCheckboxComposite*)_composite).checkbox.frame = checkboxFrame;
    
    // Center label vertically, align next to checkbox horizontally in composite
    CGRect labelFrame = CGRectMake(checkboxBounds.size.width + BDN_IOS_CHECKBOX_LABEL_MARGIN, compositeBounds.size.height / 2. - labelBounds.size.height / 2., labelBounds.size.width, labelBounds.size.height);
