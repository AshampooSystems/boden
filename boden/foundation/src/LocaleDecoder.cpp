...
{    
	...
	while( outWritten==0 && _pInNext!=_pInEnd )
	{
		...
		int convResult = _pCodec->in( _pDecodeState->state, pInBegin, _pInEnd, _pInNext, pOutBegin, pOutBegin+outBufferSize, pOutNext );
        
        outWritten = pOutNext-pOutBegin;

        // some codec implementations return "partial" when they
        // cannot convert a character. At the same time, partial will be returned
        // in normal operation when the output buffer is not big enough.
        // So we need to distinguish the two "partial" results.
        // Luckily, that is pretty easy. We can treat partial like error when we
        // got not output and no input was consumed. That is sufficient, even in the case
        // where convertible character precede the problem area. In that case the successfully
        // converted characters will be consumed and in the next iteration the problem character
        // will be at the start, thus returning 0 written and 0 read.
		if(outWritten==0
           && (   convResult==std::codecvt_base::error
               || (convResult==std::codecvt_base::partial && _pInNext==pInBegin) ) )
		{
			// no character can be converted because of an error. This should be pretty rare. On most systems
