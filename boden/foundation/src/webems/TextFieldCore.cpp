...
{
    EM_ASM_({
    	bdn_webems_TextFieldCore_onInput = Module.cwrap('bdn_webems_TextFieldCore_onInput', 'null', ['number']);

    	document.getElementById(Pointer_stringify($1)).oninput = function () {
    		bdn_webems_TextFieldCore_onInput($0);
    	}

    }, this, _domObject["id"].as<std::string>().c_str());

    emscripten_set_keydown_callback(_domObject["id"].as<std::string>().c_str(), this, false, onKeyDownCallback);
...
void TextFieldCore::onInputCallback(void* textFieldCore)
{
    ...
    P<TextField> outer = cast<TextField>(core->getOuterViewIfStillAttached());
    if (outer) {
        outer->setText( core->_domObject["value"].as<std::string>() );
    }
}

EM_BOOL TextFieldCore::onKeyDownCallback(int eventType, const EmscriptenKeyboardEvent* keyEvent, void* userData)
{
