    ...
    namespace webems
    {
                                     ...
                                     Priority priority)
        {
            ...
            if (priority == Priority::normal)
                ...
                // At the time of this writing, requestIdleCallback is
                // non-standard and only supported by a handful of browsers
                // (Chrome, Opera, Android). So we cannot really use that,
                // otherwise we risk to have vastly different timing
                // dependending on the browser.

                // It is also difficult to define what "idle" means for a web
                // browser. DOM changes take effect immediately, so there is are
                // no queued events for that. The whole rendering is done in a
                // way that is mostly opaque to the app, so we can ignore
                // rendering events. So what might be queued up are basically
                // only user input events.

                // If we use emscripten_async_call then any events that are
                // already in the queue will be processed before our idle
                // handler is called. So the only difference for external events
                // are user input events that come in between now and the time
                // emscripten_async_call calls our function. In a perfect world
                // those SHOULD be handled before our idle callback. But we
                // could argue that those events come at "random" times anyway
                // and the behaviour of the whole system does not depend on
                // exact timing for them. In other words: we could argue that we
                // can safely treat newly enqueued user input events as if they
                // happened just after our idle handler was called, instead of
                // the real occurrence time (between now and the handler call).

                // So as far as external events are concerned, it should be
                // "consistent" (although slightly incorrect) to behave the same
                // way as normal priority here, as long as we ensure the
                // ordering of our OWN scheduled calls.

                // So we put idle items in their own queue and just make sure
                // that our normal queue items are processed first.
                pQueue = &_idleQueue;
            }
            ...
            emscripten_async_call(
                &MainDispatcher::processQueueItemCallback,
                static_cast<void *>(static_cast<MainDispatcher *>(this)), 0);
        }
                                              ...
                                              Priority priority)
        {
            ...
            if (millis < 1)
                ...
                emscripten_async_call(&MainDispatcher::processTimedItemCallback,
                                      static_cast<void *>(pItem), (int)millis);
            }
        }
        ...
        void MainDispatcher::processTimer(Timer *pTimer)
        {
            try {
                ...
                if (!continueTimer) {
                    ...
                    EM_ASM_({ window.clearInterval($0); }, pTimer->jsId);
                }
            }
            catch (...) {
                if (!bdn::unhandledException(true))
                                         ...
                                         std::function<bool()> func)
        {
            ...
            // dispatcher). So we will be kept alive as long as the timer runs.
            ...
            int timerId = EM_ASM_INT(
                {
                    var eventFunc = Module.cwrap(
                        'bdn_webems_MainDispatcher_timerEventCallback',
                        'null',      // return type
                        ['number']); // argument types - note that number can be
                                     // used for pointers

                    return window.setInterval(function() { eventFunc($0); },
                                              $1);
                },
                pTimer, millis);
