...
namespace bdn
{
    namespace winuwp
    {
        ...
        void UwpLayoutBridge::invalidateArrange(
        ...
        void UwpLayoutBridge::invalidateMeasure(
            ::Windows::UI::Xaml::FrameworkElement ^ pElement)
        {
            // OutputDebugString( ( "InvalidateMeasure on "+String(
            // pElement->GetType()->ToString()->Data() )+"\n").asWidePtr() );
        ...
        void UwpLayoutBridge::measureStarting()
        {
            ...
            // measure calls will immediately follow. So we reset the pending
            // flag.
        ...
        void UwpLayoutBridge::measureEnding(IUwpLayoutDelegate *pDelegate,
                                            const Size &availableSpace)
        {
            // we also assume that arrange operations that were scheduled DURING
            // another arrange will be performed immediately. So we also reset
            // the pending flag again when arrange ends.
            _measurePending = false;
            ...
            if (_activeMeasureCalls == 0) {
                ...
                // measure calls that occur during finalize should not trigger
                // another finalization. We set _activeFinalizers to a dummy
                // number to prevent that.
                _activeMeasureCalls = 0x10000000;

                // OutputDebugString( (String(typeid(*pDelegate).name()) +"
                // finalizeUwpMeasure("+std::to_string(availableSpace.width)+",
        ...
        Size UwpLayoutBridge::doMeasure(IUwpLayoutDelegate *pDelegate,
                                        const Size &availableSpace)
        {
            measureStarting();
        ...
        void UwpLayoutBridge::arrangeStarting()
        {
            ...
            // arrange calls will immediately follow. So we reset the pending
            // flag.
        ...
        Size UwpLayoutBridge::doArrange(IUwpLayoutDelegate *pDelegate,
