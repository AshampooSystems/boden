...
private:
    static P<JSwitch> _createJSwitch(T* pOuter)
    {
        ...
        if(pParent==nullptr)
            throw ProgrammingError("SwitchCore instance requested for a Switch that does not have a parent.");

        P<ViewCore> pParentCore = cast<ViewCore>( pParent->getViewCore() );
        if(pParentCore==nullptr)
            throw ProgrammingError("SwitchCore instance requested for a Switch with core-less parent.");
...
public:
    SwitchCore( T* pOuter )
     : ViewCore( pOuter, _createJSwitch(pOuter) )
    {
