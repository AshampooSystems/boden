...
private:
    static P<JCheckBox> _createJCheckBox(T* pOuter)
    {
        ...
        if(pParent==nullptr)
            throw ProgrammingError("CheckboxCore instance requested for a Checkbox that does not have a parent.");

        P<ViewCore> pParentCore = cast<ViewCore>( pParent->getViewCore() );
        if(pParentCore==nullptr)
            throw ProgrammingError("CheckboxCore instance requested for a Checkbox with core-less parent.");
...
public:
    CheckboxCore( T* pOuter )
     : ViewCore( pOuter, _createJCheckBox(pOuter) )
    {
