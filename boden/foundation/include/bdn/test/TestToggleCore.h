...
protected:
    ...
    /** Verifies that the toggle core's label has the expected value
        (the label set in the outer toggle object's Toggle::label() property.*/
    virtual void verifyCoreLabel()=0;

    /** Verifies that the toggle core's on state has the expected value
        (the on state set in the outer toggle object's Toggle::on() property.*/
    virtual void verifyCoreOn()=0;

    /** Sets the toggle's on state on the platform only without using the framework.
