...
protected:
    ...
    void runPostInitTests() override
    {
        ...
        SECTION("on")
        {
            ...
            // We decided to NOT test the case where the platform state is set programatiacally
            /*SECTION("platform")
            {
                // If core is implemented correctly, this will notify the outer control (_pSwitch)
                // that on has been changed externally using mechanisms provided by the platform core.
                setCoreOnPlatform(true);
    ...
    // Pure virtual methods to be implemented by platform-specific core tests

    /** Verifies that the Switch core's label has the expected value
        (the label set in the outer Switch object's Switch::label() property.*/
    virtual void verifyCoreLabel()=0;

    /** Verifies that the Switch core's on state has the expected value
        (the on state set in the outer Switch object's Switch::on() property.*/
    virtual void verifyCoreOn()=0;

    /** Sets the Switch's on state on the platform only without using the framework.
        This is used to test whether the core correctly notifies its outer control
