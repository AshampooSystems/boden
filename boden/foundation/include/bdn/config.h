...
#error Unsupported wchar size
    ...
    (__EMSCRIPTEN_major__ == 1 && __EMSCRIPTEN_minor__ < 36)
// Older emscripten versions had compiler bugs that cause major troubles with
// some integral parts of the library (exception handling not working correctly
// in some cases, etc.). So we cannot use those.
#pragma error("Emscripten 1.36.0 or higher required.")
#endif

// no support for threads (yet). There is actually an implementation from
// Emscripten but as of the time of this writing it only works if the Javascript
// code is executed with Firefox nightly. It depends on an extension of the web
// worker specification, which is not standardized yet. So we disable threading
// support.
...
// the code below should definitely cover macOS
    ...
    defined(__AARCH64EB__) || defined(__THUMBEB__) || defined(_MIPSEB) ||      \
...
#define BDN_IS_BIG_ENDIAN 1
    ...
    defined(__AARCH64EL__) || defined(__THUMBEL__) || defined(_MIPSEL) ||      \
...
#if defined(__BIG_ENDIAN__) || defined(__ARMEB__) || defined(__AARCH64EB__) || \
    defined(__THUMBEB__) || defined(_MIPSEB) || defined(__MIPSEB) ||           \
    defined(__MIPSEB__) ||                                                     \
    (defined(__BYTE_ORDER) && defined(__BIG_ENDIAN) &&                         \
     __BYTE_ORDER == __BIG_ENDIAN) ||                                          \
    (defined(__BYTE_ORDER__) && defined(__ORDER_BIG_ENDIAN__) &&               \
