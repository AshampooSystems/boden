              ...
              BDN_IMPLEMENTS LayoutCoordinator::IWindowCoreExtension
        {
          public:
            WindowCore(Window *pOuterWindow)
                : ViewCore(pOuterWindow,
                           "div") // a "window" is simply a top level div
            {
                setTitle(pOuterWindow->title());

                // the window div always has the same size as the browser
                // window.
                _domObject["style"].set("width", "100%");
                _domObject["style"].set("height", "100%");

                emscripten_set_resize_callback("#window",
                                               static_cast<void *>(this), false,
                                               &WindowCore::_resizedCallback);

                // update the outer view's bounds with the current bounds from
                // this window. Do this in an async call, because the current
                // callstack is unknown and the property changes might otherwise
                // have unexpected side effects.
                P<WindowCore> pThis = this;
                asyncCallFromMainThread(
                    [pThis]() { pThis->updateOuterViewBounds(); });
            }
            ...
            {
                // we cannot modify our size and position. Do nothing and return
                // our current bounds.
                return _getBounds();
            }
            ...
            Size calcPreferredSize(const Size &availableSpace) const override
            {
                ...
                if (pWindow != nullptr)
                    return defaultWindowCalcPreferredSizeImpl(
                        pWindow, availableSpace, Margin(), Size(0, 0));
                else
                    return Size(0, 0);
            }
            ...
            void layout() override
            {
                P<Window> pWindow = cast<Window>(getOuterViewIfStillAttached());
                if (pWindow != nullptr) {
                    defaultWindowLayoutImpl(pWindow,
                                            Rect(Point(), pWindow->size()));
                }
            }
          ...
          private:
            ...
            Rect _getBounds() const
            {
                ...
                // we do not know the actual position of our window on the
                // screen. The web browser does not expose that.
            ...
            static EM_BOOL _resizedCallback(int eventType,
                                            const EmscriptenUiEvent *pEvent,
                                            void *pUserData)
            {
                return static_cast<WindowCore *>(pUserData)->resized(eventType,
                                                                     pEvent);
            }
        };
    }
