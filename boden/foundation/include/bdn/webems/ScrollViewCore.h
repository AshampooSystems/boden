        ...
        class ScrollViewCore : public ViewCore, BDN_IMPLEMENTS IScrollViewCore
        {
          public:
                  ...
                  _scrolledAreaDomObject(emscripten::val::null())
            {
                ...
                // we need a separate "scrolled area" inside the scroll view so
                // that we can force the scrolled area to have a certain size
                // and so that we can potentially clip the content view when
                // needed. Note that we cannot enable scrolling separately for
                // vertical and horizontal because the overflow-x and overflow-y
                // properties are relatively new (CSS 3) and, at the time of
                // this writing, buggy. In Chrome setting overflow-x: scroll
                // will cause the div to scroll in BOTH directions (even if
                // overflow-y is set to hidden). So because of that we enable
                // scrolling for both directions and then use the scrolled area
                // element inside to clip the content if we do not want to
                // scroll in one direction (or in both directions).

                emscripten::val docVal = emscripten::val::global("document");
                _scrolledAreaElementId =
                    "bdn_scrolledarea_" + std::to_string(IdGen::newID());
                _scrolledAreaDomObject = docVal.call<emscripten::val>(
                    "createElement", std::string("div"));

                _scrolledAreaDomObject.set("id",
                                           _scrolledAreaElementId.asUtf8());

                // we always use absolute positioning. Set this here.
                emscripten::val scrolledAreaStyleObj =
                    _scrolledAreaDomObject["style"];
                ...
                // it is very important that the scrolled area has "overflow
                // hidden". Otherwise the content of the scrolled area can
                // overflow the size we have set for it and thus it can happen
                // that scroll bars appear when we do not want them to appear
                // (e.g. if scrolling is disabled).
                scrolledAreaStyleObj.set("overflow", "hidden");

                _domObject.call<void>("appendChild", _scrolledAreaDomObject);

                emscripten_set_scroll_callback(_elementId.asUtf8Ptr(), this,
                                               false, _scrolledCallback);

                // the ViewCore constructor will have set the padding. Clear
                // that again (see setPadding for more info)
                setDefaultPadding();
            }

            void setHorizontalScrollingEnabled(const bool &enabled) override
            {
                // nothing to do - we fetch this directly from the outer window
                // during layout
            }
            ...
            void setPadding(const Nullable<UiMargin> &padding) override
            {
                ...
                // So we must not set it in the DOM element of the outer scroll
                // view.
                ...
                const Size &availableSpace = Size::none()) const override
            {
                ...
                if (pOuter != nullptr) {
                    ...
                    Size prefSize =
                        helper.calcPreferredSize(pOuter, availableSpace);
            ...
            void layout() override
            {
                ...
                if (pOuterView != nullptr) {
                    ...
                    // unfortunately the layout system of many browsers (chrome
                    // for example) is a little bit broken as far as scrolling
                    // is concerned. When a scrollbar is shown this reduces the
                    // viewport size, since some space is used by the scrollbar.
                    // The problem is that if scroll bars are currently visible
                    // then they only disappear when the content size becomes
                    // smaller than this reduced viewport size. Even if the
                    // content would fit completely into the bigger viewport
                    // with no scrollbars.
                    // For example: content size 100x100, scrollview size
                    // 100x100. Obviously no scrollbar is needed. If the content
                    // is enlarged to 200x200 then two scrollbars are shown. The
                    // viewport becomes smaller (let's say 90x90). When the
                    // content now goes back to 100x100 then scrollbars are
                    // still shown because the content is bigger than the
                    // CURRENT viewport of 90x90. When the content is made even
                    // smaller to 90x90 then the scrollbars disappear. At this
                    // point one can make the content bigger again to 100x100
                    // without scrollbars appearing.

                    // So, to work around this problem we ALWAYS set the
                    // scrolled area size to 0x0 at first to hide any scrollbars
                    // and then we enlarge it to the actual size.
            ...
            /** Returns the amount of space that the scroll bars take up when
               scrolling is done. If the scroll bars are overlays and do not
               influence the layout then the
                corresponding value will be set to 0.*/
            void getScrollBarLayoutSpaces(double &vertBarWidth,
                                          double &horzBarHeight) const
            {
                if (_vertScrollBarLayoutWidth == -1) {
                    P<ScrollView> pOuterView =
                        cast<ScrollView>(getOuterViewIfStillAttached());
                    if (pOuterView != nullptr) {
                        // first we must ensure that the scroll view has a
                        // certain minimum size, so that it can fully display a
                        // scroll bar. especially in the beginning the
                        // scrollview size might be (0,0) otherwise.
                        ...
                        // first we temporarily give the scrolled area a size
                        // that exceeds the viewport size to force the
                        // scrollbars to appear
                        setDomScrolledAreaSize(Size(500, 500));

                        // at this point the scroll bars are there and we can
                        // subtract the client size from the view size

                        // IMPORTANT: we cannot use getBoundingClientRect here.
                        // At the time of this writing, getBoundingClientRect
                        // does NOT exclude the scrollbar sizes. It should
                        // exclude them (according to the documentation), but it
                        // does not.

                        // We use clientWidth and clientHeight instead. Those
                        // are rounded to integer pixels, but we assume that the
                        // scroll bars have an integer size anyway.

                        double clientWidth =
                            _domObject["clientWidth"].as<double>();
                        double clientHeight =
                            _domObject["clientHeight"].as<double>();
          ...
          private:
            ...
            static EM_BOOL _scrolledCallback(int eventType,
                                             const EmscriptenUiEvent *pEvent,
                                             void *pUserData)
            {
                return ((ScrollViewCore *)pUserData)
                    ->_scrolled(eventType, pEvent);
            }
            ...
            void setDomScrolledAreaSize(const Size &size) const
            {
                emscripten::val scrolledAreaStyleObj =
                    _scrolledAreaDomObject["style"];
                scrolledAreaStyleObj.set(
                    "width",
                    std::to_string((int)std::lround(size.width)) + "px");
                scrolledAreaStyleObj.set(
                    "height",
                    std::to_string((int)std::lround(size.height)) + "px");
            }
