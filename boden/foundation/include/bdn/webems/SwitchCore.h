        ...
        class SwitchCore : public ViewCore, BDN_IMPLEMENTS ISwitchCore
        {
          public:
                  ...
                  _labelDomObject(emscripten::val::null())
            {
                ...
                _domObject["classList"].template call<void>(
                    "add", emscripten::val("bdn_switch_container"));

                // Set click callback on switch (label is not clickable)
                emscripten_set_click_callback(
                    _switchDomObject["id"].template as<std::string>().c_str(),
                    this, false, _clickedCallback);
            }
            ...
            void setOn(const bool &on) override
            {
                if (on != _on) {
                    ...
                    if (on) {
                        _switchDomObject.call<void>("setAttribute",
                                                    emscripten::val("data-on"),
                                                    emscripten::val("on"));
                        _switchDomObject["classList"].template call<void>(
                            "add", emscripten::val("bdn_on"));
                        _switchDomObject["classList"].template call<void>(
                            "remove", emscripten::val("bdn_off"));
                    } else {
                        _switchDomObject.call<void>("removeAttribute",
                                                    emscripten::val("data-on"));
                        _switchDomObject["classList"].template call<void>(
                            "add", emscripten::val("bdn_off"));
                        _switchDomObject["classList"].template call<void>(
                            "remove", emscripten::val("bdn_on"));
                    }
                }
            }
            ...
            emscripten::val &_getSwitchDomObject() { return _switchDomObject; }
          ...
          protected:
                          ...
                          const EmscriptenMouseEvent *pMouseEvent)
            {
                if (eventType == EMSCRIPTEN_EVENT_CLICK) {
                    ...
                    if (pView != nullptr) {
                        ...
                        // outer control before a notification is posted to
                        // onClick.
            ...
            void createSwitchCompositeInner(Switch *pOuter)
            {
                ...
                _switchDomObject.call<void>("appendChild",
                                            _switchHandleDomObject);

                // Set appropriate CSS classes
                _switchDomObject["classList"].template call<void>(
                    "add", emscripten::val("bdn_switch"));
                _labelDomObject["classList"].template call<void>(
                    "add", emscripten::val("bdn_switch_label"));
                _switchHandleDomObject["classList"].template call<void>(
                    "add", emscripten::val("bdn_switch_handle"));
