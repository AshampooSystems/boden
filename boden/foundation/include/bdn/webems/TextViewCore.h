        ...
        class TextViewCore : public ViewCore, BDN_IMPLEMENTS ITextViewCore
        {
          public:
                ...
                const Size &availableSpace = Size::none()) const override
            {
                ...
                // we want to use the preferred size hint as the maximum, so
                // that we wrap correctly.
                P<const View> pView = getOuterViewIfStillAttached();
                if (pView != nullptr) {
                    Size hint = pView->preferredSizeHint();

                    // ignore the hint height. We cannot adjust our width based
                    // on the desired height, only the other way round.
                ...
                Size prefSize =
                    ViewCore::calcPreferredSize(availableSpaceToUse);
