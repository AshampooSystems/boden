...
namespace bdn
{
    namespace winuwp
    {
        ...
        class ContainerViewCore : public ChildViewCore,
                                  ...
                                  BDN_IMPLEMENTS IViewCoreParent
        {
          ...
          public:
            ContainerViewCore(ContainerView *pOuter)
                : ChildViewCore(pOuter, ref new UwpViewWithLayoutDelegate<>(),
                                ref new ViewCoreEventForwarder(this))
            {
                BDN_WINUWP_TO_STDEXC_BEGIN;

                // note that
                _pUwpView = dynamic_cast<UwpViewWithLayoutDelegate<> ^>(
                    getFrameworkElement());

                // it is ok to set this as the delegate, since the panel only
                // holds a weak reference to the delegate. I.e. there will not
            ...
            {
                // the outer class automatically handles our own padding. So
                // nothing to do here.
            }
            ...
            Size uwpMeasureOverride(const Size &availableSpace) override
            {
                ...
                // "ContainerViewCore.measureOverride("+std::to_string(availableSpace.width)+",
                ...
                // OutputDebugString( String(
                // "/ContainerViewCore.measureOverride ->
            ...
            void
            finalizeUwpMeasure(const Size &lastMeasureAvailableSpace) override
            {
                P<View> pOuterView = getOuterViewIfStillAttached();

                if (pOuterView != nullptr)
                    defaultFinalizeUwpMeasure(pOuterView,
