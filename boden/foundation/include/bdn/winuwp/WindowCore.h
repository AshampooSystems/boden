                           ...
                           BDN_IMPLEMENTS IUwpViewCore
        {
          public:
            WindowCore(UiProvider *pUiProvider, Window *pOuterWindow)
            {
                ...
                // In UWP there is no normal "top level window" in the classical
                // sense. An UWP app has one or more "application views". While
                // these views look like a normal window to the user, they are
                // actually something different. Each application view has its
                // own UI thread and runs independent of other application
                // views. So the problem with multiple UI threads alone means
                // that we cannot simply map application views directly to
                // window objects. Instead it is probably best to consider an
                // application view as a kind of "screen" instead and create our
                // window objects as child panels of the application view.

                // For the time being, we also do not support multiple
                // application views. This concept only exists on Windows and
                // other platforms do not have a similar construct. So it does
                // not make much sense to support it right now.

                // So we use one application view with a child panel for each of
                // our window objects.

                _pAppView = Windows::UI::ViewManagement::ApplicationView::
                    GetForCurrentView();
                _appViewId = _pAppView->Id;

                _pXamlWindow = ::Windows::UI::Xaml::Window::Current;

                // create a sub-panel inside the xaml window. The sub-panel is
                // what we actually consider our "window".

                _pEventForwarder = ref new EventForwarder(this);

                // we want full control over the placement of our "window"
                // panel. Unfortunately Xaml automatically arranges the content
                // view of the top level window. To work around that we add a
                // canvas as the content view and then add the real content view
                // to the canvas.
                _pSharedTopContainer =
                    (UwpViewWithLayoutDelegate<> ^) _pXamlWindow->Content;
                if (_pSharedTopContainer == nullptr) {
                    _pSharedTopContainer =
                        ref new UwpViewWithLayoutDelegate<>();

                    _pSharedTopContainerLayoutDelegate =
                        newObj<TopContainerLayoutDelegate>();
                    _pSharedTopContainer->setLayoutDelegateWeak(
                        _pSharedTopContainerLayoutDelegate);

                    _pSharedTopContainer->Visibility =
                        ::Windows::UI::Xaml::Visibility::Visible;
                    _pXamlWindow->Content = _pSharedTopContainer;

                    _pSharedTopContainer->LayoutUpdated +=
                        ref new ::Windows::Foundation::EventHandler<
                            ::Platform::Object ^>(
                            _pEventForwarder, &EventForwarder::layoutUpdated);
                } else {
                    // we already created the top container. Fetch the existing
                    // layout delegate.
                    _pSharedTopContainerLayoutDelegate =
                        cast<TopContainerLayoutDelegate>(
                            _pSharedTopContainer->getLayoutDelegate());
                }

                // add the window to the delegate's list
                _pSharedTopContainerLayoutDelegate->addWindowWeak(pOuterWindow);

                // now add the panel that should represent this "window". Note
                // that we need a separate panel for the window (as opposed to
                // directly adding our content view to the top level container)
                // because we need a way to represent the window's own
                // properties (like visible/hidden) without modifying the
                // content panel. So we add a panel for the window to get this
                // intermediate control. Note that the top container is shared
                // by all Window objects, so we cannot use that to represent the
                // window visibility.
                _pContentContainer = ref new UwpViewWithLayoutDelegate<>();
                _pContentContainerLayoutDelegate =
                    newObj<ContentContainerLayoutDelegate>(pOuterWindow);
                _pContentContainer->setLayoutDelegateWeak(
                    _pContentContainerLayoutDelegate);

                _pContentContainer->Visibility =
                    ::Windows::UI::Xaml::Visibility::Visible;
                _pSharedTopContainer->Children->Append(_pContentContainer);

                _pXamlWindow->SizeChanged +=
                    ref new ::Windows::UI::Xaml::WindowSizeChangedEventHandler(
                        _pEventForwarder, &EventForwarder::windowSizeChanged);

                // update the position and size property of the outer window to
                // reflect the current bounds
            ...
            ~WindowCore()
            {
                ...
                if (_pContentContainer != nullptr) {
                    ...
                    if (_pSharedTopContainer != nullptr) {
                        try {
                            unsigned index = 0;
                            if (_pSharedTopContainer->Children->IndexOf(
                                    _pContentContainer, &index))
            ...
            /** Returns the Xaml UI element that represents the window.
                Note that this is NOT a Windows.UI.Xaml.Window object. Instead
               it is a child container panel inside the actual
            ...
            */
            ...
            {
                // apparently it is not possible to set the window title
                // programmatically. Windows seems to take this from the
                // manifest. So, just ignore this.
            }
            ...
            {
                // we don't care. The outer Window object will handle the
                // layout.
            }
            ...
            {
                // our preferred size does not matter for the layout, since we
                // cannot adapt the window size anyway
            }
            ...
            {
                // See ChildViewCore::needLayout for an explanation on why we
                // ignore standard property changes.
                if (reason != View::InvalidateReason::standardPropertyChanged &&
                    ...
                    // this is.
                    UwpLayoutBridge::get().invalidateMeasure(
                        _pContentContainer);
                }
            }
            ...
            {
                // it is not possible to control the size of a UWP window from
                // inside the app. So we ignore this request.
            }

            void requestCenter() override
            {
                // it is not possible to control the position of a UWP window
                // from inside the app. So we ignore this request.
            }

            Rect adjustAndSetBounds(const Rect &bounds) override
            {
                // we cannot influence our bounds. The OS / the user has the
                // only control over it.

                // So we ignore the call and return the current bounds as the
                // "adjusted" value.
                return _getBounds();
            }
            ...
            void addChildUiElement(::Windows::UI::Xaml::UIElement ^
                                   pUiElement) override
            {
                BDN_WINUWP_TO_STDEXC_BEGIN;
            ...
            void layout()
            {
                // this is called from uwpMeasureFinalize. Just call the default
                // layout for the window.

                // OutputDebugString(
                // (String(typeid(*this).name())+".layout()\n").asWidePtr() );

                P<Window> pOuter = _outerWindowWeak.toStrong();
                if (pOuter != nullptr) {
                    Size contentSize = _getContentSize();
                    Rect contentArea(0, 0, contentSize.width,
                                     contentSize.height);
          ...
          private:
            ...
            Rect _getBounds() const
            {
                ...
                // there is no "moved" event for Xaml windows. As such, we
                // cannot find out when the window position changes. So we
                // cannot update the outer window's bounds property with the
                // position. As a result it is better to always assume zero
                // position. Otherwise we can get inconsistencies with different
                // bounds being reported at different times, even though there
                // is no change event in between
                bounds.x = 0;
                bounds.y = 0;
            ...
            double getEmSizeDips() const
            {
                if (_emSizeDipsIfInitialized == -1) {
                    // the font size of a Window cannot be changed within the
                    // UWP system. Only Controls have a font size attached to
                    // them. The default font size for controls is documented as
                    // being 11 DIPs, so we use that here as the em size.
            ...
            void _scheduleUpdateOuterPositionAndSizeProperty()
            {
                ...
                // we do this asynchronously to ensure that there can be no
                // strange interactions with in-progress operations
                ...
                asyncCallFromMainThread([pThis]() {
                    ...
                    if (pView != nullptr) {
                        // call View::adjustAndSetBounds. That will
                        // automatically call OUR adjustAndSetBounds and then
                        // update the view properties according to the returned
                        // bounds rect.
            ...
            {
                internal : EventForwarder(WindowCore *pParent)
                {
              ...
              public:
                ...
                void windowSizeChanged(
                    ...
                    ::Windows::UI::Core::WindowSizeChangedEventArgs ^ pArgs)
                {
                ...
                void layoutUpdated(Platform::Object ^ pSender,
                                   ::Platform::Object ^ pArgs)
                {
                    BDN_WINUWP_TO_PLATFORMEXC_BEGIN
            ...
            /** Layout delegate for the top container. Simply sizes the child to
             * fill the entire space.*/
            class TopContainerLayoutDelegate : public Base,
                                               BDN_IMPLEMENTS IUwpLayoutDelegate
            {
              public:
                ...
                {
                  public:
                    AutoCleaningWindowIterator(
                        std::list<WeakP<Window>> &weakList)
                        : _weakList(weakList)
                    {
                ...
                Size uwpMeasureOverride(const Size &availableSpace) override
                {
                    ...
                    // "Window.TopContainerLayoutDelegate.measureOverride("+std::to_string(availableSpace.width)+",
                    ...
                    while (it.next(pWindow)) {
                        P<WindowCore> pCore =
                            tryCast<WindowCore>(pWindow->getViewCore());
                        if (pCore != nullptr) {
                            if (pCore->_pContentContainer != nullptr)
                                pCore->_pContentContainer->Measure(
                                    sizeToUwpSize(availableSpace));
                        }
                    }

                    // OutputDebugString( String(
                    // "/Window.TopContainerLayoutDelegate.measureOverride\n"
                    // ).asWidePtr() );

                    // IMPORTANT:
                    // In contrast to the documentation, UI elements cannot be
                    // made smaller than their DesiredSize (see
                    // doc_input/winuwp_layout.md for more information). Their
                    // Arrange method will always enlarge the size we pass to it
                    // and make the panel at least as big as the desired size.
                    // Since we absolutely do not want that we always return a
                    // desired size of 0. That does not interfere with our own
                    // layout, since the Window will always make this panel the
                    // same size as itself.
                    return Size(0, 0);
                }

                void finalizeUwpMeasure(
                    const Size &lastMeasureAvailableSpace) override
                {
                    // OutputDebugString( String(
                    // "Window.ContentContainerLayoutDelegate.finalizeUwpMeasure("+std::to_string(lastMeasureAvailableSpace.width)+",
                    // "+std::to_string(lastMeasureAvailableSpace.height)+")\n"
                    // ).asWidePtr() );

                    AutoCleaningWindowIterator it(_windowWeakList);
                    P<Window> pWindow;
                    while (it.next(pWindow))
                        defaultFinalizeUwpMeasure(pWindow,
                                                  lastMeasureAvailableSpace);
                }

                Size uwpArrangeOverride(const Size &finalSize) override
                {
                    // OutputDebugString( String(
                    // "Window.TopContainerLayoutDelegate.arrangeOverride("+std::to_string(finalSize.width)+",
                    // "+std::to_string(finalSize.height)+"\n" ).asWidePtr() );
                    ...
                    while (it.next(pWindow)) {
                        ...
                        if (pCore != nullptr) {
                            ...
                            pCore->_pContentContainer->Arrange(
                                rectToUwpRect(contentContainerRect));
                        }
                    }

                    // OutputDebugString( String(
                    // "/Window.TopContainerLayoutDelegate.arrangeOverride()\n"
                    // ).asWidePtr() );
            ...
            class ContentContainerLayoutDelegate
                : public Base,
                  BDN_IMPLEMENTS IUwpLayoutDelegate
            {
              public:
                ...
                {
                    // OutputDebugString( String(
                    // "Window.ContentContainerLayoutDelegate.measureOverride("+std::to_string(availableSpace.width)+",
                    // "+std::to_string(availableSpace.height)+")\n"
                    // ).asWidePtr() );

                    // OutputDebugString( String(
                    // "/Window.ContentContainerLayoutDelegate.measureOverride\n"
                    // ).asWidePtr() );
                    ...
                    const Size &lastMeasureAvailableSpace) override
                {
                    ...
                    // OutputDebugString( String(
                    // "/Window.ContentContainerLayoutDelegate.finalizeUwpMeasure\n"
                    // ).asWidePtr() );
                }
                ...
                {
                    // OutputDebugString( String(
                    // "Window.ContentContainerLayoutDelegate.arrangeOverride("+std::to_string(finalSize.width)+",
                    // "+std::to_string(finalSize.height)+"\n" ).asWidePtr() );

                    P<Window> pWindow = _windowWeak.toStrong();

                    if (pWindow != nullptr) {
                        // arrange the content view. The default implementation
                        // for uwpArrangeOverride does that.
                        defaultArrangeOverride(pWindow, finalSize);
                    }

                    // OutputDebugString( String(
                    // "/Window.ContentContainerLayoutDelegate()\n"
                    // ).asWidePtr() );
            ...
            void _windowSizeChanged()
            {
                ...
                try {
                    ...
                    if (pOuterView != nullptr) {
                        ...
                        // Update our window panel to the same size as the outer
                        window. if(_pSharedTopContainer->Width != width
                        ...
                        // Update our window panel to the same size as the outer
                        window. if(_pContentContainer->Width != width
                        ...
                        // We do that by calling View::adjustAndSetBounds. That
                        // will call our adjustAndSetBounds, which will do
                        // nothing and only return the current bounnds. And then
                        // the view will store that in its properties, which is
                        // what we want.
            ...
            void _layoutUpdated() { UwpLayoutBridge::get().layoutUpdated(); }
