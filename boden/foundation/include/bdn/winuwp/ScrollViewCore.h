...
namespace bdn
{
    namespace winuwp
    {
        ...
        {
          private:
            static ::Windows::UI::Xaml::Controls::ScrollViewer ^
                _createScrollViewer(ScrollView *pOuter) {
                    BDN_WINUWP_TO_STDEXC_BEGIN;

                    ::Windows::UI::Xaml::Controls::ScrollViewer ^
                        pScrollViewer =
                        ref new ::Windows::UI::Xaml::Controls::ScrollViewer();
                ...
                : ChildViewCore(pOuter, _createScrollViewer(pOuter),
                                ref new ScrollViewCoreEventForwarder(this))
            {
                BDN_WINUWP_TO_STDEXC_BEGIN;

                _pScrollViewer =
                    dynamic_cast<::Windows::UI::Xaml::Controls::ScrollViewer ^>(
                        getFrameworkElement());

                _pScrollViewer->HorizontalScrollBarVisibility =
                    ::Windows::UI::Xaml::Controls::ScrollBarVisibility::Auto;
                _pScrollViewer->VerticalScrollBarVisibility =
                    ::Windows::UI::Xaml::Controls::ScrollBarVisibility::Auto;

                // we want to let the scroll viewer handle the scroll bars and
                // layouting automatically. It is compatible with our own layout
                // system. However, we still want to handle the layout ourselves
                // within the viewport (padding, margin of the content view). To
                // achieve that we have to give the scroll viewer has an UWP
                // content panel that wraps the actual content view and forwards
                // layout and measure calls accordingly.
                _pContentWrapper = ref new UwpViewWithLayoutDelegate<>();
                _pContentWrapperLayoutDelegate =
                    newObj<ContentWrapperLayoutDelegate>(pOuter);
                _pContentWrapper->setLayoutDelegateWeak(
                    _pContentWrapperLayoutDelegate);

                _pScrollViewer->Content = _pContentWrapper;

                _pScrollViewer->ViewChanged +=
                    ref new ::Windows::Foundation::EventHandler<
                        ::Windows::UI::Xaml::Controls::
                            ScrollViewerViewChangedEventArgs ^>(
                        dynamic_cast<ScrollViewCoreEventForwarder ^>(
            ...
            {
                // we do not pass the padding on to the scroll view. We control
                // it manually when we lay out the content wrapper.
            ...
            {
                _pScrollViewer->HorizontalScrollMode =
                    enabled
                        ? ::Windows::UI::Xaml::Controls::ScrollMode::Enabled
                        : ::Windows::UI::Xaml::Controls::ScrollMode::Disabled;
            }

            void setVerticalScrollingEnabled(const bool &enabled) override
            {
                _pScrollViewer->VerticalScrollMode =
                    enabled
                        ? ::Windows::UI::Xaml::Controls::ScrollMode::Enabled
                        : ::Windows::UI::Xaml::Controls::ScrollMode::Disabled;
            }
            ...
            void scrollClientRectToVisible(const Rect &clientRect) override
            {
                ...
                _pScrollViewer->ChangeView(visibleRect.x, visibleRect.y,
                                           _pScrollViewer->ZoomFactor);
            }

            void layout()
            {
                // Windows takes care of the layout of the scroll viewer. But we
                // still need to lay out the content view inside of our content
                // wrapper.

                // layout is called at the end of the measure phase. I.e. at a
                // point in time when the content wrapper does not have its
                // final size yet. BUT the viewport size (i.e. the size of the
                // scrollview) is known, since View::size() is already
                // up-to-date. So we simply use the ScrolViewLayoutHelper here.
            ...
            void needLayout(View::InvalidateReason reason) override
            {
                ...
                if (reason != View::InvalidateReason::standardPropertyChanged &&
                    ...
                    // The default implementation (called above) invalidates the
            ...
            {
                // Note that we cannot call ChildViewCore::calcPreferredSize
                // here to get our desired size, since that uses Measure. And
                // for us Measure will always return 0. That is by design, since
                // Windows does not allow controls to be sized smaller than
                // their DesiredSize. So to allow us to have full control over
                // the size we force the DesiredSize to be zero.

                // So we have to calculate our preferred size ourselves by
                // looking at the content view. We can use the layout helper for
                // that.

                // note that we still have to call Measure on the control to
                // ensure that DesiredSize is initialized and to ensure that it
                // participates in the layout (Windows only includes controls
                // that have Measure called on them). And we have to do that
                // first, since the scroll viewer's measure method calls Measure
                // on the content view and will thus initialize the content
                // view's desired size. And we have to make sure that we
                // overwrite that afterwards.
                ::Windows::Foundation::Size winAvailSpace =
                    sizeToUwpSize(availableSpace);

                _pScrollViewer->Measure(winAvailSpace);

                // calling Measure on the scroll viewer will automatically call
                // measure on the content wrapper.
                ...
                Size prefSize;
                if (pOuter != nullptr) {
                    // scroll bars are overlays that only appear when they are
                    // used. So they do not take up any space.
                    ScrollViewLayoutHelper layoutHelper(0, 0);

                    prefSize =
                        layoutHelper.calcPreferredSize(pOuter, availableSpace);
                }
          ...
          protected:
            virtual bool canAdjustWidthToAvailableSpace() const { return true; }
            ...
            void _uwpSizeChanged() override
            {
                ...
                // the scroll viewer's size has changed. Ensure that we update
                // the DesiredSize that the content reports, because the desired
                // size may depend on the size and/or shape of the viewport.
          ...
          private:
            ref class ScrollViewCoreEventForwarder
                : public ViewCoreEventForwarder
            {
                internal : ScrollViewCoreEventForwarder(ChildViewCore *pParent)
                    : ViewCoreEventForwarder(pParent)
                {}

              public:
                void viewChanged(Platform::Object ^ pSender,
                                 ::Windows::UI::Xaml::Controls::
                                         ScrollViewerViewChangedEventArgs ^
                                     pArgs)
                {
                    ...
                    P<ScrollViewCore> pViewCore =
                        cast<ScrollViewCore>(getViewCoreIfAlive());
            ...
            class ContentWrapperLayoutDelegate
                : public Base,
                  BDN_IMPLEMENTS IUwpLayoutDelegate
            {
              public:
                ...
                Size uwpMeasureOverride(const Size &availableSpace) override
                {
                    ...
                    // "ContentWrapperLayoutDelegate.measureOverride("+std::to_string(availableSpace.width)+",
                    ...
                    Size resultSize;
                    if (pView != nullptr) {
                        // the layout of the inner content view is done in our
                        // own layout() method. We cannot do the same here
                        // dynamically because availableSpace does not reflect
                        // the size of the parent view - instead it is always
                        // infinity. So we cannot make a proper sizing decision
                        // for the content view here - not without some ugly
                        // hacks (like using the parent's actual size).

                        // So, instead of actually measuring the content view
                        // here, we simply assume that OUR the content View
                        // object already has the correct size that we want and
                        // we simply report that size as our desired size.

                        P<View> pContentView = pView->getContentView();
                        if (pContentView != nullptr) {
                            Margin contentMargin =
                                pContentView->uiMarginToDipMargin(
                                    pContentView->margin());
                            ...
                            resultSize.width +=
                                pos.x + padding.right + contentMargin.right;
                            resultSize.height +=
                                pos.y + padding.bottom + contentMargin.bottom;
                        }
                    }

                    // OutputDebugString( String(
                    // "ContentWrapperLayoutDelegate.measureOverride result:
                    // "+std::to_string(resultSize.width)+",
                    // "+std::to_string(resultSize.height)+"\n" ).asWidePtr() );
                    ...
                    const Size &lastMeasureAvailableSpace) override
                {
                    ...
                    // the content wrapper will automatically sized by the
                    // ScrollViewer, so we do not need to adjust its own size.
                    // But we do need to size the content view and update its
                    // layout.
                ...
                {
                    // OutputDebugString( String(
                    // "ContentWrapperLayoutDelegate.arrangeOverride("+std::to_string(finalSize.width)+",
                    // "+std::to_string(finalSize.height)+"\n" ).asWidePtr() );

                    P<ScrollView> pView = _viewWeak.toStrong();

                    // the content bounds we want have already been set in the
                    // corresponding View object. So all we need to do here is
                    // update the UWP control accordingly.

                    if (pView != nullptr) {
                        // we simply need to arrange the content view to the
                        // rect we calculated earlier.
                        P<View> pContentView = pView->getContentView();
                        if (pContentView != nullptr) {
                            P<ChildViewCore> pContentCore =
                                tryCast<ChildViewCore>(
                                    pContentView->getViewCore());
                            if (pContentCore != nullptr) {
                                ::Windows::UI::Xaml::FrameworkElement ^
                                    pContentElement =
                                    pContentCore->getFrameworkElement();
                                if (pContentElement != nullptr) {
                                    Rect contentBounds(pContentView->position(),
                                                       pContentView->size());

                                    // OutputDebugString( String( "ScrollView
                                    // content view
                                    // "+String(pContentElement->GetType()->ToString()->Data())+"
                                    // arrange:
                                    // "+std::to_string(contentBounds.x)+",
                                    // "+std::to_string(contentBounds.y)+",
                                    // "+std::to_string(contentBounds.width)+",
                                    // "+std::to_string(contentBounds.height)+"\n"
                                    // ).asWidePtr() );

                                    ::Windows::Foundation::Rect
                                        winContentBounds =
                                            rectToUwpRect(contentBounds);
                                    ...
                                    double height =
                                        pContentElement->ActualHeight;

                                    // OutputDebugString( String( "Resulting
                        ...
                        P<ScrollViewCore> pCore =
                            tryCast<ScrollViewCore>(pView->getViewCore());
                        if (pCore != nullptr) {
                    ...
                    // OutputDebugString( String(
                    // "/ContentWrapperLayoutDelegate.arrangeOverride()\n"
                    // ).asWidePtr() );
