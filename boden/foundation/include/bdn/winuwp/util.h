...
namespace bdn
{
    namespace winuwp
    {
        ...
        */
        ...
        {
            if (std::isfinite(val)) {
                if (val < 0) // UWP cannot represent negative dimension. It will
                             // yield an assertion
                    return 0;
                else
        ...
        inline Rect uwpRectToRect(const ::Windows::Foundation::Rect &rect)
        {
            ...
            return Rect(rect.X, rect.Y, uwpDimensionToDouble(rect.Width),
                        uwpDimensionToDouble(rect.Height));
        }
        ...
        /** Converts a UWP size to a Boden size. Infinity and nan values will
           pass pass through unchanged (they will remain infinity/nan in the
           output size)
            */
