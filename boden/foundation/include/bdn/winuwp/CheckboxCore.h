...
template <class T>
class CheckboxCore : public ChildViewCore, BDN_IMPLEMENTS ICheckboxCore, BDN_IMPLEMENTS ISwitchCore
{
private:
	static ::Windows::UI::Xaml::Controls::StackPanel^ _createCheckbox(T* pOuter)
	{
		...
		::Windows::UI::Xaml::Controls::CheckBox^ checkbox = ref new ::Windows::UI::Xaml::Controls::CheckBox();
		checkbox->MinWidth = 0;
...
public:
	...
	/** Used internally.*/
	ref class CheckboxCoreEventForwarder : public ViewCoreEventForwarder
	{
	internal:
		CheckboxCoreEventForwarder(CheckboxCore* pParent)
	...
	CheckboxCore(T* pOuter)
		: ChildViewCore(pOuter, _createCheckbox(pOuter), ref new CheckboxCoreEventForwarder(this) )
	{
        BDN_WINUWP_TO_STDEXC_BEGIN;

        _pContainer = dynamic_cast< ::Windows::UI::Xaml::Controls::StackPanel^ >( getFrameworkElement() );
		_pCheckbox = dynamic_cast< ::Windows::UI::Xaml::Controls::CheckBox^ >( _pContainer->Children->GetAt(0) );

		CheckboxCoreEventForwarder^ pEventForwarder = dynamic_cast<CheckboxCoreEventForwarder^>( getViewCoreEventForwarder() );
		_pCheckbox->Click += ref new ::Windows::UI::Xaml::RoutedEventHandler( pEventForwarder, &CheckboxCoreEventForwarder::clicked );
	...
	void setState(const TriState& state)
	{
        ...
        switch (state) {
        case TriState::on:    _pCheckbox->IsChecked = true;    break;
        case TriState::off:   _pCheckbox->IsChecked = false;   break;
        case TriState::mixed: _pCheckbox->IsChecked = nullptr; break;
        }
	...
	void _clicked()
	{
        ...
        if (pOuterView != nullptr) {
        	...
        	if (pCheckbox)
        	{
				...
				// Prohibit users from selecting mixed state by clicking the checkbox.
				// MS themselves say this is a non-use case, but then still allow users
				// to select mixed state by default and make it quite hard to prevent:
				// https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.controls.checkbox
				// ¯\_(ツ)_/¯
	        	if (_pCheckbox->IsChecked != nullptr && _pCheckbox->IsChecked->Value == true) {
	        		state = TriState::on;
					_pCheckbox->IsThreeState = false;
	        	} else if (_pCheckbox->IsChecked != nullptr && _pCheckbox->IsChecked->Value == false) {
	        		state = TriState::off;
