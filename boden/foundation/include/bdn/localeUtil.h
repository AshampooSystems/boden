...
namespace bdn
{
    ...
    /** Returns true if the specified locale uses Utf-8 as its multibyte
     * encoding.*/
...
// as codecvt parameters. The standard library only provides an implementation
// for the codecvt static elements with unsigned int (instead of wchar_t) and
// unsigned short (instead of char16_t). The error occurred because char16_t
// used to be a typedef in older versions but is now a distinct type - and the
// standard library was apparently not updated when this changed. See
// https://connect.microsoft.com/VisualStudio/Feedback/Details/1403302. So to
// work around this problem we have to use unsigned int / short instead of
...
/** A typedef to the std::codecvt implementation from the C++ standard library
    ...
    compiler or standard library problems make it necessary to use a different
   type.

    For example, certain versions of Microsoft Visual Studio (namely VS2015 and
   VS2017) do not provide std::codecvt_utf8_utf16<char16_t>.  Instead they only
...
/** A typedef to the std::codecvt implementation from the C++ standard library
   ...
   type.
    ...
    For example, certain versions of Microsoft Visual Studio (namely VS2015 and
   VS2017) do not provide std::codecvt_utf8<char16_t>.  Instead they only
...
/** A typedef to the std::codecvt implementation from the C++ standard library
   ...
   type.
    ...
    For example, certain versions of Microsoft Visual Studio (namely VS2015 and
   VS2017) do not provide std::codecvt_utf8<char32_t>.  Instead they only
    ...
    */
    ...
    /** A typedef to the std::codecvt implementation from the C++ standard
       library that converts between UTF-8 and UCS-4.

        This is usually an alias for CodecVtUtf8Utf32 because UTF-32 and UCS-4
       are the same.
        */
    typedef CodecVtUtf8Utf32 CodecVtUtf8Ucs4;

/** A typedef to the std::codecvt base type that converts between char and
   char16_t.

    This is usually an alias to std::codecvt<char16_t, char, mbstate_t>, except
   if compiler or standard library problems make it necessary to use a different
   type.

    For example, certain versions of Microsoft Visual Studio (namely VS2015 and
   VS2017) do not provide std::codecvt<char16_t, char, mbstate_t>.  Instead they
...
/** A typedef to the std::codecvt base type that converts between char and
   char32_t.
    ...
    This is usually an alias to std::codecvt<char32_t, char, mbstate_t>, except
   if compiler or standard library problems make it necessary to use a different
   type.
