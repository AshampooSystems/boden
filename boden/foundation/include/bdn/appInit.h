...
#define BDN_APP_INIT_WITH_CONTROLLER_CREATOR(appControllerCreator)             \
    ...
    {                                                                          \
        return bdn::win32::commandLineAppEntry(appControllerCreator, argc,     \
                                               argv);                          \
    }
...
#define BDN_APP_INIT_WITH_CONTROLLER_CREATOR(appControllerCreator)             \
    int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,         \
                         LPSTR lpCmdLine, int showCommand)                     \
    {                                                                          \
...
static_assert(
    false,
    "Unknown target platform - don't know how to define app entry point.");
...
/** \def BDN_APP_INIT( appControllerClass )
    ...
    Note that for commandline apps or high performance apps the app controller
   can also implement a "main loop" that performs the actual work of the app.
...
/** \def BDN_TEST_APP_INIT()
    ...
    Sets up a Boden test framework unittest app that runs the test cases defined
   in the app.

    This replaces the normal BDN_APP_INIT() statement that you use for normal
   apps.
