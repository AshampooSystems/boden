          ...
          public:
            ...
            ThreadLocalStorageManager()
            {
                ...
                int result = pthread_key_create(
                    &_key, &ThreadLocalStorageManager::_threadExitCleanup);
                if (result != 0) {
                    throw errnoCodeToSystemError(
                        result,
                        ErrorFields()
                            .add("func", "pthread_key_create")
                            .add("action",
                                 "bdn::pthread::ThreadLocalStorageManager "
                                 "constructor"));
                }
            }

            ~ThreadLocalStorageManager() { pthread_key_delete(_key); }
