    ...
    template <typename CharType> class AsyncStdioWriter : public Base
    {
      public:
        ...
        P<IAsyncOp<void>> write(const String &s)
        {
            ...
            // However, on the only threadless platform that we support at the
            // time of this writing (=webems) the operation should never take
            // long. Actual I/O operations cannot be done synchronously there,
            // so the stream implementation cannot do "real" I/O. It must be a
            // "fake" file in the simulated webems file system or a RAM
            // operation. And all of these should be fast enough.
