                         ...
                         BDN_IMPLEMENTS LayoutCoordinator::IViewCoreExtension
        {
          public:
            ...
            Rect adjustAndSetBounds(const Rect &requestedBounds) override
            {
                ...
                gtk_widget_set_size_request(_pWidget, alloc.width,
                                            alloc.height);
                ...
                if (pView != nullptr) {
                    ...
                    if (pParentView != nullptr) {
                        P<ViewCore> pParentViewCore =
                            cast<ViewCore>(pParentView->getViewCore());

                        pParentViewCore->_moveChildViewCore(this, alloc.x,
                                                            alloc.y);
                    }
                }
                ...
                {
                    BDN_DEBUGGER_PRINT( String(typeid(*this).name()) + " size:
            ...
            {
                // GTK uses DIPs, but with inteher values. So while there can be
                // more than 1 pixel per DIP, we cannot address them. So we have
                // to adjust to DIP boundaries, rather than pixel boundaries.
                // Note that the GTK scale factor (factor from DIPs to pixels)
                // is always an integer, so DIP boundaries are always also pixel
                // boundaries. But there may be additional pixel boundaries
                ...
                const Size &availableSpace = Size::none()) const override
            {
                ...
                P<View> pOuterView = getOuterViewIfStillAttached();
                if (pOuterView != nullptr) {
                    // preferredSizeMaximum is a hard limit, So we clip the
                    // available space to this, so that the control can adjust
                    // accordingly.
                    effectiveAvailableSpace.applyMaximum(
                        pOuterView->preferredSizeMaximum());
                }

                GtkRequisition resultSize;

                // std::cout << typeid(*this).name() << " available: " <<
                // availableWidth << "x" << availableHeight << std::endl;

                // we must clear our "size request". Otherwise the current size
                // will be the basis for the preferred size.
                gint oldWidth;
                gint oldHeight;
                ...
                gtk_widget_get_preferred_size(_pWidget, &unrestrictedMinSize,
                                              &unrestrictedNaturalSize);

                // we subtract the padding from the available space because the
                // padding is not reflected in the GTK widget by default. So we
                // make the available space smaller and ask the widget how big
                // it wants to be inside it. Then we add the padding on top of
                // the desired size.

                if (std::isfinite(effectiveAvailableSpace.width) &&
                    canAdjustWidthToAvailableSpace()) {
                    double availableWidth = effectiveAvailableSpace.width -
                    ...
                    if (availableWidth >= unrestrictedNaturalSize.width) {
                        // the unrestructed width  is smaller or equal to the
                        // available width. So we can just use that.
                        ...
                        gtk_widget_get_preferred_height_for_width(
                            _pWidget, forGtkWidth, &minHeight, &naturalHeight);

                        resultSize.height = naturalHeight;

                        // now we know our height for the case in which the
                        // width is exactly availableWidth. Note that we also
                        // know that availableWidth is smaller than the natural
                        // size (see check above). So we know that the control
                        // pretty much uses all the available space here. Note
                        // that it might seem like a good idea to use
                        // gtk_widget_get_preferred_width_for_height here to
                        // find out if the widget really does use all the
                        // available space in the restricted case. For example,
                        // text views that wrap their text to accomodate for
                        // availWidth will usually not wrap exactly at
                        // availWidth, but slightly before that. However, we
                        // cannot use gtk_widget_get_preferred_width_for_height
                        // to find out how much space the view really needs.
                        // Since gtk_widget_get_preferred_width_for_height does
                        // not have a paremeter to specify a width limit, the
                        // natural width we get will be exactly the unrestricted
                        // width (since the height limit we specify is usually
                        // actually bigger than the unrestricted height). And
                        // the minimum width will be a width for a case in which
                        // the control tries to absolutely minimize its width.
                        // For example, text views will wrap text as much as
                        // possible and also try to choose the wrap point so
                        // that all lines are roughly the same width. That is
                        // not the same text wrapping that the control used to
                        // calculate the naturalHeight for the case with the
                        // restricted width.
                        resultSize.width = availableWidth;
                    }
                } else if (std::isfinite(effectiveAvailableSpace.height) &&
                           canAdjustHeightToAvailableSpace()) {
                    double availableHeight = effectiveAvailableSpace.height -
                    ...
                    if (availableHeight >= unrestrictedNaturalSize.width) {
                        // the unrestructed height is smaller or equal to the
                        // available height. So we can just use that.
                        resultSize = unrestrictedNaturalSize;
                    } else {
                        gint minWidth = 0;
                        gint naturalWidth = 0;
                        ...
                        // see availableWidth!=-1 case for an explanation of
                        // what we do here.

                        gtk_widget_get_preferred_width_for_height(
                            _pWidget, forGtkHeight, &minWidth, &naturalWidth);
                ...
                // std::cout << typeid(*this).name() << " preferred: " <<
                // size.width << "x" << size.height << " padding: " <<
                // padding.top << "," << padding.right <<","<< padding.bottom
                // <<","<< padding.left << std::endl;
            ...
            GtkWidget *getGtkWidget() const { return _pWidget; }
            ...
            virtual void _addChildViewCore(ViewCore *pChildCore)
            {
                throw ProgrammingError(
                    "ViewCore::_addChildViewCore called on view that does not "
                    "support child views.");
            }

            virtual void _moveChildViewCore(ViewCore *pChildCore, int gtkX,
                                            int gtkY)
            {
                throw ProgrammingError(
                    "ViewCore::_moveChildViewCore called on view that does not "
                    "support child views.");
            }
          ...
          protected:
            Margin _getPaddingIntegerDips() const
            {
                ...
                // round up to full integers, since GTK cannot represent
                // anything else
            ...
            void _addToParent()
            {
                ...
                if (pParent == nullptr) {
                    ...
                    if (pParentCore == nullptr) {
                        // should never happen
                        throw ProgrammingError(
                            "Error: constructed a gtk::ViewCore for a view "
                            "whose parent does not have a core.");
                    }
                ...
                if (_emDipsIfInitialized == -1) {
                    ...
                    if (_pWidget != nullptr) {
                        GtkStyleContext *pStyleContext =
                            gtk_widget_get_style_context(_pWidget);
                        if (pStyleContext != nullptr) {
                            ...
                            if (pFontDesc != nullptr) {
                                ...
                                if (size > 0) {
                                    ...
                                    // might be in points or pixels (what they
                                    // call "absolute")
                    ...
                    if (size <= 0) {
                        // size not set explicitly => system font size is used
                        // => em same as sem
