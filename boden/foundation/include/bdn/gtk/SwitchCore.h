        ...
        class SwitchCore : public ViewCore, BDN_IMPLEMENTS ISwitchCore
        {
          public:
                ...
                : ViewCore(pOuter, gtk_layout_new(nullptr, nullptr))
            {
                ...
                g_signal_connect(_switch, "notify::active",
            ...
            void notifyOn()
            {
                ...
                if (pView != nullptr) {
                    cast<T>(pView)->setOn(
                        gtk_switch_get_active(GTK_SWITCH(_switch)));
                }
            }
                ...
                const Size &availableSpace = Size::none()) const override
            {
                ...
                float margin;
                if (String(gtk_label_get_text(GTK_LABEL(_label))).length() >
                    0) {
                    margin = 10.;
                } else {
                    margin = 0.;
                    labelSize.width =
                        0.; // GTK reports width of label as 1. even if empty
                }

                Size result = Size(switchSize.width + labelSize.width + margin,
            ...
            void layout() override
            {
                ...
                gtk_layout_move(GTK_LAYOUT(getGtkWidget()), _switch,
                                labelSize.width + margin, 0);
                gtk_layout_move(GTK_LAYOUT(getGtkWidget()), _label, 0,
                                (preferredSize.height - labelSize.height) / 2);
            }

            GtkWidget *_getLabelWidget() { return _label; }

            GtkWidget *_getSwitchWidget() { return _switch; }
          ...
          protected:
            ...
            static void toggledCallback(GObject *gobject, GParamSpec *pspec,
                                        gpointer user_data)
            {
            ...
            Size minGtkWidgetSize(GtkWidget *gtkWidget) const
            {
                ...
                gtk_widget_get_preferred_size(gtkWidget, &unrestrictedMinSize,
                                              &unrestrictedNaturalSize);
                return Size(unrestrictedMinSize.width,
                            unrestrictedMinSize.height);
            }
