...
/** Packages a method together with its object pointer and allows it to be called
    ...
    The caller is responsible for ensuring that the method's object (pObject) is still valid
    at the time the function object is called. The returned function object does nothing to keep
    the method's object alive and it also does nothing to check the validity of the object.
    If the method's object is deleted then calling the returned function object will
    likely cause a crash, so use this carefully.

    plainMethod can be used with any object of any class. The object does not need
    to implement bdn::IBase.

    When to use
    -----------

    plainMethod is a bit more lightweight than weakMethod() and strongMethod() (although the
    performance benefit matters only very rarely).
    
    It is mainly intended for cases when you are certain that the object will be valid, or when the object
    does not implement bdn::IBase.
