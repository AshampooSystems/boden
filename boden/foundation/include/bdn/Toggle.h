...
/** A simple on/off toggle adapting its appearance to the target platform
    ...
    The Toggle class implements a control providing the user with a binary on or off choice.
    On mobile platforms, it appears as a Switch whereas on desktop platforms it appears as a
    Checkbox. If the control's label is set, it appears to the left of the switch control on
    mobile platforms and to the right of the checkbox control on Desktop platforms.

    Use toggles for presenting users with a binary on/off switch which automatically conforms
    to the user interface best practices on all platforms.
 */
...
public:
    ...
    /** A notifier for click events. Subscribe to this notifier if you want to be notified about
        click events. Click events are posted when the user clicks on the toggle. If the toggle is
        displayed as a switch, the label is not clickable. If the toggle is displayed as a checkbox,
