...
namespace bdn
{
    ...
    template <class T> class RawNewAllocator_NonBase_
    {
      public:
        template <typename... Arguments> static T *alloc(Arguments &&... args)
        {
            return new T(std::forward<Arguments>(args)...);
        }
    };

    template <class T> class RawNewAllocator_Base_
    {
      public:
        template <typename... Arguments> static T *alloc(Arguments &&... args)
        {
    ...
    {
      public:
        static void doIt(T *p) { delete p; }
    };

    template <class T> class DeleteOrReleaseRef_Release_
    {
      public:
        static void doIt(T *p) { p->releaseRef(); }
    };
