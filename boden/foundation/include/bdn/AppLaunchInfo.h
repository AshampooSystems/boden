    ...
    class AppLaunchInfo : public Base
    {
      public:
        ...
        /** Returns the windows show command.
            ...
            The show command is a hint from the caller as to how the app should
           start. For example, desktop shortcuts that start apps have a
           parameter to run the app maximmized or minimized. This results in the
           show command to be the corresponing value.

            The possible values for the show command are those that can be
           passed to the Windows function ShowWindow. The default value is
           SW_SHOWNORMAL. You will get SW_SHOWMAXIMIZED or SW_SHOWMINIMIZED when
           the caller wants the app to run in maximized / minimized mode.

            The show command is rarely used and many programs ignore it.
        */
        int getWindowsShowCommand() const { return _windowsShowCommand; }
