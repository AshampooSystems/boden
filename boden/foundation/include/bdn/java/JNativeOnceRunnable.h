 ...
 *
 *
 *  In contrast to JNativeRunnable, the runnable object immediately releases the native callable object
 *  after run() is called once. That has the advantage that the object does not remain referenced until
 *  the Java garbage collector eventually removes it. So JNativeOnceRunnable provides better performance
 *  and more deterministic behaviour for cases when the runnable is only used once. *
 *
 *  If run is called multiple times then subsequence run() calls have no effect.
 * */
...
public:
    ...
    explicit JNativeOnceRunnable( const std::function<void()>& func)
     : JNativeRunnable( newInstance_(func) )
    {
    }

    explicit JNativeOnceRunnable( ISimpleCallable* pCallable )
     : JNativeRunnable( newInstance_(pCallable) )
    {
    }
    ...
    *      */
    explicit JNativeOnceRunnable(const Reference& objectRef)
    : JNativeRunnable(objectRef)
    {
    }
