    ...
    template <typename CharType> class AsyncStdioReader : public Base
    {
      public:
        ...
        P<IAsyncOp<String>> readLine()
        {
            ...
            // will set the eof flag when that data is exceeded. For the only
            // threadless platform that we currently support at the time of this
            // writing (webems) this is true for all the stdio streams (since
            // they are not connected to anything) and the "file" streams (those
            // write to temporary local storage). The only case where we could
            // potentially block for long is for custom stream implementations
            // that manually get their data from somewhere, without setting eof
            // when they reach the end. But there is no way around that.
            pOp->run();
#endif
