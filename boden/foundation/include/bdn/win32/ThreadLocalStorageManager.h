        ...
        class ThreadLocalStorageManager : public Base
        {
          public:
            struct ValueHolder
            {
                ...
                ~ValueHolder() { _deleteValue(); }

                void *getValue() { return _pValue; }
            ...
            ThreadLocalStorageManager()
            {
                ...
                // we use FlsAlloc instead of TlsAlloc. It has the advantage
                // that we can pass a cleanup callback. When no fibers are used
                // then it behaves exactly like TlsAlloc (plus the cleanup).
                // When fibers are used then the storage will be fiber-local,
                // which is an added benefit.
                _key =
                    ::FlsAlloc(&ThreadLocalStorageManager::_threadExitCleanup);
                if (_key == FLS_OUT_OF_INDEXES) {
                    BDN_WIN32_throwLastError(
                        ErrorFields()
                            .add("func", "FlsAlloc")
                            .add("action",
                                 "ThreadLocalStorageManager constructor"));
                }
            }

            ~ThreadLocalStorageManager() { ::FlsFree(_key); }
            ...
            /** Creates a new slot for a value.
                ...
                */
            int createSlot() { return _nextSlotId.fetch_add(1); }

            /** Returns a reference to the thread-local pointer for the
             * specified slot.
             */
            ValueHolder &getThreadValueHolder(int slot)
            {
                std::unordered_map<int, ValueHolder> *pMap =
                    static_cast<std::unordered_map<int, ValueHolder> *>(
                        ::FlsGetValue(_key));
                ...
                if (pMap == nullptr) {
                    ...
                    if (!::FlsSetValue(_key, pMap)) {
                        BDN_WIN32_throwLastError(
                            ErrorFields()
                                .add("func", "FlsSetValue")
                                .add("action", "ThreadLocalStorageManager::"
                                               "getThreadValueHolder"));
                    }
                }
            ...
            /** Returns a reference to a global local storage manager
             * instance.*/
          ...
          protected:
            ...
            {
                std::unordered_map<int, ValueHolder> *pMap =
                    static_cast<std::unordered_map<int, ValueHolder> *>(pValue);
