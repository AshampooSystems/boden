                             ...
                             BDN_IMPLEMENTS ISwitchCore
        {
          public:
            ...
            void setState(const TriState &state) override
            {
                ...
                switch (state) {
                case TriState::on:
                    win32CheckedState = BST_CHECKED;
                    break;
                case TriState::off:
                    win32CheckedState = BST_UNCHECKED;
                    break;
                case TriState::mixed:
                    win32CheckedState = BST_INDETERMINATE;
                    break;
                }
            ...
            void generateClick()
            {
                ...
                if (pOuter != nullptr) {
                    ...
                    // Click is triggered even if the user uses the keyboard to
                    // check the checkbox, so we set the outer's state here to
                    // inform the framework about the state change.
            ...
            {
                UINT win32CheckedState =
                    SendMessage(getHwnd(), BM_GETCHECK, 0, 0);
                TriState state;
            ...
            Size calcPreferredSize(const Size &availableSpace) const
            {
                ...
                prefSize.width +=
                    checkboxWidth +
                    4; // 4 = magic for gap between checkbox and setLabel
                ...
                UiMargin uiPadding;
                if (pad.isNull()) {
                    uiPadding = UiMargin(UiLength::sem(0), UiLength::sem(0));
                } else
                    uiPadding = pad;
          ...
          protected:
                                     ...
                                     UINT message, WPARAM wParam, LPARAM lParam)
            {
                if (message == WM_COMMAND) {
                    // Set state on HWND
                    setState(_state() == TriState::off ||
                                     _state() == TriState::mixed
                                 ? TriState::on
                                 : TriState::off);
                    // we were clicked.
                    generateClick();
