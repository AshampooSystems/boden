...
class PagesDataSource : public ListViewDataSource
{
  public:
    std::array<std::pair<String, std::function<std::shared_ptr<View>()>>, 5> pages = {
        std::make_pair("UI Demo", [=]() { return createUiDemoPage(_window); }),
  ...
  public:
    ...
    std::shared_ptr<View> viewForRowIndex(size_t rowIndex, std::shared_ptr<View> reusableView) override
    {
        ...
        if (!itemView) {
            ...
            itemView->addChildView(itemView->label.get());
        }
        itemView->label->text = pages[rowIndex].first;

        return std::dynamic_pointer_cast<View>(itemView);
    }

    float heightForRowIndex(size_t rowIndex) override { return 50; }

  private:
    class ItemView : public ContainerView
    {
      public:
        ItemView()
        {
        ...
        Property<std::shared_ptr<Label>> label;
