...
template<>
struct MakeSigned_<double>
{
	using Type = double;
};

template<>
struct MakeSigned_<float>
{
	using Type = float;
};

template<>
struct MakeSigned_<long double>
{
...
template<>
struct MakeUnsigned_<double>
{
	using Type = double;
};

template<>
struct MakeUnsigned_<float>
{
	using Type = float;
};

template<>
struct MakeUnsigned_<long double>
{
	using Type = long double;
};
		...
		<
			...
			typename std::conditional
				<
					...
					typename MakeUnsigned_<ArgNumberType>::Type
				>::type
		>::type;
};
...
{	
	SECTION("subtypes")
	{
		SECTION("SimpleType")
		{
			REQUIRE( typeid(typename ObjectType::SimpleType) == typeid(BaseSimpleType) );
		}
	}
	...
	SECTION("constants")
	{
		...
		// note: min should be lowest(). numeric_limits::min is something else entirely
...
void _testGlobalFloatFunctions()
{
...
void _testGlobalFloatFunctionsWithIntegers()
{
	IntType val = -17;
