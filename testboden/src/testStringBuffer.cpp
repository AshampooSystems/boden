...
{	
	...
	SECTION("after move construct")
	{
		...
		// we HAVE to pass a different section name prefix for each call. Otherwise
		// the section tracker will get confused and issue an assertion.
		_verifyResultImpl(other, expected, "target buffer: " );
		_verifyResultImpl(buf, "", "source buffer: " );

		// also verify that we can still write numbers (i.e. stuff whose formatting depends on the locale).
		other << 1234;
...
static void _verifySeekResult(
	bool readSeek,
	StringBuffer& buf,
	std::basic_streambuf<char32_t, UnicodeCharTraits>* pBuffer,
	const String& contents,
	std::streamoff toIndex,
	std::streamoff result )
{
	if(toIndex<0 || toIndex>contents.length())
	{
		REQUIRE( result==-1 );
	}
	else
...
{	
	...
	{
		if(fromPosData.index<0 || fromPosData.index>contents.length())
			continue;
				...
				SECTION("to "+toPosData.desc)
				{
					...
					if(which != (std::ios_base::in | std::ios_base::out) )
					{
						...
						{	
							_verifySeekResult(
								...
								toPosData.index,
								pBuffer->pubseekoff(
									toPosData.index - fromPosData.index,
									std::ios_base::cur,
									which ) );
						}
					}
					...
					{	
						_verifySeekResult(
							...
							toPosData.index,
							pBuffer->pubseekoff(
								toPosData.index-contents.length(),
								std::ios_base::end,
								which ) );
					}
				}
...
template<typename VALUE_TYPE>
void _verifyStringifyUnsupportedObject( VALUE_TYPE value, bool prefixOnly = false )
{
...
template<>
struct VerifyStringifyObject_<false>
{
	template<class VALUE_TYPE>
	static void verify(VALUE_TYPE value, const String& expectedResult)
	{
...
TEST_CASE("StringBuffer")
{
	...
	SECTION("formatted number")
	{
		...
		// SECTION("ad hoc")
		// 	_verifyResult(StringBuffer() << StreamFormat().zeroPad(8) << 42, "00000042" );
	}
	...
	SECTION("multiple puts")
	{
        ...
        // streams. See https://systems.atlassian.net/browse/BDN-57
		//SECTION("ad hoc")
		//	_verifyResult(StringBuffer() << "hello " << StreamFormat().zeroPad(4) << 17 << " world", "hello 0017 world");
	}
